...
bpar.initialise1.func<-function()
  ...
  if (g==1) bpar$gpoints.col.bg<<-"red"
    else bpar$gpoints.col.bg<<-hcl(seq(0,360,length=g+2)[-c(1,g+2)],200,60)
  bpar$gpoints.label.font<<-rep(1,g)
  bpar$gpoints.label.cex<<-rep(.85,g)
  if (g==1) bpar$gpoints.label.col<<-"black"
    else bpar$gpoints.label.col<<-hcl(seq(0,360,length=g+2)[-c(1,g+2)],200,60)
  ...
  if (g==1) bpar$gSampleGroupMeans.col.bg<<-"red"
    else bpar$gSampleGroupMeans.col.bg<<-hcl(seq(0,360,length=g+2)[-c(1,g+2)],200,60)
  bpar$gSampleGroupMeans.label.font<<-rep(2,g)
  bpar$gSampleGroupMeans.label.cex<<-rep(1,g)
  if (g==1) bpar$gSampleGroupMeans.label.col<<-"black"
    else bpar$gSampleGroupMeans.label.col<<-hcl(seq(0,360,length=g+2)[-c(1,g+2)],200,60)
  ...
  if (g==1) bpar$gConvexHullAlphaBag.col.fg<<-hcl(0,0,60)
    else bpar$gConvexHullAlphaBag.col.fg<<-hcl(seq(0,360,length=g+2)[-c(1,g+2)],200,60)
  if (g==1) bpar$gConvexHullAlphaBag.col.bg<<-hcl(0,0,85)
    else bpar$gConvexHullAlphaBag.col.bg<<-hcl(seq(0,360,length=g+2)[-c(1,g+2)],20,85)
  bpar$gConvexHullAlphaBag.TukeyMedian.pch<<-rep(0,g)
  bpar$gConvexHullAlphaBag.TukeyMedian.cex<<-rep(2,g)
  if (g==1) bpar$gConvexHullAlphaBag.TukeyMedian.col.fg<<-"red"
    else bpar$gConvexHullAlphaBag.TukeyMedian.col.fg<<-hcl(seq(0,360,length=g+2)[-c(1,g+2)],200,60)
  bpar$gConvexHullAlphaBag.TukeyMedian.col.bg<<-rep(NA,g)
  bpar$gConvexHullAlphaBag.TukeyMedian.label.font<<-rep(4,g)
  bpar$gConvexHullAlphaBag.TukeyMedian.label.cex<<-rep(1,g)
  if (g==1)  bpar$gConvexHullAlphaBag.TukeyMedian.label.col<<-"black"
    else bpar$gConvexHullAlphaBag.TukeyMedian.label.col<<-hcl(seq(0,360,length=g+2)[-c(1,g+2)],200,60)
  bpar$gConvexHullAlphaBag.TukeyMedian.label.HorizOffset<<-rep(0,g)
...
bpar.initialise2.func<-function()
  ...
  bpar$axes.tick.RelLength<<-rep(.0075,p)
  bpar$axes.marker.font<<-rep(1,p)
  bpar$axes.marker.cex<<-rep(.75,p)
  bpar$axes.marker.col<<-bpar$axes.col
  bpar$axes.marker.RelOffset<<-rep(.005,p)
  bpar$axes.label.font<<-rep(1,p)
  bpar$axes.label.cex<<-rep(.75,p)
...
bpar.initialise3.func<-function()
  ...
  bpar$interaction.highlight.ShowValues.cex<<-.75
...
bpar.initialise4.func<-function()
  ...
  bpar$DiagnosticTabs.predictivities.label.cex<<-.85
  ...
  bpar$DiagnosticTabs.ShepardDiagram.disparities.cex<<-.4
  ...
  bpar$DiagnosticTabs.ShepardDiagram.digits<<-3 # used?
...
bparp.func<-function()
  ...
  bparp$points.label.font<<-rep(bpar$gpoints.label.font,times=g.n)[temp1]
  bparp$points.label.cex<<-rep(bpar$gpoints.label.cex,times=g.n)[temp1]
  bparp$points.label.col<<-rep(bpar$gpoints.label.col,times=g.n)[temp1]
  bparp$points.label.HorizOffset<<-rep(bpar$gpoints.label.HorizOffset,times=g.n)[temp1]
...
getLabelUsr<-function(xi,yi,parin,.labels,.cex,horiz,vert,showconv=FALSE)
  ...
  c1i<-strwidth(.labels,cex=.cex,units="figure")*parin$fin[1]/parin$pin[1]
  c2i<-strheight(.labels,cex=.cex,units="figure")*parin$fin[2]/parin$pin[2]

  SSE<-Inf
  count<-0
  while (SSE>eps)
    {
    usr1new<-suppressWarnings(min(xi+strwidth("x",cex=.cex)*horiz+as.numeric(!is.na(.labels))*(-.5*c1i)*(usr2old-usr1old),xi))
    usr2new<-suppressWarnings(max(xi+strwidth("x",cex=.cex)*horiz+as.numeric(!is.na(.labels))*(.5*c1i)*(usr2old-usr1new),xi))
    SSE<-(usr1new-usr1old)^2+(usr2new-usr2old)^2
    count<-count+1
    if (showconv) cat("x :",count,":",SSE,"\n")
    usr1old<-usr1new
    usr2old<-usr2new
    }
  SSE<-Inf
  ...
  while (SSE>eps)
    {
    usr3new<-suppressWarnings(min(yi+strheight("x",cex=.cex)*vert+as.numeric(!is.na(.labels))*(-.5*c2i)*(usr4old-usr3old),yi))
    usr4new<-suppressWarnings(max(yi+strheight("x",cex=.cex)*vert+as.numeric(!is.na(.labels))*(.5*c2i)*(usr4old-usr3new),yi))
    SSE<-(usr3new-usr3old)^2+(usr4new-usr4old)^2
    count<-count+1
    if (showconv) cat("y :",count,":",SSE,"\n")
    usr3old<-usr3new
    usr4old<-usr4new
    }
...
mynewplot<-function(x,y,xlimtouse=NA,ylimtouse=NA,fitaroundlabels=FALSE,.labels=NA,labels.cex,HorizOffset,VertOffset,...)
  ...
  if (!missing(xlimtouse) && !missing(ylimtouse)) plot(x=x,y=y,xlim=xlimtouse,ylim=ylimtouse,asp=1,type="n",xlab="",ylab="",...)
            ...
            {
            if (any(names(b)%in%c("xaxt","yaxt"))) bwithoutaxt<-b[which(names(b)!="xaxt" & names(b)!="yaxt")]
              else bwithoutaxt<-b
            do.call("plot",c(list(x=x,y=y,type="n",asp=1,xlab="",ylab="",xaxt="n",yaxt="n"),bwithoutaxt))
            xylim<-getLabelUsr(xi=x,yi=y,parin=par(),.labels=.labels,.cex=labels.cex,HorizOffset,VertOffset,showconv=FALSE)

            if (any(names(bwithoutaxt)=="pch")) bwithoutaxtpch<-bwithoutaxt[which(names(bwithoutaxt)!="pch")]
              else bwithoutaxtpch<-bwithoutaxt
            do.call("plot.window",c(list(xlim=xylim[[1]],ylim=xylim[[2]]),asp=1,bwithoutaxtpch))

            if (!any(names(b)=="xaxt") | (any(names(b)=="xaxt") &&  b$xaxt!="n")) axis(side=1)
            if (!any(names(b)=="yaxt") | (any(names(b)=="yaxt") &&  b$yaxt!="n")) axis(side=2)
            }
...
linebreak<-function(stringin="",cutoff=60)
    ...
    temp1[-length(temp1)]<-paste(temp1[-length(temp1)]," ",sep="")
    nwords<-length(temp1)
    currentword<-1
    ...
    repeat
      ...
      if (length(temp3)==0) break
        else temp3<-temp3[1]
      temp1[currentindices[temp3]-1]<-paste(temp1[currentindices[temp3]-1],"\n",sep="")
      currentword<-currentindices[temp3]
      if (currentword>=nwords) break
      currentindices<-currentword:nwords
      }
...
mytktip<-function(widget,...)
    ...
    if (length(temp1)>1) temp3<-paste(unlist(temp2),collapse=" \n\n")
...
if (GUI.AvailableScreenWidth/GUI.AvailableScreenHeight<=1080/720)
  {
  GUI.ScreenWidth<-min(1080,round(GUI.AvailableScreenWidth*.9))
  GUI.ScreenHeight<-min(720,round(GUI.AvailableScreenWidth/1080*720*.9))
  }
  else
    {
    GUI.ScreenWidth<-min(1080,round(GUI.AvailableScreenHeight/720*1080*.9))
    GUI.ScreenHeight<-min(720,round(GUI.AvailableScreenHeight*.9))
    }
...
GUI.BindingsOn<-function()
  ...
  tkbind(GUI.TopLevel,"<Control-G>",function() tkinvoke(MenuBar.Format,1))
  tkbind(GUI.TopLevel,"<Control-a>",function() tkinvoke(MenuBar.Format,2))
  tkbind(GUI.TopLevel,"<Control-A>",function() tkinvoke(MenuBar.Format,2))
  tkbind(GUI.TopLevel,"<Control-r>",function() tkinvoke(MenuBar.Format,6))
  tkbind(GUI.TopLevel,"<Control-R>",function() tkinvoke(MenuBar.Format,6))
  tkbind(GUI.TopLevel,"a",function() tkinvoke(MenuBar.Points,2))
  tkbind(GUI.TopLevel,"A",function() tkinvoke(MenuBar.Points,2))
  tkbind(GUI.TopLevel,"b",function() tkinvoke(MenuBar.Points.MDS,2))
  tkbind(GUI.TopLevel,"B",function() tkinvoke(MenuBar.Points.MDS,2))
  tkbind(GUI.TopLevel,"c",function() tkinvoke(MenuBar.Points.MDS,3))
  tkbind(GUI.TopLevel,"C",function() tkinvoke(MenuBar.Points.MDS,3))
  tkbind(GUI.TopLevel,"d",function() tkinvoke(MenuBar.Points.MDS,4))
  tkbind(GUI.TopLevel,"D",function() tkinvoke(MenuBar.Points.MDS,4))
  tkbind(GUI.TopLevel,"0",function() tkinvoke(MenuBar.Axes,0))
  tkbind(GUI.TopLevel,"1",function() tkinvoke(MenuBar.Joint,0))
  tkbind(GUI.TopLevel,"2",function() tkinvoke(MenuBar.Joint,1))
  tkbind(GUI.TopLevel,"3",function() tkinvoke(MenuBar.Joint,3))
  tkbind(GUI.TopLevel,"4",function() tkinvoke(MenuBar.Axes,2))
  tkbind(GUI.TopLevel,"5",function() tkinvoke(MenuBar.Axes,3))
  tkbind(GUI.TopLevel,"6",function() tkinvoke(MenuBar.Axes,5))
  tkbind(GUI.TopLevel,"<Control-n>",function() tkinvoke(MenuBar.Additional.Interpolate,0))
  tkbind(GUI.TopLevel,"<Control-N>",function() tkinvoke(MenuBar.Additional.Interpolate,0))
  tkbind(GUI.TopLevel,"<Control-l>",function() tkinvoke(MenuBar.Additional,8))
  tkbind(GUI.TopLevel,"<Control-L>",function() tkinvoke(MenuBar.Additional,8))
  tkbind(GUI.TopLevel,"<F1>",function() tkinvoke(MenuBar.Help,0))
...
GUI.update<-function()
  ...
  if (Biplot.axes.mode==0) tkentryconfigure(MenuBar.View,6,state="disabled")
  ...
  if (tclvalue(Biplot.Axes.var)%in%c("10","13","14") | tclvalue(Other.HideAxes.var)=="1")
    ...
    tkentryconfigure(Biplot.RightClickOutside.Menu,6,state="disabled")
      ...
      tkentryconfigure(Biplot.RightClickOutside.Menu,6,state="normal")
      }

  for (temp1 in 10:12)
    {
    tkentryconfigure(MenuBar.View,temp1,variable=View.AxisLabels.var)
    tkentryconfigure(Biplot.RightClickOutside.Menu,temp1-5,variable=View.AxisLabels.var)
    }

  for (temp1 in c(0:1,3)) tkentryconfigure(MenuBar.Joint,temp1,variable=Biplot.Axes.var)

  if (tclvalue(Points.var)=="11") for (temp1 in 6:7) tkentryconfigure(MenuBar.Points.MDS,temp1,state="normal",variable=Points.MDS.ApproachToTies.var)
    else for (temp1 in 6:7) tkentryconfigure(MenuBar.Points.MDS,temp1,state="disabled",variable=Points.MDS.ApproachToTies.var)

  if (tclvalue(Points.DistanceMetric.var)=="3" && tclvalue(Points.var)=="0") for (temp1 in 2:3) tkentryconfigure(MenuBar.Axes,temp1,state="disabled")
    else for (temp1 in 2:3) tkentryconfigure(MenuBar.Axes,temp1,state="normal")

  if ((tclvalue(Points.DistanceMetric.var)=="3" && tclvalue(Points.var)=="0") || tclvalue(Points.var)%in%c("10","11","12")) for (temp1 in 5) tkentryconfigure(MenuBar.Axes,temp1,state="disabled")
    else for (temp1 in 5) tkentryconfigure(MenuBar.Axes,temp1,state="normal")

  for (temp1 in c(0,2:3,5)) tkentryconfigure(MenuBar.Axes,temp1,variable=Biplot.Axes.var)

  if (tclvalue(Biplot.Axes.var)=="10") for (temp1 in 0:1) tkentryconfigure(MenuBar.Additional.Interpolate,temp1,state="disabled")
    else for (temp1 in 0:1) tkentryconfigure(MenuBar.Additional.Interpolate,temp1,state="normal")

  if (tclvalue(Biplot.Axes.var)=="2") tkentryconfigure(MenuBar.Additional,6,state="normal")
...
File.Save.as.cmd<-function() # Called only by the keyboard shortcut `Ctrl+S'.
  ...
  switch(tclvalue(File.SaveAs.var),
    "0"=File.SaveAs.PDF.cmd(),
    "1"=File.SaveAs.Postscript.cmd(),
    "2"=File.SaveAs.Metafile.cmd(),
    "3"=File.SaveAs.Bmp.cmd(),
    "4"=File.SaveAs.Png.cmd(),
    "5"=File.SaveAs.Jpeg.cmd(),
    "6"=File.SaveAs.Jpeg.cmd(),
...
File.SaveAs.Postscript.cmd<-function()
  ...
  if (nchar(FileName))
    ...
    postscript(file=FileName,width=8,height=8,horizontal=FALSE,onefile=FALSE,paper="default",family="URWHelvetica")
    Biplot.plot(screen=FALSE)
    dev.off()
    }
  tkfocus(GUI.TopLevel)
...
File.SaveAs.Jpeg.cmd<-function()
  ...
  if (nchar(FileName))
    ...
    jpeg(FileName,width=8,height=8,units="in",restoreConsole=FALSE,res=96,quality=File.Jpeg.quality)
    Biplot.plot(screen=FALSE)
    dev.off()
    }
  tkfocus(GUI.TopLevel)
...
File.SaveAs.PicTeX.cmd<-function()
  ...
  if (nchar(FileName))
    {
    nn<-nchar(FileName)
    if (nn<5 || substr(FileName,nn-3,nn)!=".tex") FileName<-paste(FileName,".tex",sep="")
    pictex(FileName,width=8,height=8,debug=FALSE,bg="white",fg="black")
    Biplot.plot(screen=FALSE)
    dev.off()
    }
  tkfocus(GUI.TopLevel)
...
# cat("ENDLISTING Controls the dialogue box. BEGINLISTING")
  local.GUI.func<-function()
    ...
    onDefaults<-function()
      ...
      local.ProcrustesConvergence.var<<-tclVar(1e-6)
    ...
    onOK<-function()
      ...
      boptions$ThreeD.MouseButtonAction<<-c(tclvalue(tkget(combo1)),tclvalue(tkget(combo2)),tclvalue(tkget(combo3)))
    ...
    tkplace(tk2label(frame1,text="MDS relative convergence"),x=11,y=20,"in"=frame1)
    entry1<-tk2entry(frame1,textvariable=local.MDSConvergence.var,justify="right",takefocus=FALSE)
    tkplace(entry1,relx=.95,y=20,height=18,relwidth=.125,"in"=frame1,anchor="ne")
    tkplace(tk2label(frame1,text="MDS maximum iterations"),x=11,y=40,"in"=frame1)
    entry2<-tk2entry(frame1,textvariable=local.MDSMaximumIterations.var,justify="right",takefocus=FALSE)
    tkplace(entry2,relx=.95,y=40,height=18,relwidth=.125,"in"=frame1,anchor="ne")
    tkplace(tk2label(frame1,text="Procrustes absolute convergence"),x=11,y=60,"in"=frame1)
    entry3<-tk2entry(frame1,textvariable=local.ProcrustesConvergence.var,justify="right",takefocus=FALSE)
    tkplace(entry3,relx=.95,y=60,height=18,relwidth=.125,"in"=frame1,anchor="ne")
    tkplace(tk2label(frame1,text="Procrustes maximum iterations"),x=11,y=80,"in"=frame1)
    entry4<-tk2entry(frame1,textvariable=local.ProcrustesMaximumIterations.var,justify="right",takefocus=FALSE)
    tkplace(entry4,relx=.95,y=80,height=18,relwidth=.125,"in"=frame1,anchor="ne")
    frame2<-tkwidget(top,"TitleFrame",text="Graphical")
    tkplace(frame2,relx=.05,relwidth=.90,y=130,height=125,"in"=top)
    tkplace(tk2label(frame2,text="Iterations to live update"),x=11,y=20,"in"=frame2)
    entry5<-tk2entry(frame2,textvariable=local.IterationsToLiveUpdate.var,justify="right",takefocus=FALSE)
    tkplace(entry5,relx=.95,y=20,height=18,relwidth=.125,"in"=frame2,anchor="ne")
    tkplace(tk2label(frame2,text="Re-use external windows"),x=11,y=40,"in"=frame2)
    checkbutton1<-tk2checkbutton(frame2,variable=local.ReuseExternalWindows.var)
    tkplace(checkbutton1,relx=.90,y=40,height=17,"in"=frame2)
    tkplace(tk2label(frame2,text="Three-dimensional `fly-by'"),x=11,y=60,"in"=frame2)
    checkbutton2<-tk2checkbutton(frame2,variable=local.ThreeDFlyBy.var)
    tkplace(checkbutton2,relx=.90,y=60,height=17,"in"=frame2)
    tkplace(tk2label(frame2,text="Three-dimensional mouse button action:"),x=11,y=80,"in"=frame2)
    MousePossibilities<-c("none","trackball","xAxis","yAxis","zAxis","polar","zoom","fov")
    tkplace(tk2label(frame2,text="Left"),x=11,y=100,"in"=frame2)
    combo1<-tkwidget(frame2,"ComboBox",editable=FALSE,values=MousePossibilities,text=boptions$ThreeD.MouseButtonAction[1])
    tkplace(combo1,relx=.11,y=100,relwidth=.2,height=17,"in"=frame2)
    tkplace(tk2label(frame2,text="Middle"),relx=.33,y=100,"in"=frame2)
    combo2<-tkwidget(frame2,"ComboBox",editable=FALSE,values=MousePossibilities,text=boptions$ThreeD.MouseButtonAction[2])
    tkplace(combo2,relx=.44,y=100,relwidth=.2,height=17,"in"=frame2)
    tkplace(tk2label(frame2,text="Right"),relx=.66,y=100,"in"=frame2)
    combo3<-tkwidget(frame2,"ComboBox",editable=FALSE,values=MousePossibilities,text=boptions$ThreeD.MouseButtonAction[3])
    tkplace(combo3,relx=.755,y=100,relwidth=.2,height=17,"in"=frame2)
...
File.Exit.cmd<-function()
  {
  temp1<-tkmessageBox(icon="question",message="Are you sure?",parent=GUI.TopLevel,title="Exit",type="yesno")
  if (tclvalue(temp1)=="yes") tkdestroy(GUI.TopLevel)
   else GUI.BindingsOn()
  }
...
Format.Title.cmd<-function()
 ...
 # cat("ENDLISTING Sets the local variable to its default value when the Default button is clicked. BEGINLISTING")
    ...
    button3<-tk2button(top,text="Default",width=10,command=onDefault)

    tkplace(frame1,relx=.5,rely=.4,relwidth=.9,relheight=.4,anchor="center")
    tkplace(label1,relx=.05,rely=.5,"in"=frame1,anchor="w")
    tkplace(entry1,relx=.5,rely=.5,relwidth=.45,"in"=frame1,anchor="w")
    tkplace(button1,relx=.7,rely=.85,anchor="e")
...
# cat("ENDLISTING Controls the dialogue box. BEGINLISTING")
  local.GUI.func<-function()
    ...
    UpdateEntryBoxes<-function()
      ...
      if (WhichGroup==1 && tclvalue(local.ConvexHullAlphaBag.TukeyMedian.cex.var[[1]])!=" ") for (temp1 in 2:(g+1)) local.ConvexHullAlphaBag.TukeyMedian.cex.var[[temp1]]<<-tclVar(tclvalue(local.ConvexHullAlphaBag.TukeyMedian.cex.var[[1]]))
      if (WhichGroup==1 && tclvalue(local.ConvexHullAlphaBag.TukeyMedian.label.cex.var[[1]])!=" ") for (temp1 in 2:(g+1)) local.ConvexHullAlphaBag.TukeyMedian.label.cex.var[[temp1]]<<-tclVar(tclvalue(local.ConvexHullAlphaBag.TukeyMedian.label.cex.var[[1]]))
    ...
    ChangeGroup<-function()
      ...
      local.points.cex.var[[1]]<<-if (all(unlist(lapply(local.points.cex.var[-1],tclvalue))==tclvalue(local.points.cex.var[[2]]))) local.points.cex.var[[2]] else tclVar(" ")
      tkconfigure(entryA1,textvariable=local.points.cex.var[[WhichGroup]])

      local.points.col.fg.var[[1]]<<-if (all(unlist(local.points.col.fg.var[-1])==local.points.col.fg.var[[2]])) local.points.col.fg.var[[2]] else text2hex("SystemButtonFace")
      tkconfigure(labelA1,background=local.points.col.fg.var[[WhichGroup]])

      local.points.col.bg.var[[1]]<<-if (all(unlist(local.points.col.bg.var[-1])==local.points.col.bg.var[[2]])) local.points.col.bg.var[[2]] else text2hex("SystemButtonFace")
      tkconfigure(labelA2,background=local.points.col.bg.var[[WhichGroup]])

      local.points.label.font.var[[1]]<<-if (all(unlist(lapply(local.points.label.font.var[-1],tclvalue))==tclvalue(local.points.label.font.var[[2]]))) local.points.label.font.var[[2]] else tclVar(" ")
      tkconfigure(spinboxA2,textvariable=local.points.label.font.var[[WhichGroup]])

      local.points.label.cex.var[[1]]<<-if (all(unlist(lapply(local.points.label.cex.var[-1],tclvalue))==tclvalue(local.points.label.cex.var[[2]]))) local.points.label.cex.var[[2]] else tclVar(" ")
      tkconfigure(entryA2,textvariable=local.points.label.cex.var[[WhichGroup]])

      local.points.label.col.var[[1]]<<-if (all(unlist(local.points.label.col.var[-1])==local.points.label.col.var[[2]])) local.points.label.col.var[[2]] else text2hex("SystemButtonFace")
      tkconfigure(labelA3,background=local.points.label.col.var[[WhichGroup]])

      local.points.label.HorizOffset.var[[1]]<<-if (all(unlist(lapply(local.points.label.HorizOffset.var[-1],tclvalue))==tclvalue(local.points.label.HorizOffset.var[[2]]))) local.points.label.HorizOffset.var[[2]] else tclVar(" ")
      ...
      local.SampleGroupMeans.cex.var[[1]]<<-if (all(unlist(lapply(local.SampleGroupMeans.cex.var[-1],tclvalue))==tclvalue(local.SampleGroupMeans.cex.var[[2]]))) local.SampleGroupMeans.cex.var[[2]] else tclVar(" ")
      tkconfigure(entryB1,textvariable=local.SampleGroupMeans.cex.var[[WhichGroup]])

      local.SampleGroupMeans.col.fg.var[[1]]<<-if (all(unlist(local.SampleGroupMeans.col.fg.var[-1])==local.SampleGroupMeans.col.fg.var[[2]])) local.SampleGroupMeans.col.fg.var[[2]] else text2hex("SystemButtonFace")
      tkconfigure(labelB1,background=local.SampleGroupMeans.col.fg.var[[WhichGroup]])

      local.SampleGroupMeans.col.bg.var[[1]]<<-if (all(unlist(local.SampleGroupMeans.col.bg.var[-1])==local.SampleGroupMeans.col.bg.var[[2]])) local.SampleGroupMeans.col.bg.var[[2]] else text2hex("SystemButtonFace")
      tkconfigure(labelB2,background=local.SampleGroupMeans.col.bg.var[[WhichGroup]])

      local.SampleGroupMeans.label.font.var[[1]]<<-if (all(unlist(lapply(local.SampleGroupMeans.label.font.var[-1],tclvalue))==tclvalue(local.SampleGroupMeans.label.font.var[[2]]))) local.SampleGroupMeans.label.font.var[[2]] else tclVar(" ")
      tkconfigure(spinboxB2,textvariable=local.SampleGroupMeans.label.font.var[[WhichGroup]])

      local.SampleGroupMeans.label.cex.var[[1]]<<-if (all(unlist(lapply(local.SampleGroupMeans.label.cex.var[-1],tclvalue))==tclvalue(local.SampleGroupMeans.label.cex.var[[2]]))) local.SampleGroupMeans.label.cex.var[[2]] else tclVar(" ")
      tkconfigure(entryB2,textvariable=local.SampleGroupMeans.label.cex.var[[WhichGroup]])

      local.SampleGroupMeans.label.col.var[[1]]<<-if (all(unlist(local.SampleGroupMeans.label.col.var[-1])==local.SampleGroupMeans.label.col.var[[2]])) local.SampleGroupMeans.label.col.var[[2]] else text2hex("SystemButtonFace")
      tkconfigure(labelB3,background=local.SampleGroupMeans.label.col.var[[WhichGroup]])

      local.SampleGroupMeans.label.HorizOffset.var[[1]]<<-if (all(unlist(lapply(local.SampleGroupMeans.label.HorizOffset.var[-1],tclvalue))==tclvalue(local.SampleGroupMeans.label.HorizOffset.var[[2]]))) local.SampleGroupMeans.label.HorizOffset.var[[2]] else tclVar(" ")
      ...
      local.ConvexHullAlphaBag.lwd.var[[1]]<<-if (all(unlist(lapply(local.ConvexHullAlphaBag.lwd.var[-1],tclvalue))==tclvalue(local.ConvexHullAlphaBag.lwd.var[[2]]))) local.ConvexHullAlphaBag.lwd.var[[2]] else tclVar(" ")
      tkconfigure(entryC1,textvariable=local.ConvexHullAlphaBag.lwd.var[[WhichGroup]])

      local.ConvexHullAlphaBag.col.fg.var[[1]]<<-if (all(unlist(local.ConvexHullAlphaBag.col.fg.var[-1])==local.ConvexHullAlphaBag.col.fg.var[[2]])) local.ConvexHullAlphaBag.col.fg.var[[2]] else text2hex("SystemButtonFace")
      ...
      local.ConvexHullAlphaBag.TukeyMedian.cex.var[[1]]<<-if (all(unlist(lapply(local.ConvexHullAlphaBag.TukeyMedian.cex.var[-1],tclvalue))==tclvalue(local.ConvexHullAlphaBag.TukeyMedian.cex.var[[2]]))) local.ConvexHullAlphaBag.TukeyMedian.cex.var[[2]] else tclVar(" ")
      tkconfigure(entryC2,textvariable=local.ConvexHullAlphaBag.TukeyMedian.cex.var[[WhichGroup]])

      local.ConvexHullAlphaBag.TukeyMedian.col.fg.var[[1]]<<-if (all(unlist(local.ConvexHullAlphaBag.TukeyMedian.col.fg.var[-1])==local.ConvexHullAlphaBag.TukeyMedian.col.fg.var[[2]])) local.ConvexHullAlphaBag.TukeyMedian.col.fg.var[[2]] else text2hex("SystemButtonFace")
      tkconfigure(labelC3,background=local.ConvexHullAlphaBag.TukeyMedian.col.fg.var[[WhichGroup]])

      local.ConvexHullAlphaBag.TukeyMedian.col.bg.var[[1]]<<-if (all(unlist(local.ConvexHullAlphaBag.TukeyMedian.col.bg.var[-1])==local.ConvexHullAlphaBag.TukeyMedian.col.bg.var[[2]])) local.ConvexHullAlphaBag.TukeyMedian.col.bg.var[[2]] else text2hex("SystemButtonFace")
      tkconfigure(labelC4,background=local.ConvexHullAlphaBag.TukeyMedian.col.bg.var[[WhichGroup]])

      local.ConvexHullAlphaBag.TukeyMedian.label.font.var[[1]]<<-if (all(unlist(lapply(local.ConvexHullAlphaBag.TukeyMedian.label.font.var[-1],tclvalue))==tclvalue(local.ConvexHullAlphaBag.TukeyMedian.label.font.var[[2]]))) local.ConvexHullAlphaBag.TukeyMedian.label.font.var[[2]] else tclVar(" ")
      tkconfigure(spinboxC3,textvariable=local.ConvexHullAlphaBag.TukeyMedian.label.font.var[[WhichGroup]])

      local.ConvexHullAlphaBag.TukeyMedian.label.cex.var[[1]]<<-if (all(unlist(lapply(local.ConvexHullAlphaBag.TukeyMedian.label.cex.var[-1],tclvalue))==tclvalue(local.ConvexHullAlphaBag.TukeyMedian.label.cex.var[[2]]))) local.ConvexHullAlphaBag.TukeyMedian.label.cex.var[[2]] else tclVar(" ")
      tkconfigure(entryC3,textvariable=local.ConvexHullAlphaBag.TukeyMedian.label.cex.var[[WhichGroup]])

      local.ConvexHullAlphaBag.TukeyMedian.label.col.var[[1]]<<-if (all(unlist(local.ConvexHullAlphaBag.TukeyMedian.label.col.var[-1])==local.ConvexHullAlphaBag.TukeyMedian.label.col.var[[2]])) local.ConvexHullAlphaBag.TukeyMedian.label.col.var[[2]] else text2hex("SystemButtonFace")
      tkconfigure(labelC5,background=local.ConvexHullAlphaBag.TukeyMedian.label.col.var[[WhichGroup]])

      local.ConvexHullAlphaBag.TukeyMedian.label.HorizOffset.var[[1]]<<-if (all(unlist(lapply(local.ConvexHullAlphaBag.TukeyMedian.label.HorizOffset.var[-1],tclvalue))==tclvalue(local.ConvexHullAlphaBag.TukeyMedian.label.HorizOffset.var[[2]]))) local.ConvexHullAlphaBag.TukeyMedian.label.HorizOffset.var[[2]] else tclVar(" ")
  ...
  onDefaults<-function()
    ...
    onOK<-function()
      ...
      bpar$gpoints.label.font<<-as.numeric(lapply(local.points.label.font.var[-1],tclvalue))
      bpar$gpoints.label.cex<<-as.numeric(lapply(local.points.label.cex.var[-1],tclvalue))
      ...
      bpar$gSampleGroupMeans.label.font<<-as.numeric(lapply(local.SampleGroupMeans.label.font.var[-1],tclvalue))
      bpar$gSampleGroupMeans.label.cex<<-as.numeric(lapply(local.SampleGroupMeans.label.cex.var[-1],tclvalue))
      ...
      bpar$gConvexHullAlphaBag.TukeyMedian.pch<<-as.numeric(lapply(local.ConvexHullAlphaBag.TukeyMedian.pch.var[-1],tclvalue))
      bpar$gConvexHullAlphaBag.TukeyMedian.cex<<-as.numeric(lapply(local.ConvexHullAlphaBag.TukeyMedian.cex.var[-1],tclvalue))
      ...
      bpar$gConvexHullAlphaBag.TukeyMedian.label.font<<-as.numeric(lapply(local.ConvexHullAlphaBag.TukeyMedian.label.font.var[-1],tclvalue))
      bpar$gConvexHullAlphaBag.TukeyMedian.label.cex<<-as.numeric(lapply(local.ConvexHullAlphaBag.TukeyMedian.label.cex.var[-1],tclvalue))
 ...
 # cat("ENDLISTING The Points tab. BEGINLISTING")
   ...
   # cat("ENDLISTING Initialises the local variables. BEGINLISTING")
    ...
    tkplace(tk2label(frameA1,text="Symbol"),x=11,y=20,"in"=frameA1)
    spinboxA1<-tkwidget(frameA1,"SpinBox",textvariable=local.points.pch.var[[WhichGroup]],editable=FALSE,values=c(" ","NA",0:25),justify="right",modifycmd=function()
      {
      if (WhichGroup==1 && tclvalue(local.points.pch.var[[1]])!=" ") for (temp1 in 2:(g+1)) local.points.pch.var[[temp1]]<<-tclVar(tclvalue(local.points.pch.var[[1]]))
      })
    tkplace(spinboxA1,relx=.95,y=20,height=18,relwidth=.125,"in"=frameA1,anchor="ne")

    tkplace(tk2label(frameA1,text="Size"),x=11,y=40,"in"=frameA1)
    entryA1<-tk2entry(frameA1,textvariable=local.points.cex.var[[WhichGroup]],justify="right",takefocus=FALSE)
    tkplace(entryA1,relx=.95,y=40,height=18,relwidth=.125,"in"=frameA1,anchor="ne")

    tkplace(tk2label(frameA1,text="Foreground colour"),x=11,y=60,"in"=frameA1)
    labelA1<-tklabel(frameA1,background=local.points.col.fg.var[[WhichGroup]],relief="groove",borderwidth="1.5p")
    tkplace(labelA1,relx=.95,y=60,height=18,relwidth=.125,"in"=frameA1,anchor="ne")
    tkbind(labelA1,"<Button-1>",function()
      ...
      if (!(tclvalue(temp1)==""))
        ...
        tkconfigure(labelA1,background=local.points.col.fg.var[[WhichGroup]])
        if (WhichGroup==1) for (temp1 in 2:(g+1)) local.points.col.fg.var[[temp1]]<<-local.points.col.fg.var[[1]]
        }
      })

    tkplace(tk2label(frameA1,text="Background colour"),x=11,y=80,"in"=frameA1)
    labelA2<-tklabel(frameA1,background=local.points.col.bg.var[[WhichGroup]],relief="groove",borderwidth="1.5p")
    tkplace(labelA2,relx=.95,y=80,height=18,relwidth=.125,"in"=frameA1,anchor="ne")
    ...
    tkplace(tk2label(frameA2,text="Font"),x=11,y=20,"in"=frameA2)
    spinboxA2<-tkwidget(frameA2,"SpinBox",textvariable=local.points.label.font.var[[WhichGroup]],editable=FALSE,values=c(" ",1:4),justify="right",modifycmd=function()
      {
      if (WhichGroup==1 && tclvalue(local.points.label.font.var[[1]])!=" ") for (temp1 in 2:(g+1)) local.points.label.font.var[[temp1]]<<-tclVar(tclvalue(local.points.label.font.var[[1]]))
      })
    tkplace(spinboxA2,relx=.95,y=20,height=18,relwidth=.125,"in"=frameA2,anchor="ne")

    tkplace(tk2label(frameA2,text="Size"),x=11,y=40,"in"=frameA2)
    entryA2<-tk2entry(frameA2,textvariable=local.points.label.cex.var[[WhichGroup]],justify="right",takefocus=FALSE)
    tkplace(entryA2,relx=.95,y=40,height=18,relwidth=.125,"in"=frameA2,anchor="ne")

    tkplace(tk2label(frameA2,text="Colour"),x=11,y=60,"in"=frameA2)
    labelA3<-tklabel(frameA2,background=local.points.label.col.var[[WhichGroup]],relief="groove",borderwidth="1.5p")
    tkplace(labelA3,relx=.95,y=60,height=18,relwidth=.125,"in"=frameA2,anchor="ne")
    tkbind(labelA3,"<Button-1>",function()
      ...
      if (!(tclvalue(temp1)==""))
        ...
        tkconfigure(labelA3,background=local.points.label.col.var[[WhichGroup]])
        if (WhichGroup==1) for (temp1 in 2:(g+1)) local.points.label.col.var[[temp1]]<<-local.points.label.col.var[[1]]
        }
      })

    tkplace(tk2label(frameA2,text="Horizontal offset"),x=11,y=80,"in"=frameA2)
    entryA3<-tk2entry(frameA2,textvariable=local.points.label.HorizOffset.var[[WhichGroup]],justify="right",takefocus=FALSE)
    tkplace(entryA3,relx=.95,y=80,height=18,relwidth=.125,"in"=frameA2,anchor="ne")
    ...
    tkplace(tk2label(frameB1,text="Symbol"),x=11,y=20,"in"=frameB1)
    spinboxB1<-tkwidget(frameB1,"SpinBox",textvariable=local.SampleGroupMeans.pch.var[[WhichGroup]],editable=FALSE,values=c(" ","NA",0:25),justify="right",modifycmd=function()
      {
      if (WhichGroup==1 && tclvalue(local.SampleGroupMeans.pch.var[[1]])!=" ") for (temp1 in 2:(g+1)) local.SampleGroupMeans.pch.var[[temp1]]<<-tclVar(tclvalue(local.SampleGroupMeans.pch.var[[1]]))
      })
    tkplace(spinboxB1,relx=.95,y=20,height=18,relwidth=.125,"in"=frameB1,anchor="ne")

    tkplace(tk2label(frameB1,text="Size"),x=11,y=40,"in"=frameB1)
    entryB1<-tk2entry(frameB1,textvariable=local.SampleGroupMeans.cex.var[[WhichGroup]],justify="right",takefocus=FALSE)
    tkplace(entryB1,relx=.95,y=40,height=18,relwidth=.125,"in"=frameB1,anchor="ne")

    tkplace(tk2label(frameB1,text="Foreground colour"),x=11,y=60,"in"=frameB1)
    labelB1<-tklabel(frameB1,background=local.SampleGroupMeans.col.fg.var[[WhichGroup]],relief="groove",borderwidth="1.5p")
    tkplace(labelB1,relx=.95,y=60,height=18,relwidth=.125,"in"=frameB1,anchor="ne")
    tkbind(labelB1,"<Button-1>",function()
      ...
      if (!(tclvalue(temp1)==""))
        ...
        tkconfigure(labelB1,background=local.SampleGroupMeans.col.fg.var[[WhichGroup]])
        if (WhichGroup==1) for (temp1 in 2:(g+1)) local.SampleGroupMeans.col.fg.var[[temp1]]<<-local.SampleGroupMeans.col.fg.var[[1]]
        }
      })

    tkplace(tk2label(frameB1,text="Background colour"),x=11,y=80,"in"=frameB1)
    labelB2<-tklabel(frameB1,background=local.SampleGroupMeans.col.bg.var[[WhichGroup]],relief="groove",borderwidth="1.5p")
    tkplace(labelB2,relx=.95,y=80,height=18,relwidth=.125,"in"=frameB1,anchor="ne")
    ...
    tkplace(tk2label(frameB2,text="Font"),x=11,y=20,"in"=frameB2)
    spinboxB2<-tkwidget(frameB2,"SpinBox",textvariable=local.SampleGroupMeans.label.font.var[[WhichGroup]],editable=FALSE,values=c(" ",1:4),justify="right",modifycmd=function()
      {
      if (WhichGroup==1 && tclvalue(local.SampleGroupMeans.label.font.var[[1]])!=" ") for (temp1 in 2:(g+1)) local.SampleGroupMeans.label.font.var[[temp1]]<<-tclVar(tclvalue(local.SampleGroupMeans.label.font.var[[1]]))
      })
    tkplace(spinboxB2,relx=.95,y=20,height=18,relwidth=.125,"in"=frameB2,anchor="ne")

    tkplace(tk2label(frameB2,text="Size"),x=11,y=40,"in"=frameB2)
    entryB2<-tk2entry(frameB2,textvariable=local.SampleGroupMeans.label.cex.var[[WhichGroup]],justify="right",takefocus=FALSE)
    tkplace(entryB2,relx=.95,y=40,height=18,relwidth=.125,"in"=frameB2,anchor="ne")

    tkplace(tk2label(frameB2,text="Colour"),x=11,y=60,"in"=frameB2)
    labelB3<-tklabel(frameB2,background=local.SampleGroupMeans.label.col.var[[WhichGroup]],relief="groove",borderwidth="1.5p")
    tkplace(labelB3,relx=.95,y=60,height=18,relwidth=.125,"in"=frameB2,anchor="ne")
    tkbind(labelB3,"<Button-1>",function()
      ...
      if (!(tclvalue(temp1)==""))
        ...
        tkconfigure(labelB3,background=local.SampleGroupMeans.label.col.var[[WhichGroup]])
        if (WhichGroup==1) for (temp1 in 2:(g+1)) local.SampleGroupMeans.label.col.var[[temp1]]<<-local.SampleGroupMeans.label.col.var[[1]]
        }
      })

    tkplace(tk2label(frameB2,text="Horizontal offset"),x=11,y=80,"in"=frameB2)
    entryB3<-tk2entry(frameB2,textvariable=local.SampleGroupMeans.label.HorizOffset.var[[WhichGroup]],justify="right",takefocus=FALSE)
    tkplace(entryB3,relx=.95,y=80,height=18,relwidth=.125,"in"=frameB2,anchor="ne")
    ...
    tkplace(tk2label(frameC1,text="Line type"),x=11,y=20,"in"=frameC1)
    spinboxC1<-tkwidget(frameC1,"SpinBox",textvariable=local.ConvexHullAlphaBag.lty.var[[WhichGroup]],editable=FALSE,values=c(" ",0:6),justify="right",modifycmd=function()
      {
      if (WhichGroup==1 && tclvalue(local.ConvexHullAlphaBag.lty.var[[1]])!=" ") for (temp1 in 2:(g+1)) local.ConvexHullAlphaBag.lty.var[[temp1]]<<-tclVar(tclvalue(local.ConvexHullAlphaBag.lty.var[[1]]))
      })
    tkplace(spinboxC1,relx=.95,y=20,height=18,relwidth=.125,"in"=frameC1,anchor="ne")

    tkplace(tk2label(frameC1,text="Line width"),x=11,y=40,"in"=frameC1)
    entryC1<-tk2entry(frameC1,textvariable=local.ConvexHullAlphaBag.lwd.var[[WhichGroup]],justify="right",takefocus=FALSE)
    tkplace(entryC1,relx=.95,y=40,height=18,relwidth=.125,"in"=frameC1,anchor="ne")

    tkplace(tk2label(frameC1,text="Foreground colour"),x=11,y=60,"in"=frameC1)
    labelC1<-tklabel(frameC1,background=local.ConvexHullAlphaBag.col.fg.var[[WhichGroup]],relief="groove",borderwidth="1.5p")
    tkplace(labelC1,relx=.95,y=60,height=18,relwidth=.125,"in"=frameC1,anchor="ne")
    tkbind(labelC1,"<Button-1>",function()
      ...
      if (!(tclvalue(temp1)==""))
        ...
        tkconfigure(labelC1,background=local.ConvexHullAlphaBag.col.fg.var[[WhichGroup]])
        if (WhichGroup==1) for (temp1 in 2:(g+1)) local.ConvexHullAlphaBag.col.fg.var[[temp1]]<<-local.ConvexHullAlphaBag.col.fg.var[[1]]
        }
      })

    tkplace(tk2label(frameC1,text="Background colour"),x=11,y=80,"in"=frameC1)
    labelC2<-tklabel(frameC1,background=local.ConvexHullAlphaBag.col.bg.var[[WhichGroup]],relief="groove",borderwidth="1.5p")
    tkplace(labelC2,relx=.95,y=80,height=18,relwidth=.125,"in"=frameC1,anchor="ne")
    ...
    tkplace(tk2label(frameC2,text="Symbol"),x=11,y=20,"in"=frameC2)
    spinboxC2<-tkwidget(frameC2,"SpinBox",textvariable=local.ConvexHullAlphaBag.TukeyMedian.pch.var[[WhichGroup]],editable=FALSE,values=c(" ","NA",0:25),justify="right",modifycmd=function()
      {
      if (WhichGroup==1 && tclvalue(local.ConvexHullAlphaBag.TukeyMedian.pch.var[[1]])!=" ") for (temp1 in 2:(g+1)) local.ConvexHullAlphaBag.TukeyMedian.pch.var[[temp1]]<<-tclVar(tclvalue(local.ConvexHullAlphaBag.TukeyMedian.pch.var[[1]]))
      })
    tkplace(spinboxC2,relx=.95,y=20,height=18,relwidth=.125,"in"=frameC2,anchor="ne")

    tkplace(tk2label(frameC2,text="Size"),x=11,y=40,"in"=frameC2)
    entryC2<-tk2entry(frameC2,textvariable=local.ConvexHullAlphaBag.TukeyMedian.cex.var[[WhichGroup]],justify="right",takefocus=FALSE)
    tkplace(entryC2,relx=.95,y=40,height=18,relwidth=.125,"in"=frameC2,anchor="ne")

    tkplace(tk2label(frameC2,text="Foreground colour"),x=11,y=60,"in"=frameC2)
    labelC3<-tklabel(frameC2,background=local.ConvexHullAlphaBag.TukeyMedian.col.fg.var[[WhichGroup]],relief="groove",borderwidth="1.5p")
    tkplace(labelC3,relx=.95,y=60,height=18,relwidth=.125,"in"=frameC2,anchor="ne")
    tkbind(labelC3,"<Button-1>",function()
      ...
      if (!(tclvalue(temp1)==""))
        ...
        tkconfigure(labelC3,background=local.ConvexHullAlphaBag.TukeyMedian.col.fg.var[[WhichGroup]])
        if (WhichGroup==1) for (temp1 in 2:(g+1)) local.ConvexHullAlphaBag.TukeyMedian.col.fg.var[[temp1]]<<-local.ConvexHullAlphaBag.TukeyMedian.col.fg.var[[1]]
        }
      })

    tkplace(tk2label(frameC2,text="Background colour"),x=11,y=80,"in"=frameC2)
    labelC4<-tklabel(frameC2,background=local.ConvexHullAlphaBag.TukeyMedian.col.bg.var[[WhichGroup]],relief="groove",borderwidth="1.5p")
    tkplace(labelC4,relx=.95,y=80,height=18,relwidth=.125,"in"=frameC2,anchor="ne")
    ...
    tkplace(tk2label(frameC3,text="Font"),x=11,y=20,"in"=frameC3)
    spinboxC3<-tkwidget(frameC3,"SpinBox",textvariable=local.ConvexHullAlphaBag.TukeyMedian.label.font.var[[WhichGroup]],editable=FALSE,values=c(" ",1:4),justify="right",modifycmd=function()
      {
      if (WhichGroup==1 && tclvalue(local.ConvexHullAlphaBag.TukeyMedian.label.font.var[[1]])!=" ") for (temp1 in 2:(g+1)) local.ConvexHullAlphaBag.TukeyMedian.label.font.var[[temp1]]<<-tclVar(tclvalue(local.ConvexHullAlphaBag.TukeyMedian.label.font.var[[1]]))
      })
    tkplace(spinboxC3,relx=.95,y=20,height=18,relwidth=.125,"in"=frameC3,anchor="ne")

    tkplace(tk2label(frameC3,text="Size"),x=11,y=40,"in"=frameC3)
    entryC3<-tk2entry(frameC3,textvariable=local.ConvexHullAlphaBag.TukeyMedian.label.cex.var[[WhichGroup]],justify="right",takefocus=FALSE)
    tkplace(entryC3,relx=.95,y=40,height=18,relwidth=.125,"in"=frameC3,anchor="ne")

    tkplace(tk2label(frameC3,text="Colour"),x=11,y=60,"in"=frameC3)
    labelC5<-tklabel(frameC3,background=local.ConvexHullAlphaBag.TukeyMedian.label.col.var[[WhichGroup]],relief="groove",borderwidth="1.5p")
    tkplace(labelC5,relx=.95,y=60,height=18,relwidth=.125,"in"=frameC3,anchor="ne")
    tkbind(labelC5,"<Button-1>",function()
      ...
      if (!(tclvalue(temp1)==""))
        ...
        tkconfigure(labelC5,background=local.ConvexHullAlphaBag.TukeyMedian.label.col.var[[WhichGroup]])
        if (WhichGroup==1) for (temp1 in 2:(g+1)) local.ConvexHullAlphaBag.TukeyMedian.label.col.var[[temp1]]<<-local.ConvexHullAlphaBag.TukeyMedian.label.col.var[[1]]
        }
      })

    tkplace(tk2label(frameC3,text="Horizontal offset"),x=11,y=80,"in"=frameC3)
    entryC4<-tk2entry(frameC3,textvariable=local.ConvexHullAlphaBag.TukeyMedian.label.HorizOffset.var[[WhichGroup]],justify="right",takefocus=FALSE)
    tkplace(entryC4,relx=.95,y=80,height=18,relwidth=.125,"in"=frameC3,anchor="ne")
    ...
    tkplace(tk2label(frameD1,text="Background colour"),x=11,y=20,"in"=frameD1)
    labelD1<-tklabel(frameD1,background=local.ClassificationRegion.col.bg.var[[WhichGroup]],relief="groove",borderwidth="1.5p")
    tkplace(labelD1,relx=.95,y=20,height=18,relwidth=.125,"in"=frameD1,anchor="ne")
...
# cat("ENDLISTING Controls the dialogue box. BEGINLISTING")
  local.GUI.func<-function()
    ...
    UpdateEntryBoxes<-function()
      ...
      if (WhichAxis==1 && tclvalue(local.axes.tick.n.var[[1]])!=" ") for (temp1 in 2:(p+1)) local.axes.tick.n.var[[temp1]]<<-tclVar(tclvalue(local.axes.tick.n.var[[1]]))
      if (WhichAxis==1 && tclvalue(local.axes.tick.lwd.var[[1]])!=" ") for (temp1 in 2:(p+1)) local.axes.tick.lwd.var[[temp1]]<<-tclVar(tclvalue(local.axes.tick.lwd.var[[1]]))
      if (WhichAxis==1 && tclvalue(local.axes.tick.RelLength.var[[1]])!=" ") for (temp1 in 2:(p+1)) local.axes.tick.RelLength.var[[temp1]]<<-tclVar(tclvalue(local.axes.tick.RelLength.var[[1]]))
      if (WhichAxis==1 && tclvalue(local.axes.marker.cex.var[[1]])!=" ") for (temp1 in 2:(p+1)) local.axes.marker.cex.var[[temp1]]<<-tclVar(tclvalue(local.axes.marker.cex.var[[1]]))
    ...
    ChangeGroup<-function()
      ...
      UpdateEntryBoxes()
      temp<-as.numeric(tclvalue(tkcurselection(listbox1)))+1
      if (!is.na(temp)) WhichAxis<<-temp

      local.axes.lty.var[[1]]<<-if (all(unlist(lapply(local.axes.lty.var[-1],tclvalue))==tclvalue(local.axes.lty.var[[2]]))) local.axes.lty.var[[2]] else tclVar(" ")
      tkconfigure(spinboxA1,textvariable=local.axes.lty.var[[WhichAxis]])

      local.axes.lwd.var[[1]]<<-if (all(unlist(lapply(local.axes.lwd.var[-1],tclvalue))==tclvalue(local.axes.lwd.var[[2]]))) local.axes.lwd.var[[2]] else tclVar(" ")
      tkconfigure(entryA1,textvariable=local.axes.lwd.var[[WhichAxis]])

      local.axes.col.var[[1]]<<-if (all(unlist(local.axes.col.var[-1])==local.axes.col.var[[2]])) local.axes.col.var[[2]] else text2hex("SystemButtonFace")
      tkconfigure(labelA1,background=local.axes.col.var[[WhichAxis]])

      local.axes.tick.n.var[[1]]<<-if (all(unlist(lapply(local.axes.tick.n.var[-1],tclvalue))==tclvalue(local.axes.tick.n.var[[2]]))) local.axes.tick.n.var[[2]] else tclVar(" ")
      tkconfigure(entryA2,textvariable=local.axes.tick.n.var[[WhichAxis]])

      local.axes.tick.lty.var[[1]]<<-if (all(unlist(lapply(local.axes.tick.lty.var[-1],tclvalue))==tclvalue(local.axes.tick.lty.var[[2]]))) local.axes.tick.lty.var[[2]] else tclVar(" ")
      tkconfigure(spinboxA2,textvariable=local.axes.tick.lty.var[[WhichAxis]])

      local.axes.tick.lwd.var[[1]]<<-if (all(unlist(lapply(local.axes.tick.lwd.var[-1],tclvalue))==tclvalue(local.axes.tick.lwd.var[[2]]))) local.axes.tick.lwd.var[[2]] else tclVar(" ")
      tkconfigure(entryA3,textvariable=local.axes.tick.lwd.var[[WhichAxis]])

      local.axes.tick.col.var[[1]]<<-if (all(unlist(local.axes.tick.col.var[-1])==local.axes.tick.col.var[[2]])) local.axes.tick.col.var[[2]] else text2hex("SystemButtonFace")
      tkconfigure(labelA2,background=local.axes.tick.col.var[[WhichAxis]])

      local.axes.tick.RelLength.var[[1]]<<-if (all(unlist(lapply(local.axes.tick.RelLength.var[-1],tclvalue))==tclvalue(local.axes.tick.RelLength.var[[2]]))) local.axes.tick.RelLength.var[[2]] else tclVar(" ")
      tkconfigure(entryA4,textvariable=local.axes.tick.RelLength.var[[WhichAxis]])

      local.axes.marker.font.var[[1]]<<-if (all(unlist(lapply(local.axes.marker.font.var[-1],tclvalue))==tclvalue(local.axes.marker.font.var[[2]]))) local.axes.marker.font.var[[2]] else tclVar(" ")
      tkconfigure(spinboxA3,textvariable=local.axes.marker.font.var[[WhichAxis]])

      local.axes.marker.cex.var[[1]]<<-if (all(unlist(lapply(local.axes.marker.cex.var[-1],tclvalue))==tclvalue(local.axes.marker.cex.var[[2]]))) local.axes.marker.cex.var[[2]] else tclVar(" ")
      tkconfigure(entryA5,textvariable=local.axes.marker.cex.var[[WhichAxis]])

      local.axes.marker.col.var[[1]]<<-if (all(unlist(local.axes.marker.col.var[-1])==local.axes.marker.col.var[[2]])) local.axes.marker.col.var[[2]] else text2hex("SystemButtonFace")
      tkconfigure(labelA3,background=local.axes.marker.col.var[[WhichAxis]])

      local.axes.marker.RelOffset.var[[1]]<<-if (all(unlist(lapply(local.axes.marker.RelOffset.var[-1],tclvalue))==tclvalue(local.axes.marker.RelOffset.var[[2]]))) local.axes.marker.RelOffset.var[[2]] else tclVar(" ")
      tkconfigure(entryA6,textvariable=local.axes.marker.RelOffset.var[[WhichAxis]])

      local.axes.label.font.var[[1]]<<-if (all(unlist(lapply(local.axes.label.font.var[-1],tclvalue))==tclvalue(local.axes.label.font.var[[2]]))) local.axes.label.font.var[[2]] else tclVar(" ")
      tkconfigure(spinboxA4,textvariable=local.axes.label.font.var[[WhichAxis]])

      local.axes.label.cex.var[[1]]<<-if (all(unlist(lapply(local.axes.label.cex.var[-1],tclvalue))==tclvalue(local.axes.label.cex.var[[2]]))) local.axes.label.cex.var[[2]] else tclVar(" ")
      tkconfigure(entryA7,textvariable=local.axes.label.cex.var[[WhichAxis]])

      local.axes.label.las.var[[1]]<<-if (all(unlist(lapply(local.axes.label.las.var[-1],tclvalue))==tclvalue(local.axes.label.las.var[[2]]))) local.axes.label.las.var[[2]] else tclVar(" ")
   ...
   onDefaults<-function()
    ...
    onOK<-function()
      ...
      bpar$axes.lty<<-as.numeric(lapply(local.axes.lty.var[-1],tclvalue))
      bpar$axes.lwd<<-as.numeric(lapply(local.axes.lwd.var[-1],tclvalue))
      bpar$axes.col<<-unlist(local.axes.col.var[-1])
      bpar$axes.tick.n<<-as.numeric(lapply(local.axes.tick.n.var[-1],tclvalue))
      bpar$axes.tick.lty<<-as.numeric(lapply(local.axes.tick.lty.var[-1],tclvalue))
      bpar$axes.tick.lwd<<-as.numeric(lapply(local.axes.tick.lwd.var[-1],tclvalue))
      bpar$axes.tick.col<<-unlist(local.axes.tick.col.var[-1])
      bpar$axes.tick.RelLength<<-as.numeric(lapply(local.axes.tick.RelLength.var[-1],tclvalue))
      bpar$axes.marker.font<<-as.numeric(lapply(local.axes.marker.font.var[-1],tclvalue))
      bpar$axes.marker.cex<<-as.numeric(lapply(local.axes.marker.cex.var[-1],tclvalue))
      bpar$axes.marker.col<<-unlist(local.axes.marker.col.var[-1])
      bpar$axes.marker.RelOffset<<-as.numeric(lapply(local.axes.marker.RelOffset.var[-1],tclvalue))
      bpar$axes.label.font<<-as.numeric(lapply(local.axes.label.font.var[-1],tclvalue))
      bpar$axes.label.cex<<-as.numeric(lapply(local.axes.label.cex.var[-1],tclvalue))
      bpar$axes.label.las<<-as.numeric(lapply(local.axes.label.las.var[-1],tclvalue))
 ...
 # cat("ENDLISTING Defines the local variables. BEGINLISTING")
   ...
   # cat("ENDLISTING Initialises the local variables. BEGINLISTING")
    ...
    tkplace(tk2label(frameA1,text="Line type"),x=11,y=20,"in"=frameA1)
    spinboxA1<-tkwidget(frameA1,"SpinBox",textvariable=local.axes.lty.var[[WhichAxis]],editable=FALSE,values=c(" ",0:6),justify="right",modifycmd=function()
      {
      if (WhichAxis==1 && tclvalue(local.axes.lty.var[[1]])!=" ") for (temp1 in 2:(p+1)) local.axes.lty.var[[temp1]]<<-tclVar(tclvalue(local.axes.lty.var[[1]]))
      })
    tkplace(spinboxA1,relx=.95,y=20,height=18,relwidth=.125,"in"=frameA1,anchor="ne")

    tkplace(tk2label(frameA1,text="Line width"),x=11,y=40,"in"=frameA1)
    entryA1<-tk2entry(frameA1,textvariable=local.axes.lwd.var[[WhichAxis]],justify="right",takefocus=FALSE)
    tkplace(entryA1,relx=.95,y=40,height=18,relwidth=.125,"in"=frameA1,anchor="ne")

    tkplace(tk2label(frameA1,text="Colour"),x=11,y=60,"in"=frameA1)
    labelA1<-tklabel(frameA1,background=local.axes.col.var[[WhichAxis]],relief="groove",borderwidth="1.5p")
    tkplace(labelA1,relx=.95,y=60,height=18,relwidth=.125,"in"=frameA1,anchor="ne")
    tkbind(labelA1,"<Button-1>",function()
      {
    ...
    tkplace(tk2label(frameA2,text="Desired number"),x=11,y=20,"in"=frameA2)
    entryA2<-tk2entry(frameA2,textvariable=local.axes.tick.n.var[[WhichAxis]],justify="right",takefocus=FALSE)
    tkplace(entryA2,relx=.95,y=20,height=18,relwidth=.125,"in"=frameA2,anchor="ne")

    tkplace(tk2label(frameA2,text="Line type"),x=11,y=40,"in"=frameA2)
    spinboxA2<-tkwidget(frameA2,"SpinBox",textvariable=local.axes.tick.lty.var[[WhichAxis]],editable=FALSE,values=c(" ",0:6),justify="right",modifycmd=function()
      {
      if (WhichAxis==1 && tclvalue(local.axes.tick.lty.var[[1]])!=" ") for (temp1 in 2:(p+1)) local.axes.tick.lty.var[[temp1]]<<-tclVar(tclvalue(local.axes.tick.lty.var[[1]]))
      })
    tkplace(spinboxA2,relx=.95,y=40,height=18,relwidth=.125,"in"=frameA2,anchor="ne")

    tkplace(tk2label(frameA2,text="Line width"),x=11,y=60,"in"=frameA2)
    entryA3<-tk2entry(frameA2,textvariable=local.axes.tick.lwd.var[[WhichAxis]],justify="right",takefocus=FALSE)
    tkplace(entryA3,relx=.95,y=60,height=18,relwidth=.125,"in"=frameA2,anchor="ne")

    tkplace(tk2label(frameA2,text="Colour"),x=11,y=80,"in"=frameA2)
    labelA2<-tklabel(frameA2,background=local.axes.tick.col.var[[WhichAxis]],relief="groove",borderwidth="1.5p")
    tkplace(labelA2,relx=.95,y=80,height=18,relwidth=.125,"in"=frameA2,anchor="ne")
    tkbind(labelA2,"<Button-1>",function()
      {
      ...
      if (!(tclvalue(temp1)==""))
        ...
        tkconfigure(labelA2,background=local.axes.tick.col.var[[WhichAxis]])
        if (WhichAxis==1) for (temp1 in 2:(p+1)) local.axes.tick.col.var[[temp1]]<<-local.axes.tick.col.var[[1]]
        }
      })
    ...
    tkplace(tk2label(frameA3,text="Font"),x=11,y=20,"in"=frameA3)
    spinboxA3<-tkwidget(frameA3,"SpinBox",textvariable=local.axes.marker.font.var[[WhichAxis]],editable=FALSE,values=c(" ",1:4),justify="right",modifycmd=function()
      {
      if (WhichAxis==1 && tclvalue(local.axes.marker.font.var[[1]])!=" ") for (temp1 in 2:(p+1)) local.axes.marker.font.var[[temp1]]<<-tclVar(tclvalue(local.axes.marker.font.var[[1]]))
      })
    tkplace(spinboxA3,relx=.95,y=20,height=18,relwidth=.125,"in"=frameA3,anchor="ne")

    tkplace(tk2label(frameA3,text="Size"),x=11,y=40,"in"=frameA3)
    entryA5<-tk2entry(frameA3,textvariable=local.axes.marker.cex.var[[WhichAxis]],justify="right",takefocus=FALSE)
    tkplace(entryA5,relx=.95,y=40,height=18,relwidth=.125,"in"=frameA3,anchor="ne")

    tkplace(tk2label(frameA3,text="Colour"),x=11,y=60,"in"=frameA3)
    labelA3<-tklabel(frameA3,background=local.axes.marker.col.var[[WhichAxis]],relief="groove",borderwidth="1.5p")
    tkplace(labelA3,relx=.95,y=60,height=18,relwidth=.125,"in"=frameA3,anchor="ne")
    tkbind(labelA3,"<Button-1>",function()
      {
      ...
      if (!(tclvalue(temp1)==""))
        ...
        tkconfigure(labelA3,background=local.axes.marker.col.var[[WhichAxis]])
        if (WhichAxis==1) for (temp1 in 2:(p+1)) local.axes.marker.col.var[[temp1]]<<-local.axes.marker.col.var[[1]]
        }
      })

    tkplace(tk2label(frameA3,text="Relative offset"),x=11,y=80,"in"=frameA3)
    entryA6<-tk2entry(frameA3,textvariable=local.axes.marker.RelOffset.var[[WhichAxis]],justify="right",takefocus=FALSE)
    ...
    tkplace(tk2label(frameA4,text="Font"),x=11,y=20,"in"=frameA4)
    spinboxA4<-tkwidget(frameA4,"SpinBox",textvariable=local.axes.label.font.var[[WhichAxis]],editable=FALSE,values=c(" ",1:4),justify="right",modifycmd=function()
      {
      if (WhichAxis==1 && tclvalue(local.axes.label.font.var[[1]])!=" ") for (temp1 in 2:(p+1)) local.axes.label.font.var[[temp1]]<<-tclVar(tclvalue(local.axes.label.font.var[[1]]))
      })
    tkplace(spinboxA4,relx=.95,y=20,height=18,relwidth=.125,"in"=frameA4,anchor="ne")

    tkplace(tk2label(frameA4,text="Size"),x=11,y=40,"in"=frameA4)
    entryA7<-tk2entry(frameA4,textvariable=local.axes.label.cex.var[[WhichAxis]],justify="right",takefocus=FALSE)
    tkplace(entryA7,relx=.95,y=40,height=18,relwidth=.125,"in"=frameA4,anchor="ne")

    tkplace(tk2label(frameA4,text="Orientation"),x=11,y=60,"in"=frameA4)
    spinboxA5<-tkwidget(frameA4,"SpinBox",textvariable=local.axes.label.las.var[[WhichAxis]],editable=FALSE,values=c(" ",0:3),justify="right",modifycmd=function()
      {
      if (WhichAxis==1 && tclvalue(local.axes.label.las.var[[1]])!=" ") for (temp1 in 2:(p+1)) local.axes.label.las.var[[temp1]]<<-tclVar(tclvalue(local.axes.label.las.var[[1]]))
      })
    tkplace(spinboxA5,relx=.95,y=60,height=18,relwidth=.125,"in"=frameA4,anchor="ne")

    tkplace(tk2label(frameA4,text="Colour"),x=11,y=80,"in"=frameA4)
    labelA4<-tklabel(frameA4,background=local.axes.label.col.var[[WhichAxis]],relief="groove",borderwidth="1.5p")
    tkplace(labelA4,relx=.95,y=80,height=18,relwidth=.125,"in"=frameA4,anchor="ne")
    tkbind(labelA4,"<Button-1>",function()
      ...
      if (!(tclvalue(temp1)==""))
        ...
        tkconfigure(labelA4,background=local.axes.label.col.var[[WhichAxis]])
        if (WhichAxis==1) for (temp1 in 2:(p+1)) local.axes.label.col.var[[temp1]]<<-local.axes.label.col.var[[1]]
        }
      })
...
# cat("ENDLISTING Creates the dialogue box. BEGINLISTING")
  ...
  onDefaults<-function()
    ...
    local.interaction.highlight.ShowValues.cex.var<<-tclVar(.75)
 ...
 # cat("ENDLISTING Defines the local variables. BEGINLISTING")
  ...
  tkplace(tk2label(frame1,text="Line type"),x=11,y=20,"in"=frame1)
  spinbox1<-tkwidget(frame1,"SpinBox",textvariable=local.interaction.prediction.lty.var,editable=FALSE,values=c(" ","NA",0:6),justify="right")
  tkplace(spinbox1,relx=.95,y=20,height=18,relwidth=.125,"in"=frame1,anchor="ne")

  tkplace(tk2label(frame1,text="Line width"),x=11,y=40,"in"=frame1)
  entry1<-tk2entry(frame1,textvariable=local.interaction.prediction.lwd.var,justify="right",takefocus=FALSE)
  tkplace(entry1,relx=.95,y=40,height=18,relwidth=.125,"in"=frame1,anchor="ne")

  tkplace(tk2label(frame1,text="Colour"),x=11,y=60,"in"=frame1)
  label1<-tklabel(frame1,background=local.interaction.prediction.col.var,relief="groove",borderwidth="1.5p")
  tkplace(label1,relx=.95,y=60,height=18,relwidth=.125,"in"=frame1,anchor="ne")
  ...
  tkplace(tk2label(frame1,text="Plotting character: symbol"),x=11,y=80,"in"=frame1)
  spinbox2<-tkwidget(frame1,"SpinBox",textvariable=local.interaction.prediction.pch.var,editable=FALSE,values=c(" ","NA",0:25),justify="right")
  tkplace(spinbox2,relx=.95,y=80,height=18,relwidth=.125,"in"=frame1,anchor="ne")

  tkplace(tk2label(frame1,text="Plotting character: size"),x=11,y=100,"in"=frame1)
  entry2<-tk2entry(frame1,textvariable=local.interaction.prediction.cex.var,justify="right",takefocus=FALSE)
  tkplace(entry2,relx=.95,y=100,height=18,relwidth=.125,"in"=frame1,anchor="ne")

  tkplace(tk2label(frame1,text="Circle: line width"),x=11,y=120,"in"=frame1)
  entry3<-tk2entry(frame1,textvariable=local.interaction.prediction.circle.lwd.var,justify="right",takefocus=FALSE)
  tkplace(entry3,relx=.95,y=120,height=18,relwidth=.125,"in"=frame1,anchor="ne")

  tkplace(tk2label(frame1,text="Circle: colour"),x=11,y=140,"in"=frame1)
  label2<-tklabel(frame1,background=local.interaction.prediction.circle.col.var,relief="groove",borderwidth="1.5p")
  tkplace(label2,relx=.95,y=140,height=18,relwidth=.125,"in"=frame1,anchor="ne")
  ...
  tkplace(tk2label(frame2,text="Colour"),x=11,y=20,"in"=frame2)
  label3<-tklabel(frame2,background=local.interaction.highlight.axes.col.fg.var,relief="groove",borderwidth="1.5p")
  tkplace(label3,relx=.95,y=20,height=18,relwidth=.125,"in"=frame2,anchor="ne")
  ...
  tkplace(tk2label(frame2,text="Other axes: colour"),x=11,y=40,"in"=frame2)
  label4<-tklabel(frame2,background=local.interaction.highlight.axes.col.bg.var,relief="groove",borderwidth="1.5p")
  tkplace(label4,relx=.95,y=40,height=18,relwidth=.125,"in"=frame2,anchor="ne")
  ...
  tkplace(tk2label(frame2,text="Show point values: font"),x=11,y=60,"in"=frame2)
  spinbox3<-tkwidget(frame2,"SpinBox",textvariable=local.interaction.highlight.ShowValues.font.var,editable=FALSE,values=c(" ",1:4),justify="right")
  tkplace(spinbox3,relx=.95,y=60,height=18,relwidth=.125,"in"=frame2,anchor="ne")

  tkplace(tk2label(frame2,text="Show point values: size"),x=11,y=80,"in"=frame2)
  entry4<-tk2entry(frame2,textvariable=local.interaction.highlight.ShowValues.cex.var,justify="right",takefocus=FALSE)
  tkplace(entry4,relx=.95,y=80,height=18,relwidth=.125,"in"=frame2,anchor="ne")

  tkplace(tk2label(frame2,text="Show point values: colour"),x=11,y=100,"in"=frame2)
  label5<-tklabel(frame2,background=local.interaction.highlight.ShowValues.col.var,relief="groove",borderwidth="1.5p")
  tkplace(label5,relx=.95,y=100,height=18,relwidth=.125,"in"=frame2,anchor="ne")
  ...
  tkplace(tk2label(frame2,text="Show point values: horizontal offset"),x=11,y=120,"in"=frame2)
  entry5<-tk2entry(frame2,textvariable=local.interaction.highlight.ShowValues.HorizOffset.var,justify="right",takefocus=FALSE)
  tkplace(entry5,relx=.95,y=120,height=18,relwidth=.125,"in"=frame2,anchor="ne")

  tkplace(tk2label(frame2,text="Show point values: vertical offset"),x=11,y=140,"in"=frame2)
  entry6<-tk2entry(frame2,textvariable=local.interaction.highlight.ShowValues.VertOffset.var,justify="right",takefocus=FALSE)
  tkplace(entry6,relx=.95,y=140,height=18,relwidth=.125,"in"=frame2,anchor="ne")

  tkplace(tk2label(frame2,text="Show point values: digits"),x=11,y=160,"in"=frame2)
...
Format.DiagnosticTabs.cmd<-function()
  ...
  onDefaults<-function()
    ...
    local.DiagnosticTabs.predictivities.label.cex.var<<-tclVar(.85)
    ...
    local.DiagnosticTabs.ShepardDiagram.disparities.cex.var<<-tclVar(.4)
  ...
  tkplace(tk2label(frame1,text="Line type"),x=11,y=20,"in"=frame1)
  spinbox1<-tkwidget(frame1,"SpinBox",textvariable=local.DiagnosticTabs.convergence.lty.var,editable=FALSE,values=c(" ","NA",0:6),justify="right")
  tkplace(spinbox1,relx=.95,y=20,height=18,relwidth=.125,"in"=frame1,anchor="ne")

  tkplace(tk2label(frame1,text="Line width"),x=11,y=40,"in"=frame1)
  entry1<-tk2entry(frame1,textvariable=local.DiagnosticTabs.convergence.lwd.var,justify="right",takefocus=FALSE)
  tkplace(entry1,relx=.95,y=40,height=18,relwidth=.125,"in"=frame1,anchor="ne")

  tkplace(tk2label(frame1,text="Colour"),x=11,y=60,"in"=frame1)
  label1<-tklabel(frame1,background=local.DiagnosticTabs.convergence.col.var,relief="groove",borderwidth="1.5p")
  tkplace(label1,relx=.95,y=60,height=18,relwidth=.125,"in"=frame1,anchor="ne")
  tkbind(label1,"<Button-1>",function()
    {
  ...
  tkplace(tk2label(frame2,text="Plotting character: axes symbol"),x=11,y=20,"in"=frame2)
  spinbox2<-tkwidget(frame2,"SpinBox",textvariable=local.DiagnosticTabs.predictivities.axes.pch.var,editable=FALSE,values=c(" ","NA",0:25),justify="right")
  tkplace(spinbox2,relx=.95,y=20,height=18,relwidth=.125,"in"=frame2,anchor="ne")

  tkplace(tk2label(frame2,text="Plotting character: size"),x=11,y=40,"in"=frame2)
  entry2<-tk2entry(frame2,textvariable=local.DiagnosticTabs.predictivities.cex.var,justify="right",takefocus=FALSE)
  tkplace(entry2,relx=.95,y=40,height=18,relwidth=.125,"in"=frame2,anchor="ne")

  tkplace(tk2label(frame2,text="Label: font"),x=11,y=60,"in"=frame2)
  spinbox3<-tkwidget(frame2,"SpinBox",textvariable=local.DiagnosticTabs.predictivities.label.font.var,editable=FALSE,values=c(" ",1:4),justify="right")
  tkplace(spinbox3,relx=.95,y=60,height=18,relwidth=.125,"in"=frame2,anchor="ne")

  tkplace(tk2label(frame2,text="Label: size"),x=11,y=80,"in"=frame2)
  entry3<-tk2entry(frame2,textvariable=local.DiagnosticTabs.predictivities.label.cex.var,justify="right",takefocus=FALSE)
  tkplace(entry3,relx=.95,y=80,height=18,relwidth=.125,"in"=frame2,anchor="ne")

  tkplace(tk2label(frame2,text="Label: horizontal offset"),x=11,y=100,"in"=frame2)
  entry4<-tk2entry(frame2,textvariable=local.DiagnosticTabs.predictivities.label.HorizOffset.var,justify="right",takefocus=FALSE)
  tkplace(entry4,relx=.95,y=100,height=18,relwidth=.125,"in"=frame2,anchor="ne")

  tkplace(tk2label(frame2,text="Label: vertical offset"),x=11,y=120,"in"=frame2)
  ...
  tkplace(tk2label(frame3,text="Plotting character: symbol"),x=11,y=20,"in"=frame3)
  spinbox4<-tkwidget(frame3,"SpinBox",textvariable=local.DiagnosticTabs.ShepardDiagram.pch.var,editable=FALSE,values=c(" ","NA",0:25),justify="right")
  tkplace(spinbox4,relx=.95,y=20,height=18,relwidth=.125,"in"=frame3,anchor="ne")

  tkplace(tk2label(frame3,text="Plotting character: size"),x=11,y=40,"in"=frame3)
  entry6<-tk2entry(frame3,textvariable=local.DiagnosticTabs.ShepardDiagram.cex.var,justify="right",takefocus=FALSE)
  tkplace(entry6,relx=.95,y=40,height=18,relwidth=.125,"in"=frame3,anchor="ne")

  tkplace(tk2label(frame3,text="Plotting character: foreground colour"),x=11,y=60,"in"=frame3)
  label2<-tklabel(frame3,background=local.DiagnosticTabs.ShepardDiagram.col.fg.var,relief="groove",borderwidth="1.5p")
  tkplace(label2,relx=.95,y=60,height=18,relwidth=.125,"in"=frame3,anchor="ne")
  tkbind(label2,"<Button-1>",function()
    {
  ...
  tkplace(tk2label(frame3,text="Plotting character: background colour"),x=11,y=80,"in"=frame3)
  label3<-tklabel(frame3,background=local.DiagnosticTabs.ShepardDiagram.col.bg.var,relief="groove",borderwidth="1.5p")
  tkplace(label3,relx=.95,y=80,height=18,relwidth=.125,"in"=frame3,anchor="ne")
  tkbind(label3,"<Button-1>",function()
    {
  ...
  tkplace(tk2label(frame3,text="Disparities: line type"),x=11,y=100,"in"=frame3)
  spinbox5<-tkwidget(frame3,"SpinBox",textvariable=local.DiagnosticTabs.ShepardDiagram.disparities.lty.var,editable=FALSE,values=c(" ","NA",0:6),justify="right")
  tkplace(spinbox5,relx=.95,y=100,height=18,relwidth=.125,"in"=frame3,anchor="ne")

  tkplace(tk2label(frame3,text="Disparities: line width"),x=11,y=120,"in"=frame3)
  entry7<-tk2entry(frame3,textvariable=local.DiagnosticTabs.ShepardDiagram.disparities.lwd.var,justify="right",takefocus=FALSE)
  tkplace(entry7,relx=.95,y=120,height=18,relwidth=.125,"in"=frame3,anchor="ne")

  tkplace(tk2label(frame3,text="Disparities: line colour"),x=11,y=140,"in"=frame3)
  label4<-tklabel(frame3,background=local.DiagnosticTabs.ShepardDiagram.disparities.col.line.var,relief="groove",borderwidth="1.5p")
  tkplace(label4,relx=.95,y=140,height=18,relwidth=.125,"in"=frame3,anchor="ne")
  tkbind(label4,"<Button-1>",function()
    {
  ...
  tkplace(tk2label(frame3,text="Disparities: plotting character: symbol"),x=11,y=160,"in"=frame3)
  spinbox6<-tkwidget(frame3,"SpinBox",textvariable=local.DiagnosticTabs.ShepardDiagram.disparities.pch.var,editable=FALSE,values=c(" ","NA",0:25),justify="right")
  tkplace(spinbox6,relx=.95,y=160,height=18,relwidth=.125,"in"=frame3,anchor="ne")

  tkplace(tk2label(frame3,text="Disparities: plotting character: size"),x=11,y=180,"in"=frame3)
  entry8<-tk2entry(frame3,textvariable=local.DiagnosticTabs.ShepardDiagram.disparities.cex.var,justify="right",takefocus=FALSE)
  tkplace(entry8,relx=.95,y=180,height=18,relwidth=.125,"in"=frame3,anchor="ne")

  tkplace(tk2label(frame3,text="Disparities: plotting character: foreground colour"),x=11,y=200,"in"=frame3)
  label5<-tklabel(frame3,background=local.DiagnosticTabs.ShepardDiagram.disparities.col.fg.var,relief="groove",borderwidth="1.5p")
  tkplace(label5,relx=.95,y=200,height=18,relwidth=.125,"in"=frame3,anchor="ne")
  tkbind(label5,"<Button-1>",function()
    {
  ...
  tkplace(tk2label(frame3,text="Disparities: plotting character: background colour"),x=11,y=220,"in"=frame3)
  label6<-tklabel(frame3,background=local.DiagnosticTabs.ShepardDiagram.disparities.col.bg.var,relief="groove",borderwidth="1.5p")
  tkplace(label6,relx=.95,y=220,height=18,relwidth=.125,"in"=frame3,anchor="ne")
  ...
  tkplace(tk2label(frame3,text="Number of worst-fitting point pairs"),x=11,y=240,"in"=frame3)
  ...
  tkplace(tk2label(frame4,text="Digits"),x=11,y=20,"in"=frame4)
...
Joint.PCA.cmd<-function(FollowThrough=TRUE)
  {
  View.ClipAround.var<<-tclVar("0")
  if (tclvalue(View.AxisLabels.var)=="-1") View.AxisLabels.var<<-tclVar("1")
  ...
  if (Biplot.zoom.mode==0) tkwm.title(GUI.TopLevel,paste("BiplotGUI -",Joint.PCA.title))
    else tkwm.title(GUI.TopLevel,paste("BiplotGUI -",Joint.PCA.title,"(zoomed)"))
...
Joint.PCA.determine<-function()
  ...
  temp1$vectors<-(apply(temp1$vectors,2,function(x) x*sign(x[which.max(abs(x))])))
...
Joint.CovarianceCorrelation.cmd<-function(FollowThrough=TRUE)
  ...
  if (Biplot.zoom.mode==0) tkwm.title(GUI.TopLevel,paste("BiplotGUI -",Joint.CovarianceCorrelation.title))
    else tkwm.title(GUI.TopLevel,paste("BiplotGUI -",Joint.CovarianceCorrelation.title,"(zoomed)"))
...
Joint.CovarianceCorrelation.determine<-function()
  ...
  Biplot.Y3D_<<-sqrt(n.in-1)*Biplot.Xtransformed%*%V3D%*%(sqrt(solve(lambda.r.3D)))
  if (tclvalue(tkget(SettingsBox.action.combo))=="Predict") Biplot.B3D_<<-(n.in)^-.5*V3D%*%diag(sqrt(temp1$values[1:3]))
...
Joint.CovarianceCorrelation.motion<-function(x,y) Biplot.general.motion(x,y)
Joint.CovarianceCorrelation.OverAxis<-function() Biplot.linear.OverAxis()
Joint.CovarianceCorrelation.LeftClick<-function(x,y) Biplot.general.LeftClick(x,y)
Joint.CovarianceCorrelation.LeftRelease<-function(x,y) Biplot.general.LeftRelease(x,y)
...
Joint.CVA.cmd<-function(FollowThrough=TRUE)
  ...
  if (Biplot.zoom.mode==0) tkwm.title(GUI.TopLevel,paste("BiplotGUI -",Joint.CVA.title))
    else tkwm.title(GUI.TopLevel,paste("BiplotGUI -",Joint.CVA.title,"(zoomed)"))
...
Joint.CVA.determine<-function()
  ...
  Bcva<-t(Xbar)%*%N%*%Xbar
  W<-t(Biplot.Xtransformed)%*%Biplot.Xtransformed-Bcva
  temp1<-svd(W)
  Wminussqrt<-temp1$u%*%diag(temp1$d^-.5)%*%t(temp1$v)
  ...
  Biplot.Y3D_<<-Biplot.Yfull_[,1:3]
  Biplot.Y_<<-Biplot.Yfull_[,1:2]
  if (tclvalue(tkget(SettingsBox.action.combo))=="Predict") Biplot.B_<<-t(solve(V))
    else  Biplot.B_<<-V
  ...
  G<-sapply(levels(factor(group[samples.in],exclude=NULL)),function(x) as.numeric(factor(group[samples.in],exclude=NULL)==x))
  Yg<-(diag(n.in)-G%*%solve(t(G)%*%G)%*%t(G))%*%Biplot.Xtransformed
  Yghat<-Yg%*%V[,1:2]%*%solve(V)[1:2,]
  Yghat1<-Yg%*%V[,1]%*%solve(V)[1,]
  PointsTab.predictivities<<-diag(Yghat%*%solve(W)%*%t(Yghat))/diag(Yg%*%solve(W)%*%t(Yg))
  PointsTab.predictivities1dim<<-diag(Yghat1%*%solve(W)%*%t(Yghat1))/diag(Yg%*%solve(W)%*%t(Yg))
  GroupsTab.predictivities<<-diag((Xbarhat)%*%solve(W)%*%t(Xbarhat))/diag((Xbar)%*%solve(W)%*%t(Xbar))
  GroupsTab.predictivities1dim<<-diag((Xbarhat1)%*%solve(W)%*%t(Xbarhat1))/diag((Xbar)%*%solve(W)%*%t(Xbar))
...
Joint.CVA.motion<-function(x,y) Biplot.general.motion(x,y)
Joint.CVA.OverAxis<-function(x,y) Biplot.linear.OverAxis()
Joint.CVA.LeftClick<-function(x,y) Biplot.general.LeftClick(x,y)
Joint.CVA.LeftRelease<-function(x,y) Biplot.general.LeftRelease(x,y)
Joint.CVA.DoubleLeftClick<-NULL
Joint.CVA.RightClick<-function(x,y) Biplot.general.RightClick(x,y)
...
Points.DistanceMetric.FollowThrough.cmd<-function()
  ...
  switch(tclvalue(Points.var),
    "0"=Points.PCO.cmd(),
    "10"=Points.MDS.IdentityTransformation.cmd(),
...
Points.PCO.cmd<-function(FollowThrough=TRUE)
  {
  if (tclvalue(Points.DistanceMetric.var)=="0" && tclvalue(Biplot.Axes.var)%in%c("0","1","2")) Biplot.Axes.var<<-tclVar("11")
  if (tclvalue(Points.DistanceMetric.var)%in%c("1","2") && tclvalue(Biplot.Axes.var)%in%c("0","1","2")) Biplot.Axes.var<<-tclVar("13")
  if (tclvalue(Points.DistanceMetric.var)=="3" && tclvalue(Biplot.Axes.var)%in%c("0","1","2","11","12","13","14")) Biplot.Axes.var<<-tclVar("10")
  Points.MDS.ApproachToTies.var<<-tclVar("-1")
  if (Points.skipped)
    {
    switch(tclvalue(Points.DistanceMetric.var),
      "0"=Points.DistanceMetric.Pythagoras.cmd(FALSE),
  ...
  temp0<-temp0-1/n.in
  Bpco<- -.5*temp0%*%(Points.DistanceMetric.DissimilarityMatrix^2)%*%temp0
  temp1<-eigen(Bpco,symmetric=TRUE)
  temp1$vectors<-(apply(temp1$vectors,2,function(x) x*sign(x[which.max(abs(x))])))
  Biplot.Yfull<<-sweep(temp1$vectors,2,temp1$values^.5,"*")
...
Points.MDS.cmd<-function()
  ...
  MonotoneRegression<-function(DissMat,Y,WeightMat)
    ...
    if(tclvalue(Points.MDS.ApproachToTies.var)=="1" && any(temp1<-table(diss)>1))
    ...
    repeat
      {
    	if(disp[i-1]>disp[i])
        {
        temp3<-which(disp[1:(i-1)]==disp[i-1])
    		disp[c(temp3,i)]<-mean(disp[c(temp3,i)])
    		i<-max(1,temp3[1]-1)
        }
    	i<-i+1
    	if(i==length(disp)+1) break
      }
    ...
    DispMat[lower.tri(DispMat)]<-disp[order(order(diss,dist))]
    DispMat<-DispMat+t(DispMat)
    DispMat*(n.in*(n.in-1)/2)^.5/(sum(WeightMat*DispMat^2)/2)^.5
    }
  ...
  if (is.null(Biplot.Y))
    { # If a scaling hasn't been performed, the PCO solution is taken as the initial ordination.
    Bpco<- -.5*(diag(n.in)-1/n.in)%*%(Points.DistanceMetric.DissimilarityMatrix^2)%*%(diag(n.in)-1/n.in)
    temp1<-eigen(Bpco)
    temp1$vectors<-(apply(temp1$vectors,2,function(x) x*sign(x[which.max(abs(x))])))
    Biplot.Y<<-temp1$vectors%*%(diag(temp1$values)^.5)[,1:2]
    }

  WeightMat<-matrix(1,nrow=n.in,ncol=n.in)
  if (tclvalue(Points.MDS.RandomInitialConfiguration.var)=="1") Yinitial<-matrix(runif(n.in*2,min=-1,max=1),nrow=n.in,ncol=2)
    else if (!identical(samples.in,samples.in.PreviousScaling))
      {
      Yinitial<-matrix(0,nrow=n.in,ncol=2)
      Yinitial[samples.in%in%samples.in.PreviousScaling,]<-Biplot.Y[samples.in.PreviousScaling%in%samples.in,]
      }
  ...
  tkrreplot(BiplotRegion.image,fun=Axes.None.plot,hscale=BiplotRegion.HorizontalScale.func(),vscale=BiplotRegion.VerticalScale.func())
  .Tcl("update")

  Transformation<-function(Y) switch(tclvalue(Points.var),"10"=Identity(Points.DistanceMetric.DissimilarityMatrix,Y,WeightMat),"11"=MonotoneRegression(Points.DistanceMetric.DissimilarityMatrix,Y,WeightMat),"12"=MonotoneSpline(Points.DistanceMetric.DissimilarityMatrix,Y,WeightMat))

  if (!identical(Points.DistanceMetric.DissimilarityMatrix,t(Points.DistanceMetric.DissimilarityMatrix))) stop("`Points.DistanceMetric.DissimilarityMatrix' is required to be symmetric.")
  if (!identical(WeightMat,t(WeightMat))) stop("`WeightMat' is required to be symmetric.")
  Wupper<-WeightMat
  ...
  Stressfunc<-function(Y,DispMat)
    {
  	.5*sum(WeightMat*(DispMat-as.matrix(dist(Y)))^2)
    }
  ...
  for (i in 1:boptions$MDS.MaximumIterations)
    {
    Biplot.Y<<-ginv(V)%*%Bfunc(Z,Points.DistanceMetric.DisparityMatrix)%*%Z
    ...
    if (tclvalue(Other.LiveUpdates.var)=="1" && i%%boptions$IterationsToLiveUpdate==0)
      ...
      if (tclvalue(Additional.AlphaBag.var)=="1") Additional.AlphaBag.autcmd()
      tkrreplot(BiplotRegion.image,fun=Axes.None.plot,hscale=BiplotRegion.HorizontalScale.func(),vscale=BiplotRegion.VerticalScale.func())
      if (tclvalue(tcl(DiagnosticTabs.nb,"index","current"))=="1") tkrreplot(PointsTab.image,PointsTab.plot.ShepardDiagram,hscale=ConvergenceTab.HorizontalScale.func(),vscale=ConvergenceTab.VerticalScale.func())
      if (tclvalue(tcl(DiagnosticTabs.nb,"index","current"))=="0") tkrreplot(ConvergenceTab.image,ConvergenceTab.plot,hscale=ConvergenceTab.HorizontalScale.func(),vscale=ConvergenceTab.VerticalScale.func())
      .Tcl("update")
      }
...
Points.MDS.MonotoneSplineTransformation.cmd<-function()
  ...
  local.GUI.func<-function()
    ...
    spinbox1<-tkwidget(top,"SpinBox",textvariable=NewAllKnots,range=c("2","10","1"),editable=FALSE,justify="right")
    label2<-tk2label(frame1,text="Spline degree")
    NewDegree<-tclVar(Points.MDS.MonotoneSplineTransformation.degree)
    spinbox2<-tkwidget(top,"SpinBox",textvariable=NewDegree,range=c("1","2","1"),editable=FALSE,justify="right")
    button1<-tk2button(top,text="OK",width=10,command=onOK)
    button3<-tk2button(top,text="Default",width=10,command=onDefault)

    tkplace(frame1,relx=.5,rely=.4,relwidth=.9,relheight=.5,anchor="center")
    tkplace(label1,relx=.05,rely=.25,"in"=frame1,anchor="w")
    tkplace(spinbox1,relx=.8,rely=.25,relwidth=.15,"in"=frame1,anchor="w")
    tkplace(label2,relx=.05,rely=.7,"in"=frame1,anchor="w")
    tkplace(spinbox2,relx=.8,rely=.7,relwidth=.15,"in"=frame1,anchor="w")
    tkplace(button1,relx=.95,rely=.85,anchor="e")
    tkplace(button3,relx=0.05,rely=.85,anchor="w")
...
Points.MDS.MonotoneSplineTransformation.autcmd<-function(FollowThrough=TRUE)
  ...
  k<-Points.MDS.MonotoneSplineTransformation.AllKnots-2
  r<-Points.MDS.MonotoneSplineTransformation.degree

  MonotoneSplineSetup<-function(DissMat)
    {
    match.arg(as.character(r),choices=c("0","1","2"))
    diss<-sort(DissMat[lower.tri(DissMat)])
    s<-length(diss)
    t1<-quantile(diss,probs=seq(0,1,length=k+2))

    M<-matrix(nrow=s,ncol=r+k)
    switch(r+1,{ for (j in 2:(k+1+r)) M[,j-1]<-as.numeric(sapply(diss,function(pi) t1[j]<=pi & pi<=t1[k+2])) },
    	   { for (j in 2:(k+1+r)) M[,j-1]<-sapply(diss,function(pi) if (t1[j-1]<=pi & pi<t1[j]) (pi-t1[j-1])/(t1[j]-t1[j-1])
    									else if (t1[j]<=pi & pi <=t1[k+2]) 1
    										else 0 ) },
    	   { t1<-c(min(t1),t1,max(t1))
    	     for (j in 3:(k+2+r)) M[,j-2]<-sapply(diss,function(pi) if (t1[j-2]<=pi & pi<t1[j-1]) (t1[j-2]-pi)^2/(t1[j-1]-t1[j-2])/(t1[j]-t1[j-2])
    									else if(t1[j-1]<=pi & pi<t1[j]) 1-(t1[j]-pi)^2/(t1[j]-t1[j-1])/(t1[j]-t1[j-2])
    										else if (t1[j]<=pi & pi<=t1[k+3]) 1
    											else 0 ) } )
    M<-cbind(1,M)
...
Axes.None.cmd<-function(FollowThrough=TRUE)
  ...
  if (Biplot.zoom.mode==0) tkwm.title(GUI.TopLevel,paste("BiplotGUI -",Axes.None.title))
    else tkwm.title(GUI.TopLevel,paste("BiplotGUI -",Axes.None.title,"(zoomed)"))
...
Axes.None.motion<-function(x,y)
  ...
  if (Biplot.par$usr[1]<=Biplot.XY.move[1] && Biplot.XY.move[1]<=Biplot.par$usr[2] && Biplot.par$usr[3]<=Biplot.XY.move[2] && Biplot.XY.move[2]<=Biplot.par$usr[4])
    { # inside biplot
    Biplot.WasInside<<-TRUE
    if (Biplot.OverPoint() || Kraal.moving.status) tkconfigure(GUI.TopLevel,cursor="hand2")
      else tkconfigure(GUI.TopLevel,cursor="arrow")
    }
    else
      { # outside biplot
      if (Kraal.moving.status) tkconfigure(GUI.TopLevel,cursor="hand2")
      if (Biplot.WasInside)
        {
        tkconfigure(GUI.TopLevel,cursor="arrow")
...
Axes.None.LeftRelease<-function(x,y)
  ...
  if (Kraal.moving.status && (temp.XY[1]<Biplot.par$usr[1] || temp.XY[1]>Biplot.par$usr[2] || temp.XY[2]<Biplot.par$usr[3] || temp.XY[2]>Biplot.par$usr[4])) switch(Kraal.moving.type,
    "point"=
      {
      GUI.BindingsOff()
...
Axes.None.RightClick<-function(x,y)
  ...
  if (Biplot.par$usr[1]<=Biplot.XY.RightClick[1] && Biplot.XY.RightClick[1]<=Biplot.par$usr[2] && Biplot.par$usr[3]<=Biplot.XY.RightClick[2] && Biplot.XY.RightClick[2]<=Biplot.par$usr[4])
    { # Inside biplot
    if (Biplot.OverPoint()) tkpopup(Biplot.RightClickOnPoint.Menu,tclvalue(tkwinfo("pointerx",BiplotRegion.image)),tclvalue(tkwinfo("pointery",BiplotRegion.image)))
      else tkpopup(Biplot.None.RightClickInside.Menu,tclvalue(tkwinfo("pointerx",BiplotRegion.image)),tclvalue(tkwinfo("pointery",BiplotRegion.image)))
    }
    else tkpopup(Biplot.None.RightClickOutside.Menu,tclvalue(tkwinfo("pointerx",BiplotRegion.image)),tclvalue(tkwinfo("pointery",BiplotRegion.image))) # Outside biplot
  }
...
Axes.Regression.cmd<-function(FollowThrough=TRUE)
  ...
  if (Biplot.zoom.mode==0) tkwm.title(GUI.TopLevel,paste("BiplotGUI -",Axes.Regression.title))
    else tkwm.title(GUI.TopLevel,paste("BiplotGUI -",Axes.Regression.title,"(zoomed)"))
...
Axes.Regression.determine<-function()
  ...
  if (tclvalue(Points.var)=="0") Biplot.B3D<<-t(solve(t(Biplot.Y3D)%*%Biplot.Y3D)%*%t(Biplot.Y3D)%*%Biplot.Xtransformed)
...
Axes.Procrustes.cmd<-function(FollowThrough=TRUE)
  ...
  if (Biplot.zoom.mode==0) tkwm.title(GUI.TopLevel,paste("BiplotGUI -",Axes.Procrustes.title))
    else tkwm.title(GUI.TopLevel,paste("BiplotGUI -",Axes.Procrustes.title,"(zoomed)"))
...
Axes.Procrustes.determine.interpolative<-function()
  ...
  ProcrustesFit<-function (X,Y,translate=TRUE,detail=TRUE)
    ...
    q<-ncol(X)
    p<-ncol(Y)
    if(n!=nrow(Y)) stop("X and Y are required to have the same number of rows.")
    if(q>p) stop("Y is required to have at least as many columns as X.")
    if (q<p) X<-cbind(X,matrix(0,nrow=n,ncol=p-q))
    if (translate)
      {
      X<-scale(X,scale=FALSE); attr(X,"scaled:center")<-NULL
      Y<-scale(Y,scale=FALSE); attr(Y,"scaled:center")<-NULL
      }
    A<-svd(t(Y)%*%X)$v%*%t(svd(t(Y)%*%X)$u)
    Xnew<-X%*%A
    rho<-sum(diag(SquareRootMatrix(t(X)%*%Y%*%t(Y)%*%X)))/sum(diag(t(X)%*%X))
    Xnew<-Xnew*rho
    Rsquared<-1-sum(diag(SquareRootMatrix(t(X)%*%Y%*%t(Y)%*%X)))^2/(sum(diag(t(X)%*%X))*sum(diag(t(Y)%*%Y)))
    if (detail) list(Y=Y,"Matched X"=Xnew,"R squared"=Rsquared,"Translate"=translate,"Reflection/rotation matrix A"=A,"Isotropic dilation rho"=rho)
    }

  temp1<-2
  X<-Xold<-Biplot.Xtransformed
  Y<-Yold<-Biplot.Y
  if (temp1<p.in) Y<-cbind(Y,matrix(0,nrow=n.in,ncol=p.in-temp1))
  rhostar<-1
  ConvergenceTab.axes.StressVector<<-sum((X[,-(1:temp1)]-Y[,-(1:temp1)])^2)
...
Axes.Procrustes.interpolate<-function(ToInterpolate)
  ...
  B<-Axes.Procrustes.determine.interpolative()[[1]] # The behavious is correct for the log-transformations. If colMeans(Data) values are interpolated, the interpolated point isn't at (0,0). It would have been if had interpolated exp(colMeans(log(Data)).
...
Axes.CircularNonLinear.cmd<-function(FollowThrough=TRUE)
  {
  if (tclvalue(View.AxisLabels.var)%in%c("-1","1")) View.AxisLabels.var<<-tclVar("2")
  Additional.ClassificationRegion.var<<-tclVar("0")
  if (Points.skipped)
    {
    switch(tclvalue(Points.DistanceMetric.var),
  ...
  if (Biplot.zoom.mode==0) tkwm.title(GUI.TopLevel,paste("BiplotGUI -",Axes.CircularNonLinear.title))
    else tkwm.title(GUI.TopLevel,paste("BiplotGUI -",Axes.CircularNonLinear.title,"(zoomed)"))
...
Axes.CircularNonLinear.determine<-function(rho=2)
  ...
  if (tclvalue(tkget(SettingsBox.action.combo))=="Predict")
    ...
    if (any(eigenB$values[-n.in]<(-eps)))
      {
      tkmessageBox(title="Circular non-linear",parent=GUI.TopLevel,message="The configuration is not embeddable. A regression biplot is shown instead.",icon="warning",type="ok")
    ...
    U<-eigenB$vectors[,-n.in]%*%(lambda^.5) # U is the textbook's Y.

    if (rho==2) Y<-Biplot.Y
      else if (rho==3) Y3D<<-Biplot.Y3D
    ...
    derivat<-function(x.vec,mu)
        ...
        if(tclvalue(Points.DistanceMetric.var)=="2")
          {
		      afgeleide.funksie<-TRUE
		      antw<-2*x.vec*((x.vec-mu)/(x.vec+mu)^3)
          }
        antw
        }

    tempfunc<-function(j,unscaled.X,Xmat,D,U,lambda,one,B)
      {
      temptickn<-bpar$axes.tick.n[variables.in[j]]

      PrettyMarkers<-zapsmall(pretty(Data[samples.in,variables.in[j]],n=bpar$axes.tick.n[variables.in[j]]))
      PrettyMarkersIncrement<-PrettyMarkers[2]-PrettyMarkers[1]
      PrettyMarkersTemp<-PrettyMarkers[PrettyMarkers-PrettyMarkersIncrement/boptions$axes.tick.inter.n[variables.in[j]]/10>=min(Data[samples.in,variables.in[j]]) & PrettyMarkers+PrettyMarkersIncrement/boptions$axes.tick.inter.n[variables.in[j]]/10<=max(Data[samples.in,variables.in[j]])]
      ...
      markers<-zapsmall(c(min(markers)-PrettyMarkersIncrement/boptions$axes.tick.inter.n[variables.in[j]]/10,markers,max(markers)+PrettyMarkersIncrement/boptions$axes.tick.inter.n[variables.in[j]]/10))
      PrettyMarkersIndex<-match(PrettyMarkers,markers)

      std.markers<-PrettyMarkers
      interval<-SettingsBox.transformation.func(std.markers,WhichCol=j)
      axis.vals<-SettingsBox.transformation.func(markers,WhichCol=j)
      aantal.punte<-length(axis.vals)

      Biplot.variable[[j]]<<-list(markers=markers,PrettyMarkers=PrettyMarkers,PrettyMarkersCharacter=PrettyMarkersCharacter,PrettyMarkersIndex=PrettyMarkersIndex,MarkersCentred=axis.vals)

      as.punte<-matrix(0,nrow=aantal.punte,ncol=rho)

      lambda.diag<-diag(lambda)
      lambda.diag.inv<-rep(0,length(lambda.diag))
      lambda.diag.inv[lambda.diag>0]<-1/lambda.diag[lambda.diag>0]
      lambda.inv<-diag(lambda.diag.inv)
      B.inv<-U%*%(lambda.inv^2)%*%t(U)
      dd<-rep(0,n.in)

      dd13<-switch(tclvalue(Points.DistanceMetric.var),
        "0"=apply(Xmat,1,function(x) sum(x^2))-apply(Xmat,1,function(x,j) x[j]^2,j=j),
        "1"=apply(Xmat,1,function(x) sum(abs(x)))-apply(Xmat,1,function(x,j) abs(x[j]),j=j),
        "2"=apply(Xmat,1,function(x) sum((x/x)^2))-apply(Xmat,1,function(x, j) (x[j]/x[j])^2,j=j))
      for (mu in axis.vals)
        {
        dd<-dd13+switch(tclvalue(Points.DistanceMetric.var),
          "0"=apply(Xmat,1,function(x,mu,j) (x[j]-mu)^2,mu=mu,j=j),
          "1"=apply(Xmat,1,function(x,mu,j) abs(x[j]-mu),mu=mu,j=j),
          "2"=apply(Xmat,1,function(x,mu,j) ((x[j]-mu)/(x[j]+mu))^2,mu=mu,j=j))
        dd<--.5*dd
        dfdmu<-derivat(Xmat[,j],mu)
        y1<-lambda.inv%*%t(U)%*%(dd-(1/n.in)*D%*%one)
        ym1<-sqrt(abs(((1/n.in)^2)*(t(one)%*%D%*%one)-(2/n.in)*t(one)%*%dd-(t(y1)%*%y1)))
        yy<-c(y1,ym1)
        t1<-lambda.inv%*%t(U)%*%dfdmu
        tm1<-(1/ym1)*(t(dd)%*%B.inv-(1/n.in)*t(one)%*%D%*%B.inv+(1/n.in)*t(one))%*%dfdmu
        tt<-c(t1,tm1)
        tm1ym1<-(t(dd)%*%B.inv-(1/n.in)*t(one)%*%D%*%B.inv+(1/n.in)*t(one))%*%dfdmu
        zmu2<-Axes.CircularNonLinear.g[1:rho]-((t(tt[1:rho])%*%Axes.CircularNonLinear.g[1:rho]-t(t1)%*%y1+tm1ym1)/(t(tt[1:rho])%*%tt[1:rho]))*tt[1:rho]
        as.punte[axis.vals==mu,1:rho]<-zmu2
        }
      as.punte
      }

    z.axes<-lapply(1:p.in,tempfunc,unscaled.X=Data,Xmat=Biplot.Xtransformed,D=D,U=U,lambda=lambda,one=one,B=B)
    if (rho==2) Biplot.axis<<-z.axes
      else if (rho==3) Biplot.axis3D<<-z.axes
    }
...
Axes.CircularNonLinear.predictions<-function(ToProject=Biplot.points.WhereHighlight)
  ...
  A.vec<-(ToProject+Axes.CircularNonLinear.g[1:2])/2 # Center of the circle.
  r<-sqrt(sum((ToProject-A.vec)^2)) # Radius of the circle.

  lambda.func<-function(tempA,tempB)
    {
    c.vec<<-Biplot.axis[[tempA]][tempB,]
    d.vec<<-Biplot.axis[[tempA]][tempB+1,]
    a.scalar<-c.vec%*%c.vec-2*c.vec%*%d.vec+d.vec%*%d.vec
    b.scalar<-2*c.vec%*%d.vec-2*A.vec%*%c.vec+2*d.vec%*%A.vec-2*d.vec%*%d.vec
    c.scalar<--2*d.vec%*%A.vec+d.vec%*%d.vec+A.vec%*%A.vec-r^2
    if (abs(a.scalar)>eps)
      {
      if (b.scalar^2-4*a.scalar*c.scalar>=-eps)
        {
        lambda1<-(-b.scalar+sqrt(max(0,b.scalar^2-4*a.scalar*c.scalar)))/(2*a.scalar)
        lambda2<-(-b.scalar-sqrt(max(0,b.scalar^2-4*a.scalar*c.scalar)))/(2*a.scalar)
        }
        else lambda1<-lambda2<-NA # Imaginary number.
      }
  ...
  interpolate.func<-function(lambda,tempA,tempB)
    ...
    if (temp5>sqrt(sum((X.vec.temp-ToProject)^2)))
      {
      X.vec<<-X.vec.temp
      X.vec.marker.label<<-lambda*Biplot.variable[[tempA]]$markers[tempB]+(1-lambda)*Biplot.variable[[tempA]]$markers[tempB+1]
      lambda.temp<<-tempB
  ...
  for (tempA in 1:p.in)
    {
    temp0<-as.matrix(dist(rbind(A.vec,Biplot.axis[[tempA]])))[-1,1]
    temp1<-sign(temp0-r) # +1: Outside the circle; -1: Inside the circle.
    temp2<-sign(diff(temp1)) # +1 or -1: Changed from outside/inside the circle to inside/outside the cirlce.
    temp3<-abs(temp2) # 1: Changed from outside/inside the circle to inside/outside the circle.
    temp4<-which(temp3==1) # Indices of where the change from outside/inside to inside/outside the circle starts.
    temp5<-Inf # Variable used to find minimum.
    X.vec<-NULL # The predicted value in terms of display space coordinates.
    X.vec.marker.label<-NULL
    temp6<-which(temp1[-length(temp1)]==1 & temp3==0) # Indices of points on axis which are outside the circle, and for which the succeeding point is also outside the circle.
    u.mat<-Biplot.axis[[tempA]][temp6+1,]-Biplot.axis[[tempA]][temp6,]
    v.mat<-sweep(-Biplot.axis[[tempA]][temp6,],2,A.vec,"+")
    temp7<-temp6[temp7oftemp6<-(rowSums(u.mat*v.mat)>=-eps*rowSums(u.mat^2) & rowSums(u.mat*v.mat)<=(1+eps)*rowSums(u.mat^2))]
    for (tempB in sort(c(temp4,temp7)))
      {
      lambda<-lambda.func(tempA,tempB)
      if (!is.na(lambda[1]) && lambda[1]>=-eps && lambda[1]<=1+eps) interpolate.func(lambda[1],tempA,tempB)
      if (!is.na(lambda[1]) && lambda[2]>=-eps && lambda[2]<=1+eps) interpolate.func(lambda[2],tempA,tempB)
      }
      if (is.null(X.vec))
        {
        X.vec<-c(NA,NA)
      ...
      Biplot.points.WhereClosestOnAxis.temp<-matrix(rbind(Biplot.points.WhereClosestOnAxis.temp,X.vec),ncol=2)
      Biplot.points.WhichClosestOnAxis.temp<-c(Biplot.points.WhichClosestOnAxis.temp,X.vec.marker.label)
...
Axes.Default.cmd<-function()
  {
  if (tclvalue(Points.var)=="0") switch(tclvalue(Points.DistanceMetric.var),
    "0"={ Biplot.Axes.var<<-tclVar("11"); Axes.Regression.cmd() },
    "1"={ Biplot.Axes.var<<-tclVar("13"); Axes.CircularNonLinear.cmd() },
...
Additional.Interpolate.ANewSample.cmd<-function()
  ...
  local.GUI.func<-function()
    ...
    onDefault<-function()
      {
      NewValues<<-tclVar(paste(colMeans(Data[samples.in,variables.in]),collapse=","))
    ...
    onFormat<-function()
      ...
      tkplace(tk2label(frame_1,text="Symbol"),x=11,y=20,"in"=frame_1)
      spinbox_1<-tkwidget(frame_1,"SpinBox",textvariable=local.ANewSample.pch.var,editable=FALSE,values=c(" ","NA",0:25),justify="right")
      tkplace(spinbox_1,relx=.95,y=20,height=18,relwidth=.125,"in"=frame_1,anchor="ne")

      tkplace(tk2label(frame_1,text="Size"),x=11,y=40,"in"=frame_1)
      entry_1<-tk2entry(frame_1,textvariable=local.ANewSample.cex.var,justify="right",takefocus=FALSE)
      tkplace(entry_1,relx=.95,y=40,height=18,relwidth=.125,"in"=frame_1,anchor="ne")

      tkplace(tk2label(frame_1,text="Foreground colour"),x=11,y=60,"in"=frame_1)
      label_1<-tklabel(frame_1,background=local.ANewSample.col.fg.var,relief="groove",borderwidth="1.5p")
      tkplace(label_1,relx=.95,y=60,height=18,relwidth=.125,"in"=frame_1,anchor="ne")
      ...
      tkplace(tk2label(frame_1,text="Background colour"),x=11,y=80,"in"=frame_1)
      label_2<-tklabel(frame_1,background=local.ANewSample.col.bg.var,relief="groove",borderwidth="1.5p")
      tkplace(label_2,relx=.95,y=80,height=18,relwidth=.125,"in"=frame_1,anchor="ne")
      ...
      tkplace(tk2label(frame_2,text="Font"),x=11,y=20,"in"=frame_2)
      spinbox_2<-tkwidget(frame_2,"SpinBox",textvariable=local.ANewSample.label.font.var,editable=FALSE,values=c(" ",1:4),justify="right")
      tkplace(spinbox_2,relx=.95,y=20,height=18,relwidth=.125,"in"=frame_2,anchor="ne")

      tkplace(tk2label(frame_2,text="Size"),x=11,y=40,"in"=frame_2)
      entry_2<-tk2entry(frame_2,textvariable=local.ANewSample.label.cex.var,justify="right",takefocus=FALSE)
      tkplace(entry_2,relx=.95,y=40,height=18,relwidth=.125,"in"=frame_2,anchor="ne")

      tkplace(tk2label(frame_2,text="Colour"),x=11,y=60,"in"=frame_2)
      label_3<-tklabel(frame_2,background=local.ANewSample.label.col.var,relief="groove",borderwidth="1.5p")
      tkplace(label_3,relx=.95,y=60,height=18,relwidth=.125,"in"=frame_2,anchor="ne")
      ...
      tkplace(tk2label(frame_2,text="Horizontal offset"),x=11,y=80,"in"=frame_2)
      entry_3<-tk2entry(frame_2,textvariable=local.ANewSample.label.HorizOffset.var,justify="right",takefocus=FALSE)
      tkplace(entry_3,relx=.95,y=80,height=18,relwidth=.125,"in"=frame_2,anchor="ne")

      tkplace(tk2label(frame_2,text="Vertical offset"),x=11,y=100,"in"=frame_2)
      entry_4<-tk2entry(frame_2,textvariable=local.ANewSample.label.VertOffset.var,justify="right",takefocus=FALSE)
      tkplace(entry_4,relx=.95,y=100,height=18,relwidth=.125,"in"=frame_2,anchor="ne")

      button_1<-tk2button(top_,text="Defaults",width=10,command=onDefaults_)
      button_2<-tk2button(top_,text="OK",width=10,command=onOK_)
      button_3<-tk2button(top_,text="Cancel",width=10,command=onCancel_)

      tkplace(button_1,relx=.05,rely=.99,anchor="sw")
      tkplace(button_2,relx=.775,rely=.99,anchor="se")
      tkplace(button_3,relx=.96,rely=.99,anchor="se")
      ...
      tkwm.geometry(top_,paste("390x292","+",round(GUI.AvailableScreenWidth/2-390/2,0),"+",round(GUI.AvailableScreenHeight/2-292/2,0),sep=""))
    ...
    onOK<-function()
      {
      tkdestroy(top)
      eval(parse(text=paste("Additional.Interpolate.ANewSample.values<<-matrix(c(",as.character(tclvalue(NewValues)),"),ncol=1)")))
    ...
    NewValues<-tclVar(paste(Additional.Interpolate.ANewSample.values,collapse=","))
    ...
    button1<-tk2button(top,text="Defaults",width=9,command=onDefault)
    button2<-tk2button(top,text="Format",width=9,command=onFormat)
    button3<-tk2button(top,text="OK",width=9,command=onOK)
    button4<-tk2button(top,text="Off",width=9,command=onOff)

    tkplace(frame1,relx=.5,rely=.1,relwidth=.92,height=100,anchor="n")
    tkplace(label1,relx=.02,y=18,"in"=frame1,anchor="w")
    tkplace(entry1,relx=.57,y=18,relwidth=.4,height=17,"in"=frame1,anchor="w")
    tkplace(label2,relx=.02,y=38,"in"=frame1,anchor="w")
    tkplace(entry2,relx=.57,y=38,relwidth=.4,height=17,"in"=frame1,anchor="w")
    tkplace(label3,relx=.02,y=58,"in"=frame1,anchor="w")
    tkplace(checkbutton1,relx=.91,y=58,height=17,"in"=frame1,anchor="w")

    tkplace(button1,relx=0.035,rely=.89,anchor="w")
    tkplace(button2,relx=0.24,rely=.89,anchor="w")
    tkplace(button3,relx=.75,rely=.89,anchor="e")
    tkplace(button4,relx=.955,rely=.89,anchor="e")
    ...
    tkwm.geometry(top,paste("320x150","+",round(GUI.AvailableScreenWidth/2-320/2,0),"+",round(GUI.AvailableScreenHeight/2-150/2,0),sep=""))
    tkwm.focusmodel(top,"active")
    tkwm.resizable(top,"0","0")
    tkwm.deiconify(top)
...
Additional.Interpolate.SampleGroupMeans.cmd<-function()
  ...
  local.GUI.func<-function()
    ...
    onOK<-function()
      {
      Additional.Interpolate.SampleGroupMeans.for<<-which(tclvalue(tkget(combo1))==ForPossibilities)-2
    ...
    if (g==1)
      ...
      NewFor<-"All points"
      combo1<-tkwidget(frame1,"ComboBox",editable=FALSE,values=ForPossibilities,text=NewFor,state="disabled")
      }
      else
        {
        ForPossibilities<-c("All samples","All groups",bpar$groups.label.text)
        NewFor<-ForPossibilities[Additional.Interpolate.SampleGroupMeans.for+2]
        combo1<-tkwidget(frame1,"ComboBox",editable=FALSE,values=ForPossibilities,text=NewFor)
        }
    ...
    tkplace(frame1,relx=.5,rely=.1,relwidth=.92,height=100,anchor="n")
    tkplace(label1,relx=.02,y=18,"in"=frame1,anchor="w")
    tkplace(combo1,relx=.57,y=18,relwidth=.4,height=17,"in"=frame1,anchor="w")
    tkplace(label2,relx=.02,y=38,"in"=frame1,anchor="w")
    tkplace(checkbutton1,relx=.91,y=38,height=17,"in"=frame1,anchor="w")
  ...
  if (tclvalue(Additional.Interpolate.SampleGroupMeans.var)=="1")
    ...
    Additional.Interpolate.SampleGroupMeans.label.text<<-switch(as.character(Additional.Interpolate.SampleGroupMeans.for),
      "-1"="All samples",
      "0"=if (g==1) "All samples" else bpar$groups.label.text[groups.in],
...
Additional.Interpolate.SampleGroupMeans.autcmd<-function(FollowThrough=TRUE)
  ...
  if (tclvalue(Additional.Interpolate.SampleGroupMeans.var)=="1")
    ...
    if (substr(tclvalue(tkget(SettingsBox.transformation.combo)),start=1,stop=3)=="Log")
      {
      Additional.Interpolate.SampleGroupMeans.coordinates<<-switch(as.character(Additional.Interpolate.SampleGroupMeans.for),
        "-1"=matrix(Biplot.interpolate(exp(colMeans(log(Data[samples.in,variables.in])))),ncol=2),
        "0"=t(apply(exp(apply(log(Data[samples.in,variables.in]),2,function(x) tapply(x,factor(group[samples.in],exclude=NULL),mean))),1,Biplot.interpolate)),
        matrix(Biplot.interpolate(exp(colMeans(log(Data[samples.in[as.numeric(group[samples.in])==Additional.Interpolate.SampleGroupMeans.for],variables.in])))),ncol=2))
      }
      else
      {
      Additional.Interpolate.SampleGroupMeans.coordinates<<-switch(as.character(Additional.Interpolate.SampleGroupMeans.for),
        "-1"=matrix(Biplot.interpolate(colMeans(Data[samples.in,variables.in])),ncol=2),
        "0"=t(apply(apply(Data[samples.in,variables.in],2,function(x) tapply(x,factor(group[samples.in],exclude=NULL),mean)),1,Biplot.interpolate)),
        matrix(Biplot.interpolate(colMeans(Data[samples.in[as.numeric(group[samples.in])==Additional.Interpolate.SampleGroupMeans.for],variables.in])),ncol=2))
      }
    }
  }
...
Additional.ConvexHull.cmd<-function()
  ...
  local.GUI.func<-function()
    ...
    onOK<-function()
      {
      Additional.ConvexHullAlphaBag.for<<-which(tclvalue(tkget(combo1))==ForPossibilities)-2
    ...
    if (g==1)
      ...
      combo1<-tkwidget(frame1,"ComboBox",editable=FALSE,values=ForPossibilities,text=NewFor,state="disabled")
        ...
        {
        ForPossibilities<-c("All points","All groups",bpar$groups.label.text)
        NewFor<-ForPossibilities[Additional.ConvexHullAlphaBag.for+2]
        combo1<-tkwidget(frame1,"ComboBox",editable=FALSE,values=ForPossibilities,text=NewFor)
        }

    button1<-tk2button(top,text="Default",width=9,command=onDefault,state=if (g==1) "disabled" else "normal")
    button2<-tk2button(top,text="Format",width=9,command=onFormat)
    button3<-tk2button(top,text="OK",width=9,command=onOK)
    button4<-tk2button(top,text="Off",width=9,command=onOff)
...
Additional.ConvexHull.autcmd<-function(FollowThrough=TRUE)
  ...
  switch(as.character(Additional.ConvexHullAlphaBag.for),
    "-1"=Additional.ConvexHullAlphaBag.coordinates<<-list(Y[c(temp1<-chull(Y),temp1[1]),]),
    "0"=Additional.ConvexHullAlphaBag.coordinates<<-tapply(1:n.in,factor(group[samples.in],exclude=NULL),function(x) Y[x[c(temp1<-chull(Y[x,]),temp1[1])],]),
    {
    temp1<-which(as.numeric(group[samples.in])==Additional.ConvexHullAlphaBag.for)
    if (length(temp1)>0) Additional.ConvexHullAlphaBag.coordinates<<-list(Y[temp1[c(temp2<-chull(Y[temp1,]),temp2[1])],])
      else Additional.ConvexHullAlphaBag.coordinates<<-NULL
    })
  }
...
Additional.AlphaBag.cmd<-function()
  ...
  local.GUI.func<-function()
    ...
    onOK<-function()
      ...
      if (temp1>0 && temp1<1)
       ...
       bpar$ConvexHullAlphaBag.TukeyMedian.LabelsInBiplot<<-as.logical(as.numeric(tclvalue(NewLabelsInBiplot)))
        ...
        else tkmessageBox(title="Alpha-bags",parent=top,message="Choose a value of alpha larger than 0 and smaller than 1.",icon="warning",type="ok")
      }
    ...
    button1<-tk2button(top,text="Default",width=9,command=onDefault)
    button2<-tk2button(top,text="Format",width=9,command=onFormat)
    button3<-tk2button(top,text="OK",width=9,command=onOK)
    button4<-tk2button(top,text="Off",width=9,command=onOff)
    ...
    tkplace(combo1,relx=.57,y=18,relwidth=.4,height=17,"in"=frame1,anchor="w")
    tkplace(label2,relx=.02,y=38,"in"=frame1,anchor="w")
    tkplace(entry1,relx=.825,y=38,relwidth=.145,height=17,"in"=frame1,anchor="w")
    tkplace(label3,relx=.02,y=58,"in"=frame1,anchor="w")
    tkplace(checkbutton1,relx=.91,y=58,"in"=frame1,anchor="w")
    tkplace(label4,relx=.02,y=78,"in"=frame1,anchor="w")
    tkplace(checkbutton2,relx=.91,y=78,height=17,"in"=frame1,anchor="w")
  ...
  if (tclvalue(Additional.AlphaBag.var)=="1")
    ...
    Additional.ConvexHullAlphaBag.TukeyMedian.label.text<<-switch(as.character(Additional.ConvexHullAlphaBag.for),
      "-1"="All points",
      "0"=if (g==1) "All points" else bpar$groups.label.text[groups.in],
...
Additional.AlphaBag.autcmd<-function()
  ...
  CalculateAlphaBag<-function(x,y,alpha,name)
    ...
    whisk<-2
    abagplot.out<-.Fortran("abagplot",as.integer(n),as.integer(alpha),x,y,as.integer(whisk),tukm=double(2),interpx=interpx,interpy=interpy,num=as.integer(0),datatyp=datatyp,indoutl=integer(n),datatyp2=datatyp2,pxpy=pxpy,boxpl=as.integer(0),nointer=as.integer(0),PACKAGE="BiplotGUI")
    ...
    if (all(x.vec == 0) || all(y.vec == 0))
      ...
      if (Additional.ConvexHullAlphaBag.FirstRun)
        {
        tkmessageBox(title="Alpha-bags",parent=GUI.TopLevel,message=paste("There are too few points to construct a ",alpha,"% alpha-bag for ",name,".\nA convex hull is shown instead.",sep=""),icon="warning",type="ok")
        tkfocus(GUI.TopLevel)
        }
      temp1<-cbind(x,y)
      list(temp1[c(temp2<-chull(temp1),temp2[1]),],c(NA,NA))
      }
      else
        {
        temp1<-which(!(x.vec==0 & y.vec==0))
        list(cbind(x.vec[c(temp1,temp1[1])],y.vec[c(temp1,temp1[1])]),tukmedian)
        }
    }
  ...
  switch(as.character(Additional.ConvexHullAlphaBag.for),
    "-1"=
      {
      temp1<-CalculateAlphaBag(as.matrix(Y[,1]),as.matrix(Y[,2]),Additional.ConvexHullAlphaBag.alpha*100,"All points")
      Additional.ConvexHullAlphaBag.coordinates<<-list(temp1[[1]])
      Additional.ConvexHullAlphaBag.TukeyMedian.coordinates<<-matrix(temp1[[2]],ncol=2)
      },
    "0"=
      {
      Additional.ConvexHullAlphaBag.coordinates<<-vector(length=g.in,mode="list")
      Additional.ConvexHullAlphaBag.TukeyMedian.coordinates<<-matrix(nrow=g.in,ncol=2)
      for (i in 1:g.in)
        {
        temp1<-which(as.numeric(group[samples.in])==groups.in[i])
        temp2<-CalculateAlphaBag(as.matrix(Y[temp1,1]),as.matrix(Y[temp1,2]),Additional.ConvexHullAlphaBag.alpha*100,bpar$groups.label.text[groups.in[i]])
        Additional.ConvexHullAlphaBag.coordinates[[i]]<<-temp2[[1]]
        Additional.ConvexHullAlphaBag.TukeyMedian.coordinates[i,]<<-temp2[[2]]
        }
      },
    {
    temp1<-which(as.numeric(group[samples.in])==Additional.ConvexHullAlphaBag.for)
    if (length(temp1)>0)
      {
      temp2<-CalculateAlphaBag(as.matrix(Y[temp1,1]),as.matrix(Y[temp1,2]),Additional.ConvexHullAlphaBag.alpha*100,bpar$groups.label.text[Additional.ConvexHullAlphaBag.for])
      Additional.ConvexHullAlphaBag.coordinates<<-list(temp2[[1]])
      Additional.ConvexHullAlphaBag.TukeyMedian.coordinates<<-matrix(temp2[[2]],ncol=2)
      }
      else
        {
        Additional.ConvexHullAlphaBag.coordinates<<-NULL
        Additional.ConvexHullAlphaBag.TukeyMedian.coordinates<<-c(NA,NA)
        }
    })
...
Additional.ConvexHullAlphaBag.FirstRun<-TRUE
Additional.ConvexHullAlphaBag.alpha<-.9
...
Additional.PointDensities.cmd<-function()
  ...
  local.GUI.func<-function()
    ...
    onOK<-function()
      {
      Additional.PointDensities.for<<-which(tclvalue(tkget(combo1))==ForPossibilities)-1
      Additional.PointDensities.palette<<-which(tclvalue(tkget(combo2))==PalettePossibilities)-2
    ...
    if (g==1)
      ...
      combo1<-tkwidget(frame1,"ComboBox",editable=FALSE,values=ForPossibilities,text=NewFor,state="disabled")
        ...
        NewFor<-ForPossibilities[Additional.PointDensities.for+1]
        combo1<-tkwidget(frame1,"ComboBox",editable=FALSE,values=ForPossibilities,text=NewFor)
        }
    label2<-tk2label(frame1,text="Palette")
    PalettePossibilities<-c("Terrain","Heat",1:8)
    NewPalette<-PalettePossibilities[Additional.PointDensities.palette+2]
    combo2<-tkwidget(frame1,"ComboBox",editable=FALSE,values=PalettePossibilities,text=NewPalette)
    ...
    tkplace(combo1,relx=.57,y=18,relwidth=.4,height=17,"in"=frame1,anchor="w")
    tkplace(label2,relx=.02,y=38,"in"=frame1,anchor="w")
    tkplace(combo2,relx=.77,y=38,relwidth=.2,height=17,"in"=frame1,anchor="w")
    tkplace(label3,relx=.02,y=58,"in"=frame1,anchor="w")
    tkplace(entry1,relx=.87,y=58,relwidth=.1,height=17,"in"=frame1,anchor="w")
  ...
  if (data.class(try(.find.package("KernSmooth"),TRUE))=="try-error" || data.class(try(.find.package("vcd"),TRUE))=="try-error")
    {
    tkmessageBox(title="Point densities",parent=GUI.TopLevel,message="This option requires the `KernSmooth' and `vcd' packages. \nPlease download and install.",icon="info",type="ok")
    tkfocus(GUI.TopLevel)
    Additional.PointDensities.var<<-tclVar("0")
    }
    else
  ...
  if (tclvalue(Additional.PointDensities.var)!="1") Additional.PointDensities.estimate<<-NULL
  # Does not call `Additional.PointDensities.autcmd'. It is called from within the plotting functions themselves.
  }

Additional.PointDensities.var<-tclVar("0")

Additional.PointDensities.autcmd<-function(FollowThrough=TRUE) # `Additional.PointDensities.autcmd' and `Additional.ClassificationRegion.autcmd' are the exceptions. They are called from within the plotting functions themselves.
  {
  if (as.numeric(tclvalue(Biplot.Axes.var))<10) Y<-Biplot.Y_
    else Y<-Biplot.Y

  if (Additional.PointDensities.for==0) Ylocal<-Y
    else Ylocal<-Y[as.numeric(group[samples.in])==Additional.PointDensities.for,]

  temp1<-list(c(min(Ylocal[,1],Biplot.par$usr[1]),max(Ylocal[,1],Biplot.par$usr[2])),c(min(Ylocal[,2],Biplot.par$usr[3]),max(Ylocal[,2],Biplot.par$usr[4])))
  Additional.PointDensities.estimate<<-bkde2D(Ylocal,bandwidth=c((Biplot.par$usr[2]-Biplot.par$usr[1])/20,(Biplot.par$usr[4]-Biplot.par$usr[3])/20),gridsize=c(51,51),range.x=temp1)

  temp2<-switch(as.character(Additional.PointDensities.palette),
    "-1"={rev(terrain_hcl(Additional.PointDensities.NumberOfColours,c.=c(50,0),l=c(70,100)))},
    "0"=rev(heat_hcl(Additional.PointDensities.NumberOfColours,c.=c(50,0),l=c(70,100))),
    {
    temp3<-seq(from=0,to=360,length.out=10)[-c(0,10)]
    rev(sequential_hcl(Additional.PointDensities.NumberOfColours,h=temp3[Additional.PointDensities.palette],c.=c(50,0),l=c(70,100)))
    })
  image(Additional.PointDensities.estimate$x1,Additional.PointDensities.estimate$x2,Additional.PointDensities.estimate$fhat,add=TRUE,col=temp2)
  }
...
Additional.ClassificationRegion.cmd<-function()
  ...
  local.GUI.func<-function()
    ...
    combo1<-tkwidget(frame1,"ComboBox",editable=FALSE,values=DimensionsPossibilities,text=NewDimensions)
    ...
    entry1<-tk2entry(frame1,textvariable=NewPixels,justify="right",takefocus=FALSE)
    ...
    tkplace(frame1,relx=.5,rely=.1,relwidth=.92,height=100,anchor="n")
    tkplace(label1,relx=.02,y=18,"in"=frame1,anchor="w")
    tkplace(combo1,relx=.77,y=18,relwidth=.2,height=17,"in"=frame1,anchor="w")
    tkplace(label2,relx=.02,y=38,"in"=frame1,anchor="w")
    tkplace(entry1,relx=.77,y=38,relwidth=.2,height=18,"in"=frame1,anchor="w")
  ...
  if (tclvalue(Additional.ClassificationRegion.var)=="1")
    {
    Additional.PointDensities.var<<-tclVar("0")
    Additional.PointDensities.estimate<<-NULL
    }
...
tkadd(MenuBar.File.SaveAs,"radiobutton",label="PDF...",underline="0",variable=File.SaveAs.var,value="0",command=function()
  {
  GUI.BindingsOff()
  File.SaveAs.PDF.cmd()
  GUI.BindingsOn()
  })
tkadd(MenuBar.File.SaveAs,"radiobutton",label="Postscript...",underline="4",variable=File.SaveAs.var,value="1",command=function()
  {
  GUI.BindingsOff()
  File.SaveAs.Postscript.cmd()
  GUI.BindingsOn()
  })
tkadd(MenuBar.File.SaveAs,"radiobutton",label="Metafile...",underline="0",variable=File.SaveAs.var,value="2",command=function()
  {
  GUI.BindingsOff()
  File.SaveAs.Metafile.cmd()
  GUI.BindingsOn()
  })
tkadd(MenuBar.File.SaveAs,"radiobutton",label="Bmp...",underline="0",variable=File.SaveAs.var,value="3",command=function()
  {
  GUI.BindingsOff()
  File.SaveAs.Bmp.cmd()
  GUI.BindingsOn()
  })
tkadd(MenuBar.File.SaveAs,"radiobutton",label="Png...",underline="1",variable=File.SaveAs.var,value="4",command=function()
  {
  GUI.BindingsOff()
  File.SaveAs.Png.cmd()
  GUI.BindingsOn()
...
tkadd(MenuBar.File.SaveAs.Jpeg,"radiobutton",label="50% quality...",underline="0",variable=File.SaveAs.var,value="5",command=function()
  {
  GUI.BindingsOff()
...
tkadd(MenuBar.File.SaveAs.Jpeg,"radiobutton",label="75% quality...",underline="0",variable=File.SaveAs.var,value="6",command=function()
  {
  GUI.BindingsOff()
...
tkadd(MenuBar.File.SaveAs.Jpeg,"radiobutton",label="100% quality...",underline="0",variable=File.SaveAs.var,value="7",command=function()
  {
  GUI.BindingsOff()
...
tkadd(MenuBar.File.SaveAs,"cascade",label="Jpeg",underline="0",menu=MenuBar.File.SaveAs.Jpeg)
tkadd(MenuBar.File.SaveAs,"radiobutton",label="PicTeX",underline="3",variable=File.SaveAs.var,value="8",command=function()
  {
  GUI.BindingsOff()
  File.SaveAs.PicTeX.cmd()
  GUI.BindingsOn()
  })
tkadd(MenuBar.File,"cascade",label="Save as",underline="0",accelerator="Ctrl+S",menu=MenuBar.File.SaveAs)
tkadd(MenuBar.File,"command",label="Copy",underline="0",accelerator="Ctrl+C",state=if (.Platform$OS.type != "windows") "disabled" else "normal",command=function()
  {
  GUI.BindingsOff()
  File.Copy.cmd()
  GUI.BindingsOn()
...
tkadd(MenuBar.File,"command",label="Print...",underline="0",accelerator="Ctrl-P",state=if (.Platform$OS.type != "windows") "disabled" else "normal",command=function()
  {
  GUI.BindingsOff()
  File.Print.cmd()
  GUI.BindingsOn()
...
tkadd(MenuBar.File,"separator")
tkadd(MenuBar.File,"command",label="Options...",underline="0",command=function()
  {
  GUI.BindingsOff()
  File.Options.cmd()
  GUI.BindingsOn()
...
tkadd(MenuBar.View,"radiobutton",label="Clip around points",underline="12",variable=View.ClipAround.var,value="0",command=function()
  {
  GUI.BindingsOff()
  View.ClipAroundPoints.cmd()
  GUI.BindingsOn()
  })
tkadd(MenuBar.View,"radiobutton",label="Clip around points and axes",underline="13",variable=View.ClipAround.var,value="1",command=function()
  {
  GUI.BindingsOff()
...
tkadd(MenuBar.View,"separator")
tkadd(MenuBar.View,"checkbutton",label="Show point labels",underline="11",variable=View.ShowPointLabels.var,command=function()
  {
  GUI.BindingsOff()
  View.ShowPointLabels.cmd()
  GUI.BindingsOn()
  })
tkadd(MenuBar.View,"checkbutton",label="Show point values",underline="11",variable=View.ShowPointValues.var,state="disabled",command=function()
  {
  GUI.BindingsOff()
...
tkadd(MenuBar.View,"separator")
tkadd(MenuBar.View,"checkbutton",label="Show group labels in legend",underline="5",variable=View.ShowGroupLabelsInLegend.var,state=if (g==1) "disabled" else "normal",command=function()
  {
  GUI.BindingsOff()
...
tkadd(MenuBar.View,"separator")
tkadd(MenuBar.View,"radiobutton",label="Don't show axis labels",underline="12",variable=View.AxisLabels.var,value="0",command=function()
  {
  GUI.BindingsOff()
  View.DontShowAxisLabels.cmd()
  GUI.BindingsOn()
  })
tkadd(MenuBar.View,"radiobutton",label="Show clinging axis labels",underline="5",variable=View.AxisLabels.var,value="1",command=function()
  {
  GUI.BindingsOff()
  View.ShowClingingAxisLabels.cmd()
  GUI.BindingsOn()
  })
tkadd(MenuBar.View,"radiobutton",label="Show axis labels in legend",underline="5",variable=View.AxisLabels.var,value="2",command=function()
  {
  GUI.BindingsOff()
...
tkadd(MenuBar.View,"separator")
tkadd(MenuBar.View,"checkbutton",label="Show Additional labels in legend",underline="6",variable=View.ShowAdditionalLabelsInLegend.var,command=function()
  {
  GUI.BindingsOff()
...
tkadd(MenuBar.Format,"command",label="By group...",underline="3",accelerator="Ctrl+G",command=function()
  {
  GUI.BindingsOff()
  Format.ByGroup.cmd()
  GUI.BindingsOn()
  })
tkadd(MenuBar.Format,"command",label="Axes...",underline="0",accelerator="Ctrl+A",command=function()
  {
  GUI.BindingsOff()
  Format.Axes.cmd()
  GUI.BindingsOn()
  })
tkadd(MenuBar.Format,"command",label="Interaction...",underline="0",command=function()
  {
  GUI.BindingsOff()
  Format.Interaction.cmd()
  GUI.BindingsOn()
  })
tkadd(MenuBar.Format,"command",label="Diagnostic tabs...",underline="0",command=function()
  {
  GUI.BindingsOff()
...
tkadd(MenuBar.Format,"command",label="Reset all...",underline="0",accelerator="Ctrl+R",command=function()
  {
  GUI.BindingsOff()
  Format.ResetAll.cmd()
  GUI.BindingsOn()
...
tkadd(MenuBar.Joint,"radiobutton",label="Covariance/Correlation",underline="0",variable=Biplot.Axes.var,value="1",accelerator="2",command=function()
  {
  GUI.BindingsOff()
...
tkadd(MenuBar.Joint,"radiobutton",label="CVA",underline="1",variable=Biplot.Axes.var,value="2",accelerator="3",state=if (g==1) "disabled" else "normal",command=function()
  {
  GUI.BindingsOff()
  Joint.CVA.cmd()
  GUI.BindingsOn()
...
tkadd(MenuBar.Points.DistanceMetric,"radiobutton",label="Pythagoras",underline="0",variable=Points.DistanceMetric.var,value="0",command=function()
  {
  GUI.BindingsOff()
  Points.DistanceMetric.Pythagoras.cmd()
  GUI.BindingsOn()
  })
tkadd(MenuBar.Points.DistanceMetric,"radiobutton",label="Square-root-of-Manhattan",underline="0",variable=Points.DistanceMetric.var,value="1",command=function()
  {
  GUI.BindingsOff()
  Points.DistanceMetric.SquareRootOfManhattan.cmd()
  GUI.BindingsOn()
  })
tkadd(MenuBar.Points.DistanceMetric,"radiobutton",label="Clark",underline="0",variable=Points.DistanceMetric.var,value="2",command=function()
  {
  GUI.BindingsOff()
  Points.DistanceMetric.Clark.cmd()
  GUI.BindingsOn()
  })
tkadd(MenuBar.Points.DistanceMetric,"radiobutton",label="Mahalanobis",underline="0",variable=Points.DistanceMetric.var,value="3",command=function()
  {
  GUI.BindingsOff()
  Points.DistanceMetric.Mahalanobis.cmd()
  GUI.BindingsOn()
  })
tkadd(MenuBar.Points,"cascade",label="Distance metric",underline="0",menu=MenuBar.Points.DistanceMetric)
tkadd(MenuBar.Points,"separator")
tkadd(MenuBar.Points,"radiobutton",label="PCO",underline="0",variable=Points.var,value="0",accelerator="A",command=function()
  {
  GUI.BindingsOff()
  Points.PCO.cmd()
  GUI.BindingsOn()
...
tkadd(MenuBar.Points.MDS,"checkbutton",label="Random initial configuration",underline="0",variable=Points.MDS.RandomInitialConfiguration.var)
tkadd(MenuBar.Points.MDS,"separator")
tkadd(MenuBar.Points.MDS,"radiobutton",label="Identity transformation",underline="0",variable=Points.var,value="10",accelerator="B",command=function()
  {
  GUI.BindingsOff()
  Points.MDS.IdentityTransformation.cmd()
  GUI.BindingsOn()
  })
tkadd(MenuBar.Points.MDS,"radiobutton",label="Monotone regression",underline="0",variable=Points.var,value="11",accelerator="C",command=function()
  {
  GUI.BindingsOff()
  Points.MDS.MonotoneRegression.cmd()
  GUI.BindingsOn()
  })
tkadd(MenuBar.Points.MDS,"radiobutton",label="Monotone spline transformation...",underline="9",variable=Points.var,value="12",accelerator="D",command=function()
  {
  GUI.BindingsOff()
...
tkadd(MenuBar.Points.MDS,"separator")
tkadd(MenuBar.Points.MDS,"radiobutton",label="Primary approach to ties",underline="20",variable=Points.MDS.ApproachToTies.var,value="0")
tkadd(MenuBar.Points.MDS,"radiobutton",label="Secondary approach to ties",underline="24",variable=Points.MDS.ApproachToTies.var,value="1")
tkadd(MenuBar.Points.MDS,"separator")
tkadd(MenuBar.Points.MDS,"checkbutton",label="In terms of principal axes",underline="12",variable=Points.MDS.InTermsOfPrincipalAxes.var)
tkadd(MenuBar.Points,"cascade",label="MDS",underline="0",menu=MenuBar.Points.MDS)
...
tkadd(MenuBar.Axes,"radiobutton",label="Regression",underline="0",variable=Biplot.Axes.var,value="11",accelerator="4",command=function()
  {
  GUI.BindingsOff()
  Axes.Regression.cmd()
  GUI.BindingsOn()
  })
tkadd(MenuBar.Axes,"radiobutton",label="Procrustes",underline="0",variable=Biplot.Axes.var,value="12",accelerator="5",command=function()
  {
  GUI.BindingsOff()
  Axes.Procrustes.cmd()
  GUI.BindingsOn()
...
tkadd(MenuBar.Axes,"separator")
tkadd(MenuBar.Axes,"radiobutton",label="Circular non-linear",underline="0",variable=Biplot.Axes.var,value="13",accelerator="6",command=function()
  {
  GUI.BindingsOff()
...
tkadd(MenuBar.Axes,"separator")
tkadd(MenuBar.Axes,"command",label="Default",underline="0",command=function()
  {
  GUI.BindingsOff()
  Axes.Default.cmd()
  GUI.BindingsOn()
...
tkadd(MenuBar.Additional.Interpolate,"checkbutton",label="A new sample...",underline="2",accelerator="Ctrl+N",variable=Additional.Interpolate.ANewSample.var,command=function()
  {
  GUI.BindingsOff()
...
tkadd(MenuBar.Additional.Interpolate,"checkbutton",label="Sample group means...",underline="7",variable=Additional.Interpolate.SampleGroupMeans.var,command=function()
  {
  GUI.BindingsOff()
...
tkadd(MenuBar.Additional,"cascade",label="Interpolate",underline="0",menu=MenuBar.Additional.Interpolate)
tkadd(MenuBar.Additional,"separator")
tkadd(MenuBar.Additional,"checkbutton",label="Convex hulls...",underline="7",variable=Additional.ConvexHull.var,command=function()
  {
  GUI.BindingsOff()
...
tkadd(MenuBar.Additional,"separator")
tkadd(MenuBar.Additional,"checkbutton",label="Point densities...",underline="6",variable=Additional.PointDensities.var,command=function()
  {
  GUI.BindingsOff()
...
tkadd(MenuBar.Additional,"checkbutton",label="Classification regions...",underline="0",variable=Additional.ClassificationRegion.var,state=if (g==1) "disabled" else "normal",command=function()
  {
  GUI.BindingsOff()
...
tkadd(MenuBar.Additional,"separator")
tkadd(MenuBar.Additional,"command",label="Clear all",underline="1",accelerator="Ctrl+L",command=function()
  {
  GUI.BindingsOff()
...
tkadd(MenuBar.Help,"separator")
tkadd(MenuBar.Help,"command",label="About...",underline="0",command=function()
  {
  GUI.BindingsOff()
  Help.About.cmd()
  GUI.BindingsOn()
  })
tkadd(MenuBar.menu,"cascade",label="Help",underline="0",menu=MenuBar.Help)
...
BiplotRegion.HorizontalScale.func<-function() as.numeric(tkwinfo("width",BiplotRegion.frame))/as.numeric(tkwinfo("fpixels",BiplotRegion.frame,"1i"))/4
BiplotRegion.VerticalScale.func<-function() as.numeric(tkwinfo("height",BiplotRegion.frame))/as.numeric(tkwinfo("fpixels",BiplotRegion.frame,"1i"))/4
...
Biplot.ConvertCoordinates<-function(xin,yin)
  ...
  plotregionstartxprop<-figwidthprop*Biplot.par$plt[1]+Biplot.par$fig[1]
  plotregionendxprop<-figwidthprop*Biplot.par$plt[2]+Biplot.par$fig[1]
  plotregionstartyprop<-figheightprop*Biplot.par$plt[3]+Biplot.par$fig[3]
  plotregionendyprop<-figheightprop*Biplot.par$plt[4]+Biplot.par$fig[3]
    ...
    if (tclvalue(Biplot.points.mode)%in%c("1","2"))
      ...
      if (tclvalue(Biplot.points.mode)=="1")
        Biplot.points.WhereHighlight<<-Biplot.XY.move
          ...
          {
          Biplot.points.WhichHighlight<<-which.min(PythagorasDistance(matrix(Biplot.XY.move,nrow=1),Y))
      ...
      if (Biplot.WasInside)
        {
        tkconfigure(GUI.TopLevel,cursor="arrow")
        if (tclvalue(Biplot.points.mode)%in%c("1","2")) Biplot.replot()
        Biplot.WasInside<<-FALSE
        }
      }
  }
...
Legend.func<-function()
  ...
  if (tclvalue(View.AxisLabels.var)=="2")
    ...
    Legend.text.col<<-c(Legend.text.col,rep("black",p.in))
    Legend.lty<<-c(Legend.lty,bpar$axes.lty[variables.in])
    Legend.lwd<<-c(Legend.lwd,bpar$axes.lwd[variables.in])
    if (Biplot.axes.mode==0) Legend.lines.col<<-c(Legend.lines.col,bpar$axes.col[variables.in])
      else
        {
        temp1<-rep(bpar$interaction.highlight.axes.col.bg,p.in)
    ...
    Legend.points.col<<-c(Legend.points.col,rep(NA,p.in))
    Legend.pt.bg<<-c(Legend.pt.bg,rep(NA,p.in))
    }

  if (tclvalue(View.ShowAdditionalLabelsInLegend.var)=="1")
    {
    if (tclvalue(Additional.Interpolate.ANewSample.var)=="1" && !bpar$ANewSample.LabelsInBiplot)
      {
    ...
    if (tclvalue(Additional.Interpolate.SampleGroupMeans.var)=="1" && !bpar$SampleGroupMeans.LabelsInBiplot)
      {
      switch(as.character(Additional.Interpolate.SampleGroupMeans.for),
        "-1"=
          {
          ...
          Legend.points.col<<-c(Legend.points.col,"black")
          Legend.pt.bg<<-c(Legend.pt.bg,"black")
          },
        "0"=
          {
          Legend.legend<<-c(Legend.legend,paste("SGM:",bpar$groups.label.text[groups.in]))
          Legend.text.col<<-c(Legend.text.col,rep("black",g.in))
          Legend.lty<<-c(Legend.lty,rep(1,g.in))
          Legend.lwd<<-c(Legend.lwd,rep(1,g.in))
          Legend.lines.col<<-c(Legend.lines.col,rep(NA,g.in))
          ...
          Legend.points.col<<-c(Legend.points.col,bpar$gSampleGroupMeans.col.fg[groups.in])
          Legend.pt.bg<<-c(Legend.pt.bg,bpar$gSampleGroupMeans.col.bg[groups.in])
          },
        {
        Legend.legend<<-c(Legend.legend,paste("SGM:",bpar$groups.label.text[Additional.Interpolate.SampleGroupMeans.for]))
        Legend.text.col<<-c(Legend.text.col,"black")
        Legend.lty<<-c(Legend.lty,1)
        Legend.lwd<<-c(Legend.lwd,1)
        Legend.lines.col<<-c(Legend.lines.col,NA)
        ...
        Legend.points.col<<-c(Legend.points.col,bpar$gSampleGroupMeans.col.fg[Additional.Interpolate.SampleGroupMeans.for])
        Legend.pt.bg<<-c(Legend.pt.bg,bpar$gSampleGroupMeans.col.bg[Additional.Interpolate.SampleGroupMeans.for])
        })
      }

    if (tclvalue(Additional.ConvexHull.var)=="1" || tclvalue(Additional.AlphaBag.var)=="1")
      {
      switch(as.character(Additional.ConvexHullAlphaBag.for),
        "-1"=
          {
          if (tclvalue(Additional.ConvexHull.var)=="1") Legend.legend<<-c(Legend.legend,"CH: All points")
          ...
          Legend.lines.col<<-c(Legend.lines.col,if (g==1) bpar$gConvexHullAlphaBag.col.fg else hcl(0,0,60))
          Legend.pch<<-c(Legend.pch,NA)
          Legend.pt.cex<<-c(Legend.pt.cex,NA)
          Legend.points.col<<-c(Legend.points.col,NA)
          Legend.pt.bg<<-c(Legend.pt.bg,NA)
          },
        "0"=
          {
          if (tclvalue(Additional.ConvexHull.var)=="1") Legend.legend<<-c(Legend.legend,paste("CH:",bpar$groups.label.text[groups.in]))
            else Legend.legend<<-c(Legend.legend,paste("AB:",bpar$groups.label.text[groups.in]))
          Legend.text.col<<-c(Legend.text.col,rep("black",g.in))
          ...
          Legend.lines.col<<-c(Legend.lines.col,bpar$gConvexHullAlphaBag.col.fg[groups.in])
          Legend.pch<<-c(Legend.pch,rep(NA,g.in))
          Legend.pt.cex<<-c(Legend.pt.cex,rep(NA,g.in))
          Legend.points.col<<-c(Legend.points.col,rep(NA,g.in))
          Legend.pt.bg<<-c(Legend.pt.bg,rep(NA,g.in))
          },
        {
        if (tclvalue(Additional.ConvexHull.var)=="1") Legend.legend<<-c(Legend.legend,paste("CH:",bpar$groups.label.text[Additional.ConvexHullAlphaBag.for]))
          else Legend.legend<<-c(Legend.legend,paste("AB:",bpar$groups.label.text[Additional.ConvexHullAlphaBag.for]))
        Legend.text.col<<-c(Legend.text.col,"black")
        ...
        Legend.lines.col<<-c(Legend.lines.col,bpar$gConvexHullAlphaBag.col.fg[Additional.ConvexHullAlphaBag.for])
        Legend.pch<<-c(Legend.pch,NA)
        Legend.pt.cex<<-c(Legend.pt.cex,NA)
        Legend.points.col<<-c(Legend.points.col,NA)
        Legend.pt.bg<<-c(Legend.pt.bg,NA)
    ...
    if (tclvalue(Additional.AlphaBag.var)=="1" && Additional.ConvexHullAlphaBag.ShowTukeyMedian && !bpar$ConvexHullAlphaBag.TukeyMedian.LabelsInBiplot)
      {
      switch(as.character(Additional.ConvexHullAlphaBag.for),
        "-1"=
          {
          ...
          Legend.pt.cex<<-c(Legend.pt.cex,2)
          Legend.points.col<<-c(Legend.points.col,if (g==1) bpar$gConvexHullAlphaBag.col.fg else hcl(0,0,60))
          Legend.pt.bg<<-c(Legend.pt.bg,NA)
          },
        "0"=
          {
          Legend.legend<<-c(Legend.legend,paste("TM:",bpar$groups.label.text[groups.in]))
          Legend.text.col<<-c(Legend.text.col,rep("black",g.in))
          Legend.lty<<-c(Legend.lty,rep(1,g.in))
          Legend.lwd<<-c(Legend.lwd,rep(1,g.in))
          Legend.lines.col<<-c(Legend.lines.col,rep(NA,g.in))
          ...
          Legend.points.col<<-c(Legend.points.col,bpar$gConvexHullAlphaBag.col.fg[groups.in])
          Legend.pt.bg<<-c(Legend.pt.bg,rep(NA,g.in))
          },
        {
        Legend.legend<<-c(Legend.legend,paste("TM:",bpar$groups.label.text[Additional.ConvexHullAlphaBag.for]))
        Legend.text.col<<-c(Legend.text.col,"black")
        Legend.lty<<-c(Legend.lty,1)
        Legend.lwd<<-c(Legend.lwd,1)
        Legend.lines.col<<-c(Legend.lines.col,NA)
        Legend.pch<<-c(Legend.pch,0)
        Legend.pt.cex<<-c(Legend.pt.cex,2)
        Legend.points.col<<-c(Legend.points.col,bpar$gConvexHullAlphaBag.col.fg[Additional.ConvexHullAlphaBag.for])
        Legend.pt.bg<<-c(Legend.pt.bg,NA)
        })
      }
    ...
    if (tclvalue(Additional.ClassificationRegion.var)=="1")
      {
      Legend.legend<<-c(Legend.legend,paste("CR:",bpar$groups.label.text[groups.in]))
      Legend.text.col<<-c(Legend.text.col,rep("black",g.in))
      Legend.lty<<-c(Legend.lty,rep(1,g.in))
      Legend.lwd<<-c(Legend.lwd,rep(1,g.in))
      Legend.lines.col<<-c(Legend.lines.col,rep(NA,g.in))
  ...
  Legend.LastPage<<-(length(Legend.legend)-1)%/%16+1
  if (Legend.CurrentPage>Legend.LastPage) Legend.CurrentPage<<-Legend.LastPage
  if (Legend.CurrentPage<Legend.LastPage)
    {
    tkentryconfigure(MenuBar.View,16,state="normal")
    tkentryconfigure(Biplot.RightClickOutside.Menu,11,state="normal")
    tkentryconfigure(Biplot.None.RightClickOutside.Menu,6,state="normal")
    }
    else
      {
      tkentryconfigure(MenuBar.View,16,state="disabled")
      tkentryconfigure(Biplot.RightClickOutside.Menu,11,state="disabled")
      tkentryconfigure(Biplot.None.RightClickOutside.Menu,6,state="disabled")
      }
  if (Legend.CurrentPage>1)
    {
    tkentryconfigure(MenuBar.View,17,state="normal")
    tkentryconfigure(Biplot.RightClickOutside.Menu,12,state="normal")
    tkentryconfigure(Biplot.None.RightClickOutside.Menu,7,state="normal")
    }
    else
      {
      tkentryconfigure(MenuBar.View,17,state="disabled")
      tkentryconfigure(Biplot.RightClickOutside.Menu,12,state="disabled")
      tkentryconfigure(Biplot.None.RightClickOutside.Menu,7,state="disabled")
      }
  Legend.CurrentIndices<<-((Legend.CurrentPage-1)*16+1):(min(Legend.CurrentPage*16,length(Legend.legend)))

  legend2(x="center",legend=Legend.transpose(Legend.legend[Legend.CurrentIndices]),cex=.85,text.width=strwidth("12345678901234",cex=.85),text.col=Legend.transpose(Legend.text.col[Legend.CurrentIndices]),ncol=4,lty=Legend.transpose(Legend.lty[Legend.CurrentIndices]),lwd=Legend.transpose(Legend.lwd[Legend.CurrentIndices]),lines.col=Legend.transpose(Legend.lines.col[Legend.CurrentIndices]),pch=Legend.transpose(Legend.pch[Legend.CurrentIndices]),pt.cex=Legend.transpose(Legend.pt.cex[Legend.CurrentIndices]),col=Legend.transpose(Legend.points.col[Legend.CurrentIndices]),pt.bg=Legend.transpose(Legend.pt.bg[Legend.CurrentIndices]))
  }
...
tkadd(Biplot.RightClickInside.Menu,"command",label="Zoom out",command=function()
  {
  GUI.BindingsOff()
  Biplot.ZoomOut.cmd()
  GUI.BindingsOn()
  })
tkadd(Biplot.RightClickInside.Menu,"command",label="Reset zoom",command=function()
  {
  GUI.BindingsOff()
...
tkadd(Biplot.RightClickInside.Menu,"radiobutton",label="Don't predict",variable=Biplot.points.mode,value="0",command=function()
  {
  GUI.BindingsOff()
  Biplot.DontPredict.cmd()
  GUI.BindingsOn()
  })
tkadd(Biplot.RightClickInside.Menu,"radiobutton",label="Predict cursor positions",variable=Biplot.points.mode,value="1",command=function()
  {
  GUI.BindingsOff()
  Biplot.PredictCursorPositions.cmd()
  GUI.BindingsOn()
  })
tkadd(Biplot.RightClickInside.Menu,"radiobutton",label="Predict points closest to cursor positions",variable=Biplot.points.mode,value="2",command=function()
  {
  GUI.BindingsOff()
...
tkadd(Biplot.RightClickInside.Menu,"separator")
tkadd(Biplot.RightClickInside.Menu,"command",label="Remove axis highlight",state="disabled",command=function()
  {
  GUI.BindingsOff()
...
tkadd(Biplot.RightClickInside.Menu,"separator")
tkadd(Biplot.RightClickInside.Menu,"command",label="Format by group...",command=function()
  {
  GUI.BindingsOff()
  Format.ByGroup.cmd()
  GUI.BindingsOn()
  })
tkadd(Biplot.RightClickInside.Menu,"command",label="Format axes...",command=function()
  {
  GUI.BindingsOff()
  Format.Axes.cmd()
  GUI.BindingsOn()
...
tkadd(Biplot.RightClickInside.Menu,"separator")
tkadd(Biplot.RightClickInside.Menu,"cascade",label="Save as",menu=MenuBar.File.SaveAs)
tkadd(Biplot.RightClickInside.Menu,"command",label="Copy",state=if (.Platform$OS.type != "windows") "disabled" else "normal",command=function()
  {
  GUI.BindingsOff()
  File.Copy.cmd()
  GUI.BindingsOn()
...
tkadd(Biplot.RightClickInside.Menu,"separator")
tkadd(Biplot.RightClickInside.Menu,"command",label="Print...",state=if (.Platform$OS.type != "windows") "disabled" else "normal",command=function()
  {
  GUI.BindingsOff()
  File.Print.cmd()
  GUI.BindingsOn()
...
Biplot.None.RightClickInside.Menu<-tk2menu(GUI.TopLevel,tearoff=FALSE)
tkadd(Biplot.None.RightClickInside.Menu,"command",label="Zoom in",command=function()
  {
  GUI.BindingsOff()
  Biplot.ZoomIn.cmd()
  GUI.BindingsOn()
  })
tkadd(Biplot.None.RightClickInside.Menu,"command",label="Zoom out",command=function()
  {
  GUI.BindingsOff()
  Biplot.ZoomOut.cmd()
  GUI.BindingsOn()
  })
tkadd(Biplot.None.RightClickInside.Menu,"command",label="Reset zoom",command=function()
  {
  GUI.BindingsOff()
  Biplot.ResetZoom.cmd()
  GUI.BindingsOn()
...
tkadd(Biplot.None.RightClickInside.Menu,"command",label="Format by group...",command=function()
  {
  GUI.BindingsOff()
  Format.ByGroup.cmd()
  GUI.BindingsOn()
...
tkadd(Biplot.None.RightClickInside.Menu,"separator")
tkadd(Biplot.None.RightClickInside.Menu,"cascade",label="Save as",menu=MenuBar.File.SaveAs)
tkadd(Biplot.None.RightClickInside.Menu,"command",label="Copy",state=if (.Platform$OS.type != "windows") "disabled" else "normal",command=function()
  {
  GUI.BindingsOff()
  File.Copy.cmd()
  GUI.BindingsOn()
...
tkadd(Biplot.None.RightClickInside.Menu,"separator")
tkadd(Biplot.None.RightClickInside.Menu,"command",label="Print...",state=if (.Platform$OS.type != "windows") "disabled" else "normal",command=function()
  {
  GUI.BindingsOff()
  File.Print.cmd()
  GUI.BindingsOn()
...
tkadd(Biplot.RightClickOnPoint.Menu,"command",label="Format...",command=function()
  {
  GUI.BindingsOff()
  if (as.numeric(tclvalue(Biplot.Axes.var))<10) Y<-Biplot.Y_
    else Y<-Biplot.Y
  if (g>1)
    {
    temp1<-samples.in[which.min(PythagorasDistance(matrix(Biplot.XY.RightClick,nrow=1),Y))]
...
tkadd(Biplot.RightClickOnAxis.Menu,"command",label="Send to kraal",command=function()
  {
  GUI.BindingsOff()
  Biplot.SendAxisToKraal.cmd()
  GUI.BindingsOn()
...
tkadd(Biplot.RightClickOnAxis.Menu,"separator")
tkadd(Biplot.RightClickOnAxis.Menu,"command",label="Format...",command=function()
  {
  GUI.BindingsOff()
  ...
  if (as.numeric(tclvalue(Biplot.Axes.var))<13)
    ...
    temp1<-variables.in[which.min(PythagorasDistance(matrix(Biplot.XY.RightClick,nrow=1),t(apply(B,1,function(x) x%*%t(x)%*%Biplot.XY.RightClick/sum(x^2)))))]
    }
    else
      {
      temp1<-variables.in[which.min(unlist(lapply(Biplot.axis,function(mat)
        {
        temp1<-nrow(mat)
        temp2<-sweep(-mat[-temp1,],2,Biplot.XY.RightClick,"+") # m-a
        temp3<-diff(mat) #b-a
...
tkadd(Biplot.RightClickOutside.Menu,"command",label="Format title...",command=function()
  {
  GUI.BindingsOff()
  Format.Title.cmd()
  GUI.BindingsOn()
...
tkadd(Biplot.RightClickOutside.Menu,"checkbutton",label="Show group labels in legend",variable=View.ShowGroupLabelsInLegend.var,state=if (g==1) "disabled" else "normal",command=function()
  {
  GUI.BindingsOff()
  View.ShowGroupLabelsInLegend.cmd()
  GUI.BindingsOn()
...
tkadd(Biplot.RightClickOutside.Menu,"separator")
tkadd(Biplot.RightClickOutside.Menu,"radiobutton",label="Don't show axis labels",variable=View.AxisLabels.var,value="0",command=function()
  {
  GUI.BindingsOff()
  View.DontShowAxisLabels.cmd()
  GUI.BindingsOn()
  })
tkadd(Biplot.RightClickOutside.Menu,"radiobutton",label="Show clinging axis labels",variable=View.AxisLabels.var,value="1",command=function()
  {
  GUI.BindingsOff()
  View.ShowClingingAxisLabels.cmd()
  GUI.BindingsOn()
  })
tkadd(Biplot.RightClickOutside.Menu,"radiobutton",label="Show axis labels in legend",variable=View.AxisLabels.var,value="2",command=function()
  {
  GUI.BindingsOff()
  View.ShowAxisLabelsInLegend.cmd()
  GUI.BindingsOn()
...
tkadd(Biplot.RightClickOutside.Menu,"separator")
tkadd(Biplot.RightClickOutside.Menu,"checkbutton",label="Show Additional labels in legend",variable=View.ShowAdditionalLabelsInLegend.var,command=function()
  {
  GUI.BindingsOff()
  View.ShowAdditionalLabelsInLegend.cmd()
  GUI.BindingsOn()
...
tkadd(Biplot.RightClickOutside.Menu,"separator")
tkadd(Biplot.RightClickOutside.Menu,"cascade",label="Save as",menu=MenuBar.File.SaveAs)
tkadd(Biplot.RightClickOutside.Menu,"command",label="Copy",state=if (.Platform$OS.type != "windows") "disabled" else "normal",command=function()
  {
  GUI.BindingsOff()
  File.Copy.cmd()
  GUI.BindingsOn()
...
tkadd(Biplot.RightClickOutside.Menu,"separator")
tkadd(Biplot.RightClickOutside.Menu,"command",label="Print...",state=if (.Platform$OS.type != "windows") "disabled" else "normal",command=function()
  {
  GUI.BindingsOff()
  File.Print.cmd()
  GUI.BindingsOn()
...
Biplot.None.RightClickOutside.Menu<-tk2menu(BiplotRegion.image,tearoff=FALSE)
tkadd(Biplot.None.RightClickOutside.Menu,"checkbutton",label="Show title",variable=View.ShowTitle.var,command=function()
  {
  GUI.BindingsOff()
  View.ShowTitle.cmd()
  GUI.BindingsOn()
  })
tkadd(Biplot.None.RightClickOutside.Menu,"command",label="Format title...",command=function()
  {
  GUI.BindingsOff()
  Format.Title.cmd()
  GUI.BindingsOn()
...
tkadd(Biplot.None.RightClickOutside.Menu,"checkbutton",label="Show group labels in legend",variable=View.ShowGroupLabelsInLegend.var,state=if (g==1) "disabled" else "normal",command=function()
  {
  GUI.BindingsOff()
  View.ShowGroupLabelsInLegend.cmd()
  GUI.BindingsOn()
  })
tkadd(Biplot.None.RightClickOutside.Menu,"checkbutton",label="Show Additional labels in legend",variable=View.ShowAdditionalLabelsInLegend.var,command=function()
  {
  GUI.BindingsOff()
  View.ShowAdditionalLabelsInLegend.cmd()
  GUI.BindingsOn()
...
tkadd(Biplot.None.RightClickOutside.Menu,"separator")
tkadd(Biplot.None.RightClickOutside.Menu,"cascade",label="Save as",menu=MenuBar.File.SaveAs)
tkadd(Biplot.None.RightClickOutside.Menu,"command",label="Copy",state=if (.Platform$OS.type != "windows") "disabled" else "normal",command=function()
  {
  GUI.BindingsOff()
  File.Copy.cmd()
  GUI.BindingsOn()
...
tkadd(Biplot.None.RightClickOutside.Menu,"separator")
tkadd(Biplot.None.RightClickOutside.Menu,"command",label="Print...",state=if (.Platform$OS.type != "windows") "disabled" else "normal",command=function()
  {
  GUI.BindingsOff()
  File.Print.cmd()
  GUI.BindingsOn()
...
SettingsBox.action.cmd<-function(FollowThrough=TRUE)
  ...
  if (tclvalue(tkget(SettingsBox.action.combo))!="Predict") Biplot.points.mode<<-tclVar("0")
  switch(tclvalue(Biplot.Axes.var),
    "1"=Joint.CovarianceCorrelation.determine(),
    "2"=Joint.CVA.determine(),
    "12"=Axes.Procrustes.determine(),
...
SettingsBox.transformation.cmd<-function(FollowThrough=TRUE)
  ...
  SettingsBox.transformation.func<<-switch(temp1,
    "Centre"=function(IN,ARow=NA,WhichCol=NA)
      {
      temp1<-colMeans(Data[samples.in,variables.in])
      if (!is.na(ARow)) IN-temp1
        else if (!is.na(WhichCol)) IN-temp1[WhichCol]
          else stop("Specify either `ARow' or `WhichCol' in SettingsBox.transformation.func")
      },
    "Centre, scale"=function(IN,ARow=NA,WhichCol=NA)
      {
      temp1<-colMeans(Data[samples.in,variables.in])
      temp2<-apply(Data[samples.in,variables.in],2,sd)
      if (!is.na(ARow)) (IN-temp1)/temp2
        else if (!is.na(WhichCol)) (IN-temp1[WhichCol])/temp2[WhichCol]
          else stop("Specify either `ARow' or `WhichCol' in SettingsBox.transformation.func")
      },
    "Unitise, centre"=function(IN,ARow=NA,WhichCol=NA)
      {
      temp1<-apply(Data[samples.in,variables.in],2,min)
      temp2<-apply(Data[samples.in,variables.in],2,function(x) max(x)-min(x))
      temp3<-colMeans(apply(Data[samples.in,variables.in],2,function(x) (x-min(x))/(max(x)-min(x))))
      if (!is.na(ARow)) (IN-temp1)/temp2-temp3
        else if (!is.na(WhichCol)) (IN-temp1[WhichCol])/temp2[WhichCol]-temp3[WhichCol]
          else stop("Specify either `ARow' or `WhichCol' in SettingsBox.transformation.func")
      },
    "Log, centre"=function(IN,ARow=NA,WhichCol=NA)
      {
      temp1<-colMeans(log(Data[samples.in,variables.in]))
      if (!is.na(ARow)) log(IN)-temp1
        else if (!is.na(WhichCol)) log(IN)-temp1[WhichCol]
          else stop("Specify either `ARow' or `WhichCol' in SettingsBox.transformation.func")
      },
    "Log, centre, scale"=function(IN,ARow=NA,WhichCol=NA)
      {
      temp1<-colMeans(log(Data[samples.in,variables.in]))
      temp2<-apply(log(Data[samples.in,variables.in]),2,sd)
      if (!is.na(ARow)) (log(IN)-temp1)/temp2
        else if (!is.na(WhichCol)) (log(IN)-temp1[WhichCol])/temp2[WhichCol]
          else stop("Specify either `ARow' or `WhichCol' in SettingsBox.transformation.func")
      },
    "Log, unitise, centre"=function(IN,ARow=NA,WhichCol=NA)
      {
      temp1<-apply(log(Data[samples.in,variables.in]),2,min)
      temp2<-apply(log(Data[samples.in,variables.in]),2,function(x) max(x)-min(x))
      temp3<-colMeans(apply(log(Data[samples.in,variables.in]),2,function(x) (x-min(x))/(max(x)-min(x))))
      if (!is.na(ARow)) (log(IN)-temp1)/temp2-temp3
        else if (!is.na(WhichCol)) (log(IN)-temp1[WhichCol])/temp2[WhichCol]-temp3[WhichCol]
          else stop("Specify either `ARow' or `WhichCol' in SettingsBox.transformation.func")
      })
  ...
  SettingsBox.BackTransformation.func<<-switch(temp1,
    ...
    "Centre, scale"=function(IN,ARow=NA,WhichCol=NA)
      ...
      temp1<-colMeans(Data[samples.in,variables.in])
      temp2<-apply(Data[samples.in,variables.in],2,sd)
      if (!is.na(ARow)) IN*temp2+temp1
        else if (!is.na(WhichCol)) IN*temp2[WhichCol]+temp1[WhichCol]
          else stop("Specify either `ARow' or `WhichCol' in SettingsBox.BackTransformation.func")
      },
    "Unitise, centre"=function(IN,ARow=NA,WhichCol=NA)
      {
      ...
      temp2<-apply(Data[samples.in,variables.in],2,function(x) max(x)-min(x))
      temp3<-colMeans(apply(Data[samples.in,variables.in],2,function(x) (x-min(x))/(max(x)-min(x))))
      if (!is.na(ARow)) (IN+temp3)*temp2+temp1
        else if (!is.na(WhichCol)) (IN+temp3[WhichCol])*temp2[WhichCol]+temp1[WhichCol]
          else stop("Specify either `ARow' or `WhichCol' in SettingsBox.BackTransformation.func")
      },
    "Log, centre"=function(IN,ARow=NA,WhichCol=NA)
      {
      temp1<-colMeans(log(Data[samples.in,variables.in]))
      if (!is.na(ARow)) exp(IN+temp1)
        else if (!is.na(WhichCol)) exp(IN+temp1[WhichCol])
          else stop("Specify either `ARow' or `WhichCol' in SettingsBox.BackTransformation.func")
      },
    "Log, centre, scale"=function(IN,ARow=NA,WhichCol=NA)
      {
      temp1<-colMeans(log(Data[samples.in,variables.in]))
      temp2<-apply(log(Data[samples.in,variables.in]),2,sd)
      if (!is.na(ARow)) exp(IN*temp2+temp1)
        else if (!is.na(WhichCol)) exp(IN*temp2[WhichCol]+temp1[WhichCol])
          else stop("Specify either `ARow' or `WhichCol' in SettingsBox.BackTransformation.func")
      },
    "Log, unitise, centre"=function(IN,ARow=NA,WhichCol=NA)
      {
      ...
      temp2<-apply(log(Data[samples.in,variables.in]),2,function(x) max(x)-min(x))
      temp3<-colMeans(apply(log(Data[samples.in,variables.in]),2,function(x) (x-min(x))/(max(x)-min(x))))
      if (!is.na(ARow)) exp((IN+temp3)*temp2+temp1)
        else if (!is.na(WhichCol)) exp((IN+temp3[WhichCol])*temp2[WhichCol]+temp1[WhichCol])
          else stop("Specify either `ARow' or `WhichCol' in SettingsBox.BackTransformation.func")
      })
  ...
  Biplot.Xtransformed<<-switch(temp1,
    "Centre"=scale(Data[samples.in,variables.in],scale=FALSE),
    "Centre, scale"=scale(Data[samples.in,variables.in]),
    "Unitise, centre"=scale(apply(Data[samples.in,variables.in],2,function(x) (x-min(x))/(max(x)-min(x))),scale=FALSE),
    "Log, centre"=scale(log(Data[samples.in,variables.in]),scale=FALSE),
    "Log, centre, scale"=scale(log(Data[samples.in,variables.in])),
    "Log, unitise, centre"=scale(apply(log(Data[samples.in,variables.in]),2,function(x) (x-min(x))/(max(x)-min(x))),scale=FALSE))
...
SettingsBox.transformation.FollowThrough.cmd<-function()
  ...
  if (as.numeric(tclvalue(Biplot.Axes.var))<10)
    ...
    switch(tclvalue(Biplot.Axes.var),
      "0"=Joint.PCA.cmd(),
      "1"=Joint.CovarianceCorrelation.cmd(),
      "2"=Joint.CVA.cmd())
    }
      else
        switch(tclvalue(Points.DistanceMetric.var),
          "0"=Points.DistanceMetric.Pythagoras.cmd(),
          "1"=Points.DistanceMetric.SquareRootOfManhattan.cmd(),
          "2"=Points.DistanceMetric.Clark.cmd(),
...
DiagnosticTabs.switch<-function()
  {
  switch(DiagnosticTabs.which,
    "1"=ConvergenceTab.plot(screen=FALSE),
    "2"=if (tclvalue(Biplot.Axes.var)%in%c("0","2")) PointsTab.plot.predictivities(screen=FALSE)
          else if (tclvalue(Biplot.Axes.var)!="1") PointsTab.plot.ShepardDiagram(screen=FALSE),
...
ConvergenceTab.plot<-function(screen=TRUE)
  ...
  if (!screen && Legend.yes())
    {
    layout(mat=matrix(c(2,2,1,1),ncol=2,byrow=TRUE),heights=c(8-1.1,1.1))
    par(mar=c(0,0,0,0),bg="white")
    plot(.5,.5,bty="n",type="n",xaxt="n",yaxt="n",xlab="",ylab="",xaxs="i",yaxs="i",xlim=c(0,1),ylim=c(0,1))
    par(mar=c(1,4,4,4))
    }
    else if (!screen) par(mar=c(2,4,2,4))
      else par(mar=c(2.5,3,2.5,2))

  par("bg"="white",pty="s")
  if (!is.null(ConvergenceTab.points.StressVector)) plot(ConvergenceTab.points.StressVector,type="s",main="",xlab="",ylab="",cex.axis=.75,lty=bpar$DiagnosticTabs.convergence.lty,lwd=bpar$DiagnosticTabs.convergence.lwd,col=bpar$DiagnosticTabs.convergence.col)
    else plot(0,0,type="n",main="",xlab="",ylab="",cex.axis=.75,lty=bpar$DiagnosticTabs.convergence.lty,lwd=bpar$DiagnosticTabs.convergence.lwd,col=bpar$DiagnosticTabs.convergence.col)
  if (tclvalue(ConvergenceTab.ShowTitle.var)=="1")
    if (!screen) title(main="Stress by iteration",line=1.75)
      else title(main="Stress by iteration",cex.main=.85)
 }

ConvergenceTab.replot<-function()
  {
  tkrreplot(ConvergenceTab.image,ConvergenceTab.plot,hscale=ConvergenceTab.HorizontalScale.func(),vscale=ConvergenceTab.VerticalScale.func())
  ConvergenceTab.update<<-FALSE
  }
...
PointsTab.plot.predictivities<-function(screen=TRUE)
  ...
  par("bg"="white",pty="s")
  leaveout<-sort(unique(c(which(is.na(PointsTab.predictivities1dim)),which(is.na(PointsTab.predictivities))))) # To take care of the case of a group with only one sample.
  if (length(leaveout)>0) leavein<-(1:length(PointsTab.predictivities))[-leaveout] else leavein<-1:length(PointsTab.predictivities) # To take care of the case of a group with only one sample.
  plot(PointsTab.predictivities1dim[leavein],PointsTab.predictivities[leavein],xaxt="n",yaxt="n",type="n",xlab="",ylab="",cex.lab=.85,cex.axis=.85,xlim=c(0,1),ylim=c(0,1) )
  if (tclvalue(PointsTab.ShowPointLabels.var)=="1") xylim<-getLabelUsr(c(PointsTab.predictivities1dim[leavein],0,1),c(PointsTab.predictivities[leavein],0,1),par(),c(bparp$points.label.text[samples.in][leavein],NA,NA),bpar$DiagnosticTabs.predictivities.label.cex,bpar$DiagnosticTabs.predictivities.label.HorizOffset,bpar$DiagnosticTabs.predictivities.label.VertOffset)
  ...
  if (tclvalue(PointsTab.ShowTitle.var)=="1")
    if (!screen) title(main="Point predictivities",line=1.75)
      else title(main="Point predictivities",cex.main=.85)
  axis(side=1,at=tickatx,cex.axis=.75)
  axis(side=2,at=tickaty,cex.axis=.75)
  lines(x=c(0,1),y=c(0,1),col="orange")
  if (tclvalue(PointsTab.ShowPointLabels.var)=="1") text(x=PointsTab.predictivities1dim[leavein]+bpar$DiagnosticTabs.predictivities.label.HorizOffset*strwidth("x",cex=bpar$DiagnosticTabs.predictivities.label.cex),y=PointsTab.predictivities[leavein]+bpar$DiagnosticTabs.predictivities.label.VertOffset*strheight("x",cex=bpar$DiagnosticTabs.predictivities.label.cex),labels=bparp$points.label.text[samples.in][leavein],font=bpar$DiagnosticTabs.predictivities.label.font,cex=bpar$DiagnosticTabs.predictivities.label.cex,col=bparp$points.label.col[samples.in][leavein])
  points(PointsTab.predictivities1dim[leavein],PointsTab.predictivities[leavein],col=bparp$points.col.fg[samples.in][leavein],bg=bparp$points.col.bg[samples.in][leavein],pch=bparp$points.pch[samples.in][leavein],cex=bpar$DiagnosticTabs.predictivities.cex)
  }
...
PointsTab.plot.ShepardDiagram<-function(screen=TRUE)
  ...
  if (n.in<=250)
    ...
    plot(dissordered,dist,main="",xlab="",ylab="",xaxt="n",yaxt="n",type="p",cex.lab=.85,cex.axis=.85,pch=bpar$DiagnosticTabs.ShepardDiagram.pch,cex=bpar$DiagnosticTabs.ShepardDiagram.cex,col=bpar$DiagnosticTabs.ShepardDiagram.col.fg,bg=bpar$DiagnosticTabs.ShepardDiagram.col.bg)
    axis(side=1,cex.axis=.75)
    axis(side=2,cex.axis=.75)
    if (tclvalue(PointsTab.ShowTitle.var)=="1")
      if (!screen) title(main="Shepard diagram",line=1.75)
        else title(main="Shepard diagram",cex.main=.85)
    if (tclvalue(Points.var)=="11") lines(dissordered,disp,type="s",lty=bpar$DiagnosticTabs.ShepardDiagram.disparities.lty,lwd=bpar$DiagnosticTabs.ShepardDiagram.disparities.lwd,col=bpar$DiagnosticTabs.ShepardDiagram.disparities.col.line)
      else lines(dissordered,disp,lty=bpar$DiagnosticTabs.ShepardDiagram.disparities.lty,lwd=bpar$DiagnosticTabs.ShepardDiagram.disparities.lwd,col=bpar$DiagnosticTabs.ShepardDiagram.disparities.col.line)
    points(dissordered,disp,pch=bpar$DiagnosticTabs.ShepardDiagram.disparities.pch,cex=bpar$DiagnosticTabs.ShepardDiagram.disparities.cex,col=bpar$DiagnosticTabs.ShepardDiagram.disparities.col.fg,bg=bpar$DiagnosticTabs.ShepardDiagram.disparities.col.bg)
    ...
    if (nLargest>0)
      ...
      temp3<-outer(bparp$points.label.text[samples.in],bparp$points.label.text[samples.in],function(x,y) paste(y,x,sep=", "))
      nLargestNames<-temp3[lower.tri(temp3)][order(diss)][temp1][order(temp2)]
      nLargestAbsDevs<-(abs(disp-dist))[temp1][order(temp2)]
      points(dissordered[temp1],dist[temp1],pch=22,col="blue",bg="lightblue",cex=1.5)
      text(dissordered[temp1],dist[temp1],labels=temp2,col="red",cex=.6)
      legend(x="topleft",legend=paste(nLargestRanks,". ",nLargestNames,sep=""),cex=.75,bty="n") #,inset=.02
      }
    }
    else
      {
...
PointsTab.replot<-function()
  {
  if (tclvalue(Biplot.Axes.var)%in%c("0","2")) PointsTab.predictivities.replot()
    else if (tclvalue(Biplot.Axes.var)!="1") PointsTab.ShepardDiagram.replot()
...
GroupsTab.plot.predictivities<-function(screen=TRUE)
  ...
  par("bg"="white",pty="s")
  leaveout<-sort(unique(c(which(is.na(GroupsTab.predictivities1dim)),which(is.na(GroupsTab.predictivities))))) # To take care of the case of a group with only one sample.
  if (length(leaveout)>0) leavein<-(1:length(GroupsTab.predictivities))[-leaveout] else leavein<-1:length(GroupsTab.predictivities) # To take care of the case of a group with only one sample.
  plot(GroupsTab.predictivities1dim[leavein],GroupsTab.predictivities[leavein],xaxt="n",yaxt="n",type="n",xlab="",ylab="",cex.lab=.85,cex.axis=.85,xlim=c(0,1),ylim=c(0,1) )
  if (tclvalue(GroupsTab.ShowGroupLabels.var)=="1") xylim<-getLabelUsr(c(GroupsTab.predictivities1dim[leavein],0,1),c(GroupsTab.predictivities[leavein],0,1),par(),c(bpar$groups.label.text[groups.in][leavein],NA,NA),bpar$DiagnosticTabs.predictivities.label.cex,bpar$DiagnosticTabs.predictivities.label.HorizOffset,bpar$DiagnosticTabs.predictivities.label.VertOffset)
    else xylim<-list(c(0,1),c(0,1))
  tickatx<-axTicks(side=1)
  tickaty<-axTicks(side=2)
  ...
  if (tclvalue(GroupsTab.ShowTitle.var)=="1")
    if (!screen) title(main="Group predictivities",line=1.75)
      else title(main="Group predictivities",cex.main=.85)
  axis(side=1,at=tickatx,cex.axis=.75)
  axis(side=2,at=tickaty,cex.axis=.75)
  lines(x=c(0,1),y=c(0,1),col="orange")
  if (tclvalue(GroupsTab.ShowGroupLabels.var)=="1") text(x=GroupsTab.predictivities1dim[leavein]+bpar$DiagnosticTabs.predictivities.label.HorizOffset*strwidth("x",cex=bpar$DiagnosticTabs.predictivities.label.cex),y=GroupsTab.predictivities[leavein]+bpar$DiagnosticTabs.predictivities.label.VertOffset*strheight("x",cex=bpar$DiagnosticTabs.predictivities.label.cex),labels=bpar$groups.label.text[groups.in][leavein],font=bpar$DiagnosticTabs.predictivities.label.font,cex=bpar$DiagnosticTabs.predictivities.label.cex,col=bpar$gpoints.label.col[groups.in][leavein])
  points(GroupsTab.predictivities1dim[leavein],GroupsTab.predictivities[leavein],col=bpar$gpoints.col.fg[groups.in][leavein],bg=bpar$gpoints.col.bg[groups.in][leavein],pch=bpar$gpoints.pch[groups.in][leavein],cex=bpar$DiagnosticTabs.predictivities.cex)
  }

GroupsTab.replot<-function()
  {
  if (tclvalue(Biplot.Axes.var)=="2") tkrreplot(GroupsTab.image,GroupsTab.plot.predictivities,hscale=ConvergenceTab.HorizontalScale.func(),vscale=ConvergenceTab.VerticalScale.func())
  GroupsTab.update<<-FALSE
  }
...
AxesTab.plot.predictivities<-function(screen=TRUE)
  ...
  if (tclvalue(AxesTab.ShowTitle.var)=="1")
    if (!screen) title(main="Axis predictivities",line=1.75)
      else title(main="Axis predictivities",cex.main=.85)
  axis(side=1,at=tickatx,cex.axis=.75)
  axis(side=2,at=tickaty,cex.axis=.75)
  lines(x=c(0,1),y=c(0,1),col="orange")
  if (Biplot.axes.mode==0)
    {
    if (tclvalue(AxesTab.ShowAxisLabels.var)=="1") text(x=AxesTab.predictivities1dim[leavein]+bpar$DiagnosticTabs.predictivities.label.HorizOffset*strwidth("x",cex=bpar$DiagnosticTabs.predictivities.label.cex),y=AxesTab.predictivities[leavein]+bpar$DiagnosticTabs.predictivities.label.VertOffset*strheight("x",cex=bpar$DiagnosticTabs.predictivities.label.cex),labels=bpar$axes.label.text[variables.in][leavein],font=bpar$DiagnosticTabs.predictivities.label.font,cex=bpar$DiagnosticTabs.predictivities.label.cex,col=bpar$axes.label.col[variables.in][leavein])
    points(AxesTab.predictivities1dim[leavein],AxesTab.predictivities[leavein],col=bpar$axes.col[variables.in][leavein],bg=bpar$axes.col[variables.in][leavein],pch=bpar$DiagnosticTabs.predictivities.axes.pch,cex=bpar$DiagnosticTabs.predictivities.cex)
    }
    else
      {
    temp1<-which(variables.in[leavein]!=Biplot.axes.WhichHighlight)
    temp2<-which(variables.in[leavein]==Biplot.axes.WhichHighlight)
    if (tclvalue(AxesTab.ShowAxisLabels.var)=="1") text(x=AxesTab.predictivities1dim[leavein][temp1]+bpar$DiagnosticTabs.predictivities.label.HorizOffset*strwidth("x",cex=bpar$DiagnosticTabs.predictivities.label.cex),y=AxesTab.predictivities[leavein][temp1]+bpar$DiagnosticTabs.predictivities.label.VertOffset*strheight("x",cex=bpar$DiagnosticTabs.predictivities.label.cex),labels=bpar$axes.label.text[variables.in][leavein][temp1],font=bpar$DiagnosticTabs.predictivities.label.font,cex=bpar$DiagnosticTabs.predictivities.label.cex,col=bpar$interaction.highlight.axes.col.bg)
    points(AxesTab.predictivities1dim[leavein][temp1],AxesTab.predictivities[leavein][temp1],col=bpar$interaction.highlight.axes.col.bg,bg=bpar$interaction.highlight.axes.col.bg,pch=bpar$DiagnosticTabs.predictivities.axes.pch,cex=bpar$DiagnosticTabs.predictivities.cex)
    if (tclvalue(AxesTab.ShowAxisLabels.var)=="1") text(x=AxesTab.predictivities1dim[leavein][temp2]+bpar$DiagnosticTabs.predictivities.label.HorizOffset*strwidth("x",cex=bpar$DiagnosticTabs.predictivities.label.cex),y=AxesTab.predictivities[leavein][temp2]+bpar$DiagnosticTabs.predictivities.label.VertOffset*strheight("x",cex=bpar$DiagnosticTabs.predictivities.label.cex),labels=bpar$axes.label.text[variables.in][leavein][temp2],font=bpar$DiagnosticTabs.predictivities.label.font,cex=bpar$DiagnosticTabs.predictivities.label.cex,col=bpar$interaction.highlight.axes.col.fg)
    points(AxesTab.predictivities1dim[leavein][temp2],AxesTab.predictivities[leavein][temp2],col=bpar$interaction.highlight.axes.col.fg,bg=bpar$interaction.highlight.axes.col.fg,pch=bpar$DiagnosticTabs.predictivities.axes.pch,cex=bpar$DiagnosticTabs.predictivities.cex)
      }
  }

AxesTab.replot<-function()
  {
  if (tclvalue(Biplot.Axes.var)%in%c("0","2")) tkrreplot(AxesTab.image,AxesTab.plot.predictivities,hscale=ConvergenceTab.HorizontalScale.func(),vscale=ConvergenceTab.VerticalScale.func())
...
PredictionsTab.update<-function()
  ...
  if (tclvalue(Biplot.points.mode)=="1")
    {
    PredictionsTab.arrayR<<-c("Variable",bpar$axes.label.text[variables.in],"Predicted",format(round(Biplot.points.WhichClosestOnAxis,bpar$DiagnosticTabs.predictions.digits),nsmall=bpar$DiagnosticTabs.predictions.digits,trim=TRUE),"Actual",rep(" ",p.in),"RelAbsErr%",rep(" ",p.in))
    dim(PredictionsTab.arrayR)<-c(p.in+1,4)
    for (i in 1:p.in) PredictionsTab.arrayTcl[[i,1]]<<-PredictionsTab.arrayR[i+1,2]
    }
    else if (tclvalue(Biplot.points.mode)=="2")
      {
      PredictionsTab.arrayR<<-c("Variable",bpar$axes.label.text[variables.in],"Predicted",format(round(Biplot.points.WhichClosestOnAxis,bpar$DiagnosticTabs.predictions.digits),nsmall=bpar$DiagnosticTabs.predictions.digits,trim=TRUE),"Actual",format(round(Data[samples.in[Biplot.points.WhichHighlight],variables.in],bpar$DiagnosticTabs.predictions.digits),nsmall=bpar$DiagnosticTabs.predictions.digits,trim=TRUE),"RelAbsErr%",format(round(abs(Biplot.points.WhichClosestOnAxis-Data[samples.in[Biplot.points.WhichHighlight],variables.in])/apply(Data[samples.in,variables.in],2,function(x) max(x)-min(x))*100,max(bpar$DiagnosticTabs.predictions.digits-2,0)),nsmall=max(bpar$DiagnosticTabs.predictions.digits-2,0),trim=TRUE))
      dim(PredictionsTab.arrayR)<-c(p.in+1,4)
      for (i in (1:p.in)) for (j in (1:3)) PredictionsTab.arrayTcl[[i,j]]<<-PredictionsTab.arrayR[i+1,j+1]
      }
  tkconfigure(PredictionsTab.table,variable=PredictionsTab.arrayTcl)
  }
...
ExportTab.update<-function()
  {
  if (length(ExportTab.data)>0) for (i in 1:length(ExportTab.data)) tcl(ExportTab.tree,"delete",paste("R",i,sep=""))
  ExportTab.data<<-NULL
  ...
  # General
  ExportTab.data<<-c(ExportTab.data,list("General"=c(
    "X, the matrix of original data",
    "Xtr, the matrix of transformed data")))
  ExportTab.data.name<<-c(ExportTab.data.name,list(c(
    "General.X",
    "General.Xtr")))
  ExportTab.data.func<<-c(ExportTab.data.func,list(c(
    function()
      {
  ...
  if (tclvalue(Biplot.Axes.var)=="0")
    {
    ExportTab.data<<-c(ExportTab.data,list("PCA"=c(
      "Vr, the matrix of basis vectors",
      "Y, the matrix of point coordinates",
      "Delta, the matrix of inter-point distances",
      "eigen, the vector of eigenvalues",
      "quality, the quality of the display",
    ...
    ExportTab.data.name<<-c(ExportTab.data.name,list(c(
      ...
      "PCA.eigen",
      "PCA.quality",
      "PCA.PointPredictivities",
      "PCA.AxisPredictivities",
      "PCA.adequacies")))
    if (tclvalue(tkget(SettingsBox.action.combo))=="Predict")
      ExportTab.data.name[[length(ExportTab.data.name)]]<<-c(ExportTab.data.name[[length(ExportTab.data.name)]],
        ...
        "PCA.RelMeanAbsErr")
    ExportTab.data.func<<-c(ExportTab.data.func,list(c(
     function() # Vr, the matrix of basis vectors
      {
      temp1<-Biplot.B_
     ...
     function() # Y, the matrix of point coordinates
      {
      temp1<-Biplot.Y_
      rownames(temp1)<-PointLabels[samples.in]
      colnames(temp1)<-paste("Dimension",1:2)
     ...
     function() # Delta, the matrix of inter-point distances
      {
     ...
     function() # eigen, the vector of eigenvalues
      {
      temp1<-eigen(t(Biplot.Xtransformed)%*%Biplot.Xtransformed,symmetric=TRUE)$values
      names(temp1)<-paste("Dimension",1:length(temp1))
      temp1
      },
     function() sum((temp1<-eigen(t(Biplot.Xtransformed)%*%Biplot.Xtransformed,symmetric=TRUE)$values)[1:2])/sum(temp1), # quality, the quality of the display
     function() # PointPredictivities, the matrix of point predictivities
      {
      temp1<-cbind(PointsTab.predictivities1dim,PointsTab.predictivities)
      rownames(temp1)<-PointLabels[samples.in]
      colnames(temp1)<-c("Dimension 1","Dimensions 1 and 2")
      temp1
      },
     function() # AxisPredictivities, the matrix of axis predictivities
      {
     ...
     function() # adequacies, the vector of adequacies
      {
    ...
    if (tclvalue(tkget(SettingsBox.action.combo))=="Predict")
      ExportTab.data.func[[length(ExportTab.data.func)]]<<-c(ExportTab.data.func[[length(ExportTab.data.func)]],
        function() # Pred, the list of predictions, actual values and percentage relative absolute errors
        {
        temp1<-list()
        for (i in 1:p.in)
          {
          temp2<-Biplot.Y_%*%Biplot.B_[i,]%*%t(Biplot.B_[i,])/sum(Biplot.B_[i,]^2)
          temp5<-cbind(temp3<-SettingsBox.BackTransformation.func(temp2[,1]/(Biplot.B_[i,1]/sum(Biplot.B_[i,]^2)),WhichCol=i),temp4<-Data[samples.in,variables.in[i]],abs(temp3-temp4)/(max(temp4)-min(temp4))*100)
          colnames(temp5)<-c("Prediction","Actual","RelAbsErr%")
          temp1<-c(temp1,list(temp5))
          }
        names(temp1)<-AxisLabels[variables.in]
        temp1
        },
        function() # RelMeanAbsErr, the vector of axis percentage relative mean absolute errors
        {
        temp3<-sapply(1:p.in,function(i)
          {
          temp1<-Biplot.Y_%*%Biplot.B_[i,]%*%t(Biplot.B_[i,])/sum(Biplot.B_[i,]^2)
          temp2<-Data[samples.in,variables.in[i]]
          mean(abs(SettingsBox.BackTransformation.func(temp1[,1]/(Biplot.B_[i,1]/sum(Biplot.B_[i,]^2)),WhichCol=i)-temp2)/(max(temp2)-min(temp2))*100)
          })
  ...
  if (tclvalue(Biplot.Axes.var)=="1")
    {
    ExportTab.data<<-c(ExportTab.data,list("Covariance/Correlation"=c(
      "Vr, the matrix of basis vectors",
      "Y, the matrix of point coordinates",
      "CovCor, the matrix of covariances/correlations",
      "eigen, the vector of eigenvalues",
      "quality, the quality of the display")))
    if (tclvalue(tkget(SettingsBox.action.combo))=="Predict")
      ExportTab.data[[length(ExportTab.data)]]<<-c(ExportTab.data[[length(ExportTab.data)]],
        ...
        "RelMeanAbsErr, the vector of axis percentage relative mean absolute errors")
    ExportTab.data.name<<-c(ExportTab.data.name,list(c(
      "CovarianceCorrelation.Vr",
      "CovarianceCorrelation.Y",
      "CovarianceCorrelation.CovCor",
      "CovarianceCorrelation.eigen",
      "CovarianceCorrelation.quality")))
    if (tclvalue(tkget(SettingsBox.action.combo))=="Predict")
      ExportTab.data.name[[length(ExportTab.data.name)]]<<-c(ExportTab.data.name[[length(ExportTab.data.name)]],
      "CovarianceCorrelation.Pred",
      "CovarianceCorrelation.RelMeanAbsErr")
    ExportTab.data.func<<-c(ExportTab.data.func,list(c(
      function() # Vr, the matrix of basis vectors
        {
        temp1<-Biplot.B_
      ...
      function() # CovCor, the matrix of covariances/correlations
        {
  ...
  if (tclvalue(Biplot.Axes.var)=="2")
    {
    ExportTab.data<<-c(ExportTab.data,list("CVA"=c(
      "Xbar, the matrix of sample group means",
      ...
      "N, the matrix of group sizes",
      "Vr, the matrix of axis basis vectors",
      "Y, the matrix of point coordinates",
      "Delta, the matrix of inter-point distances",
    ...
    ExportTab.data.name<<-c(ExportTab.data.name,list(c(
      ...
      "CVA.Y",
      "CVA.Delta",
      "CVA.PointPredictivities",
      "CVA.GroupPredictivities",
     ...
     if (tclvalue(tkget(SettingsBox.action.combo))=="Predict")
      ExportTab.data.name[[length(ExportTab.data.name)]]<<-c(ExportTab.data.name[[length(ExportTab.data.name)]],
        ...
        "CVA.RelMeanAbsErr")
    ExportTab.data.func<<-c(ExportTab.data.func,list(c(
      function() # Xbar, the matrix of sample group means
        {
        temp1<-apply(Biplot.Xtransformed,2,function(x) tapply(x,factor(group[samples.in],exclude=NULL),mean))
        rownames(temp1)<-bpar$groups.label.text[groups.in]
        colnames(temp1)<-AxisLabels[variables.in]
        temp1
        },
      function() # B, the matrix of between-groups sums-of-squares-and-crossproducts
        {
        Xbar<-apply(Biplot.Xtransformed,2,function(x) tapply(x,factor(group[samples.in],exclude=NULL),mean))
        N<-diag(table(factor(group[samples.in],exclude=NULL)))
        temp1<-t(Xbar)%*%N%*%Xbar
      ...
      function() # W, the matrix of within-groups sums-of-squares-and-crossproducts
        {
        Xbar<-apply(Biplot.Xtransformed,2,function(x) tapply(x,factor(group[samples.in],exclude=NULL),mean))
        N<-diag(table(factor(group[samples.in],exclude=NULL)))
        Bcva<-t(Xbar)%*%N%*%Xbar
        temp1<-t(Biplot.Xtransformed)%*%Biplot.Xtransformed-Bcva
        rownames(temp1)<-AxisLabels[variables.in]
        colnames(temp1)<-AxisLabels[variables.in]
        temp1
        },
      function() # N, the matrix of group sizes
        {
        temp1<-diag(table(factor(group[samples.in],exclude=NULL)))
        rownames(temp1)<-bpar$groups.label.text[groups.in]
        colnames(temp1)<-bpar$groups.label.text[groups.in]
        temp1
        },
      function() # Vr, the matrix of axis basis vectors
        {
        temp1<-Biplot.B_
        rownames(temp1)<-AxisLabels[variables.in]
        colnames(temp1)<-paste("Dimension",1:2)
      ...
      function() # PointPredictivities, the vector of point predictivities
        {
        temp1<-cbind(PointsTab.predictivities1dim,PointsTab.predictivities)
        rownames(temp1)<-PointLabels[samples.in]
        colnames(temp1)<-c("Dimension 1","Dimensions 1 and 2")
      ...
      function() # GroupPredictivities, the vector of group predictivities
        {
        temp1<-cbind(GroupsTab.predictivities1dim,GroupsTab.predictivities)
        rownames(temp1)<-bpar$groups.label.text[groups.in]
        colnames(temp1)<-c("Dimension 1","Dimensions 1 and 2")
        temp1
        },
      function() # AxisPredictivities, the vector of axis predictivities
        {
        temp1<-cbind(AxesTab.predictivities1dim,AxesTab.predictivities)
        rownames(temp1)<-AxisLabels[variables.in]
        colnames(temp1)<-c("Dimension 1","Dimensions 1 and 2")
  ...
  # PCO
  if (as.numeric(tclvalue(Biplot.Axes.var))>=10 && tclvalue(Points.var)=="0")
    {
    ExportTab.data<<-c(ExportTab.data,list("PCO"=c(
      "D, the matrix of inter-sample dissimilarities",
      "B, the matrix of sums-of-squares-and-crossproducts",
      "Y, the matrix of point coordinates",
      "Delta, the matrix of inter-point distances",
      "eigenB, the vector of eigenvalues of B",
      "quality, the quality of the display")))
    ExportTab.data.name<<-c(ExportTab.data.name,list(c(
      "PCO.D",
      "PCO.B",
      ...
      "PCO.quality")))
    ExportTab.data.func<<-c(ExportTab.data.func,list(c(
      function() # D, the matrix of inter-sample dissimilarities
        {
      ...
      function() # B, the matrix of sums-of-squares-and-crossproducts
        {
        temp1<-matrix(0,nrow=n.in,ncol=n.in)
        temp1[cbind(1:n.in,1:n.in)]<-1
        temp1<-temp1-1/n.in
        temp2<--.5*temp1%*%(Points.DistanceMetric.DissimilarityMatrix^2)%*%temp1
      ...
      function() # eigenB, the vector of eigenvalues of B
        {
        temp1<-matrix(0,nrow=n.in,ncol=n.in)
        temp1[cbind(1:n.in,1:n.in)]<-1
        temp1<-temp1-1/n.in
      ...
      function() # quality, the quality of the display
        {
        temp1<-matrix(0,nrow=n.in,ncol=n.in)
        temp1[cbind(1:n.in,1:n.in)]<-1
        temp1<-temp1-1/n.in
  ...
  # MDS identity
  if (as.numeric(tclvalue(Biplot.Axes.var))>=10 && tclvalue(Points.var)=="10")
    {
    ExportTab.data<<-c(ExportTab.data,list("MDS"=c(
      "D, the matrix of inter-sample dissimilarities",
      "Y, the matrix of point coordinates",
      "Delta, the matrix of inter-point distances",
    ...
    ExportTab.data.func<<-c(ExportTab.data.func,list(c(
      ...
      function() # Y0, the matrix of initial point coordinates
        {
      ...
      function() ConvergenceTab.points.StressVector # stress, the vector of stress by iteration
      )))
    }
  # MDS monotone regression, MDS monotone spline transformation
  if (as.numeric(tclvalue(Biplot.Axes.var))>=10 && tclvalue(Points.var)%in%c("11","12"))
    {
    ExportTab.data<<-c(ExportTab.data,list("MDS"=c(
      "D, the matrix of inter-sample dissimilarities",
    ...
    ExportTab.data.func<<-c(ExportTab.data.func,list(c(
      ...
      function() # Dhat, the matrix of inter-sample disparities
        {
  ...
  if (tclvalue(Biplot.Axes.var)=="11")
    {
    ExportTab.data<<-c(ExportTab.data,list("Regression"=c(
    ...
    if (tclvalue(tkget(SettingsBox.action.combo))=="Predict")
      ExportTab.data.name[[length(ExportTab.data.name)]]<<-c(ExportTab.data.name[[length(ExportTab.data.name)]],
      "Regression.Pred",
      "Regression.RelMeanAbsErr")
    ExportTab.data.func<<-c(ExportTab.data.func,list(c(
      function() # B, the matrix of regression coefficients
        {
        temp1<-Biplot.B
        rownames(temp1)<-AxisLabels[variables.in]
        colnames(temp1)<-paste("Dimension",1:2)
    ...
    if (tclvalue(tkget(SettingsBox.action.combo))=="Predict")
      ExportTab.data.func[[length(ExportTab.data.func)]]<<-c(ExportTab.data.func[[length(ExportTab.data.func)]],
        function() # Pred, the list of predictions, actual values and percentage relative absolute errors
          ...
          for (i in 1:p.in)
            {
            temp2<-Biplot.Y%*%Biplot.B[i,]%*%t(Biplot.B[i,])/sum(Biplot.B[i,]^2)
            temp5<-cbind(temp3<-SettingsBox.BackTransformation.func(temp2[,1]/(Biplot.B[i,1]/sum(Biplot.B[i,]^2)),WhichCol=i),temp4<-Data[samples.in,variables.in[i]],abs(temp3-temp4)/(max(temp4)-min(temp4))*100)
            colnames(temp5)<-c("Prediction","Actual","RelAbsErr%")
            temp1<-c(temp1,list(temp5))
            }
          names(temp1)<-AxisLabels[variables.in]
        ...
        function() # RelMeanAbsErr, the vector of axis percentage relative mean absolute errors
          ...
          temp3<-sapply(1:p.in,function(i)
            {
            temp1<-Biplot.Y%*%Biplot.B[i,]%*%t(Biplot.B[i,])/sum(Biplot.B[i,]^2)
            temp2<-Data[samples.in,variables.in[i]]
            mean(abs(SettingsBox.BackTransformation.func(temp1[,1]/(Biplot.B[i,1]/sum(Biplot.B[i,]^2)),WhichCol=i)-temp2)/(max(temp2)-min(temp2))*100)
            })
          names(temp3)<-AxisLabels[variables.in]
          temp3
          } )
    }
  # Procrustes: prediction
   if (tclvalue(Biplot.Axes.var)=="12" && tclvalue(tkget(SettingsBox.action.combo))=="Predict")
    {
    ExportTab.data<<-c(ExportTab.data,list("Procrustes"=c(
      "Q, the orthogonal Procrustes matrix",
      "Pred, the list of predictions, actual values and percentage relative absolute errors",
      "RelMeanAbsErr, the vector of axis percentage relative mean absolute errors")))
    ExportTab.data.name<<-c(ExportTab.data.name,list(c(
      "Procrustes.Q",
      "Procrustes.Pred",
      "Procrustes.RelMeanAbsErr")))
    ExportTab.data.func<<-c(ExportTab.data.func,list(c(
      function() # Q, the orthogonal Procrustes matrix
        {
        temp1<-Biplot.B
        rownames(temp1)<-AxisLabels[variables.in]
        colnames(temp1)<-paste("Dimension",1:2)
  ...
  if (tclvalue(Biplot.Axes.var)=="12" && tclvalue(tkget(SettingsBox.action.combo))!="Predict")
    {
    ExportTab.data<<-c(ExportTab.data,list("Procrustes"=c(
      "P, the minimal projection error Procrustes matrix")))
    ExportTab.data.name<<-c(ExportTab.data.name,list(c(
      "Procrustes.P")))
    ExportTab.data.func<<-c(ExportTab.data.func,list(c(
      function() # P, the minimal projection error Procrustes matrix
        {
        temp1<-Biplot.B
        rownames(temp1)<-AxisLabels[variables.in]
        colnames(temp1)<-paste("Dimension",1:2)
  ...
  if (tclvalue(Biplot.Axes.var)=="13")
    {
    ExportTab.data<<-c(ExportTab.data,list("Circular non-linear"=c(
      "Axis, the list of axis coordinates and marker labels")))
    if (tclvalue(tkget(SettingsBox.action.combo))=="Predict")
      ExportTab.data[[length(ExportTab.data)]]<<-c(ExportTab.data[[length(ExportTab.data)]],
        "Pred, the list of predictions, actual values and percentage relative absolute errors",
        "RelMeanAbsErr, the vector of axis percentage relative mean absolute errors")
    ExportTab.data.name<<-c(ExportTab.data.name,list(c(
      "CircularNonLinear.Axis")))
    if (tclvalue(tkget(SettingsBox.action.combo))=="Predict")
      ExportTab.data.name[[length(ExportTab.data.name)]]<<-c(ExportTab.data.name[[length(ExportTab.data.name)]],
      "CircularNonLinear.Pred",
      "CircularNonLinear.RelMeanAbsErr")
    ExportTab.data.func<<-c(ExportTab.data.func,list(c(
      function() # Axis, the list of axis coordinates and marker labels
        {
        temp1<-Biplot.axis
        names(temp1)<-AxisLabels[variables.in]
        for (i in 1:length(temp1)) colnames(temp1[[i]])<-paste("Dimension",1:2)
        temp1
        } )))
    if (tclvalue(tkget(SettingsBox.action.combo))=="Predict")
      ExportTab.data.func[[length(ExportTab.data.func)]]<<-c(ExportTab.data.func[[length(ExportTab.data.func)]],
        function() # Pred, the list of predictions, actual values and percentage relative absolute errors
          ...
          for (i in 1:p.in)
            {
            temp4<-cbind(temp1[,i],temp3<-Data[samples.in,variables.in[i]],abs(temp1[,i]-temp3)/(max(temp3)-min(temp3))*100)
            colnames(temp4)<-c("Prediction","Actual","RelAbsErr%")
        ...
        function() # RelMeanAbsErr, the vector of axis percentage relative mean absolute errors
          {
          temp1<-t(apply(Biplot.Y,1,function(x) Axes.CircularNonLinear.predictions(x)))
          temp2<-colMeans(sweep(abs(temp1-Data[samples.in,variables.in]),2,apply(Data[samples.in,variables.in],2,function(x) max(x)-min(x)),"/")*100,na.rm=TRUE)
          names(temp2)<-AxisLabels[variables.in]
          temp2
          }
        )
    }
  # Interpolate: a new sample
  if (tclvalue(Additional.Interpolate.ANewSample.var)=="1")
    {
    ExportTab.data<<-c(ExportTab.data,list("Interpolate: A new sample"=c(
      "Coord, the vector of coordinates")))
    ExportTab.data.name<<-c(ExportTab.data.name,list(c(
      "InterpolateANewSample.Coord")))
    ExportTab.data.func<<-c(ExportTab.data.func,list(c(
      function() # Coord, the vector of coordinates
        {
  ...
  # Interpolate: sample group means
  if (tclvalue(Additional.Interpolate.SampleGroupMeans.var)=="1")
    {
    ExportTab.data<<-c(ExportTab.data,list("Interpolate: Sample group means"=c(
      "Coord, the matrix of coordinates")))
    ExportTab.data.name<<-c(ExportTab.data.name,list(c(
      "InterpolateSampleGroupMeans.Coord")))
    ExportTab.data.func<<-c(ExportTab.data.func,list(c(
      function() # Coord, the matrix of coordinates
        {
  ...
  if (tclvalue(Additional.ConvexHull.var)=="1")
    {
    ExportTab.data<<-c(ExportTab.data,list("Convex hulls"=c(
      "Coord, the list of coordinates")))
    ExportTab.data.name<<-c(ExportTab.data.name,list(c(
      "ConvexHulls.Coord")))
    ExportTab.data.func<<-c(ExportTab.data.func,list(c(
      function() # Coord, the list of coordinates
        {
        temp1<-Additional.ConvexHullAlphaBag.coordinates
        names(temp1)<-switch(as.character(Additional.ConvexHullAlphaBag.for),
          "-1"="All points",
          "0"=bpar$groups.label.text[groups.in],
          bpar$groups.label.text[Additional.ConvexHullAlphaBag.for])
        for (i in 1:length(temp1)) colnames(temp1[[i]])<-paste("Dimension",1:2)
        temp1
        }
  ...
  if (tclvalue(Additional.AlphaBag.var)=="1")
    {
    ExportTab.data<<-c(ExportTab.data,list("Alpha-bags"=c(
      "Coord, the list of coordinates")))
    ...
    ExportTab.data.func<<-c(ExportTab.data.func,list(c(
      ...
      if (Additional.ConvexHullAlphaBag.ShowTukeyMedian)
        ExportTab.data.func[[length(ExportTab.data.func)]]<<-c(ExportTab.data.func[[length(ExportTab.data.func)]],
          function() # Tukey, the list of coordinates
            {
  ...
  if (tclvalue(Additional.PointDensities.var)=="1")
    {
    ExportTab.data<<-c(ExportTab.data,list("Point densitities"=c(
      "Dens, the list of point densities")))
    ExportTab.data.name<<-c(ExportTab.data.name,list(c(
      "PointDensities.Dens")))
    ExportTab.data.func<<-c(ExportTab.data.func,list(c(
      function() # Dens, the list of point densities
        {
  ...
  if (tclvalue(Additional.ClassificationRegion.var)=="1")
    {
    ExportTab.data<<-c(ExportTab.data,list("Classification regions"=c(
      "Class, the list of classification regions")))
    ExportTab.data.name<<-c(ExportTab.data.name,list(c(
      "ClassificationRegions.Class")))
    ExportTab.data.func<<-c(ExportTab.data.func,list(c(
      function() # Class, the list of classification regions
        {
          xseq<-seq(Biplot.par$usr[1],Biplot.par$usr[2],length=bpar$ClassificationRegion.PixelsPerBiplotDimension)
          yseq<-seq(Biplot.par$usr[3],Biplot.par$usr[4],length=bpar$ClassificationRegion.PixelsPerBiplotDimension)
          if (Additional.ClassificationRegion.dimensions==1) L<-as.matrix(xseq)
          ...
          if (Additional.ClassificationRegion.dimensions==1) list(x1=xseq,x2=yseq,class=class.region)
            else list(x1=xseq,x2=yseq,class=class.region)
        }
      )))
    }
  ...
  for (i in 1:length(ExportTab.data))
    {
    tkinsert(ExportTab.tree,"end","root",paste("R",i,sep=""),text=names(ExportTab.data)[i],deltax=10)
    for (j in 1:length(ExportTab.data[[i]])) tkinsert(ExportTab.tree,"end",paste("R",i,sep=""),paste("R",i,"N",j,sep=""),text=ExportTab.data[[i]][j],deltax=5)
  ...
  tkconfigure(ExportTab.tree,selectcommand=function(...)
    ...
    if (nchar(ItemIdent)>2)
      {
      ExportTab.Rvalue<<-as.numeric(substr(ItemIdent,start=2,stop=2))
      ExportTab.Nvalue<<-as.numeric(substr(ItemIdent,start=4,stop=nchar(ItemIdent)))
      tkconfigure(ExportTab.DisplayInConsole.but,state="normal")
      tkconfigure(ExportTab.ExportToWorkspace.but,state="normal")
      }
      else
        {
        tkconfigure(ExportTab.DisplayInConsole.but,state="disabled")
...
tkplace(ExportTab.ExportToWorkspace.but,relx=.520,rely=.92,relwidth=.28,height=22,"in"=DiagnosticTabs.Export,anchor="w")
...
Kraal.plot<-function()
  {
  par(mar=c(0,0,0,0),"bg"="white")
  plot(rbind(Kraal.points.Y,Kraal.axes.Y ),type="n",xaxt="n",yaxt="n",xaxs="i",yaxs="i",xlim=c(0,1),ylim=c(0,1),main="",xlab="",ylab="",bty="n")
  points(Kraal.grid,pch="-",cex=.5,col="gray95")
  if (length(Kraal.samples.in)>0)
    {
    text(Kraal.points.Y[,1]+bparp$points.label.HorizOffset[Kraal.samples.in]*strwidth("x",cex=bparp$points.label.cex[Kraal.samples.in]),Kraal.points.Y[,2]+bparp$points.label.VertOffset[Kraal.samples.in]*strheight("x",cex=bparp$points.label.cex[Kraal.samples.in]),labels=bparp$points.label.text[Kraal.samples.in],font=bparp$points.label.font[Kraal.samples.in],cex=bparp$points.label.cex[Kraal.samples.in],col=bparp$points.label.col[Kraal.samples.in])
    points(Kraal.points.Y,pch=bparp$points.pch[Kraal.samples.in],cex=bparp$points.cex[Kraal.samples.in],col=bparp$points.col.fg[Kraal.samples.in],bg=bparp$points.col.bg[Kraal.samples.in])
    }
  if (length(Kraal.variables.in)>0)
    {
    text(Kraal.axes.Y[,1],Kraal.axes.Y[,2]-.9*strheight("x",cex=bpar$axes.label.cex),label=bpar$axes.label.text[Kraal.variables.in],font=bpar$axes.label.font[Kraal.variables.in],cex=bpar$axes.label.cex[Kraal.variables.in],col=bpar$axes.label.col[Kraal.variables.in])
    linelength<-3*strwidth("x",cex=bpar$axes.label.cex)
    segments(x0=Kraal.axes.Y[,1]-linelength/2,y0=Kraal.axes.Y[,2],x1=Kraal.axes.Y[,1]+linelength/2,y1=Kraal.axes.Y[,2],lty=bpar$axes.lty[Kraal.variables.in],lwd=bpar$axes.lwd[Kraal.variables.in],col=bpar$axes.col[Kraal.variables.in])
    }
  Kraal.par<<-par()
...
Kraal.LeftClick<-function(x,y)
  ...
  if (Kraal.OverPoint())
    {
    Kraal.moving.which<<-Kraal.samples.in[which.min(PythagorasDistance(matrix(Kraal.XY.move,nrow=1),Kraal.points.Y))]
    Kraal.moving.type<<-"point"
    Kraal.moving.status<<-TRUE
    }
    else if (Kraal.OverAxis())
      {
      Kraal.moving.which<<-Kraal.variables.in[which.min(PythagorasDistance(matrix(Kraal.XY.move,nrow=1),Kraal.axes.Y))]
      Kraal.moving.type<<-"axis"
      Kraal.moving.status<<-TRUE
      }
  }
...
Kraal.LeftRelease<-function(x,y)
  ...
  if (Kraal.moving.status)
    ...
    if (temp1[1]<0 || temp1[1]>1 || temp1[2]<0 || temp1[2]>1)
      {
      GUI.BindingsOff()
      Kraal.out.func()
      GUI.BindingsOn()
      ...
      if (Kraal.moving.type=="point")
        {
        ClosestGridPosition<-which.min(PythagorasDistance(matrix(temp1,ncol=2),Kraal.grid[Kraal.grid.open,]))
        temp2<-Kraal.samples.in==Kraal.moving.which
        temp3<-Kraal.grid.open[ClosestGridPosition]
        Kraal.points.Y[temp2,]<<-Kraal.grid[temp3,]
        Kraal.grid.open<<-sort(c(Kraal.samples.WhereInGrid[temp2],Kraal.grid.open[-ClosestGridPosition]))
        Kraal.samples.WhereInGrid[temp2]<<-temp3
          ...
          Kraal.grid.open<<-sort(c(Kraal.variables.WhereInGrid[temp2],Kraal.grid.open[-ClosestGridPosition]))
...
Kraal.RightClick<-function(x,y)
  ...
  if (n.in<n && Kraal.OverPoint()) tkpopup(Kraal.RightClickOnPoint.Menu,tclvalue(tkwinfo("pointerx",Kraal.image)),tclvalue(tkwinfo("pointery",Kraal.image)))
    else if (p.in<p && Kraal.OverAxis()) tkpopup(Kraal.RightClickOnAxis.Menu,tclvalue(tkwinfo("pointerx",Kraal.image)),tclvalue(tkwinfo("pointery",Kraal.image)))
...
Kraal.in.func<-function(FollowThrough=TRUE)
  ...
  if (n-n.in==0 && p-p.in==0)
    ...
    temp5<-1
      ...
      {
      temp1<-PythagorasDistance(Kraal.grid[Kraal.grid.open,],rbind(Kraal.points.Y,Kraal.axes.Y))
      temp2<-apply(temp1,1,min)
      temp3<-which(round(temp2,8)==round(max(temp2,na.rm=TRUE),8))
      if (length(temp3)==1) temp5<-temp3
        else
          {
          temp4<-temp3[which(round(Kraal.grid[Kraal.grid.open[temp3],2],8)==round(max(Kraal.grid[Kraal.grid.open[temp3],2]),8))]
          if (length(temp4)==1) temp5<-temp4
            else temp5<-temp4[which.min(Kraal.grid[Kraal.grid.open[temp4],1])]
          }
    ...
    if (Kraal.moving.type=="point")
      ...
      Kraal.samples.WhereInGrid<<-c(Kraal.samples.WhereInGrid,Kraal.grid.open[temp5])
      ...
      Kraal.variables.WhereInGrid<<-c(Kraal.variables.WhereInGrid,Kraal.grid.open[temp5])
      ...
      if (Biplot.axes.mode==1 && Biplot.axes.WhichHighlight==Kraal.moving.which)
        ...
        Biplot.axes.mode<<-0
        tkentryconfigure(Biplot.RightClickInside.Menu,8,state="normal")
        if (tclvalue(Biplot.Axes.var)%in%c("0","2")) AxesTab.update<<-TRUE
        }
      PredictionsTab.ArraySetup()
      for (temp7 in 1:2) tkentryconfigure(Kraal.RightClick.Menu,temp7,state="normal")
      }
  Kraal.replot()
  if (FollowThrough) Kraal.FollowThrough.cmd()
  }
...
Kraal.out.func<-function(FollowThrough=TRUE)
  ...
  if (Kraal.moving.type=="point")
    {
    Kraal.points.Y<<-matrix(Kraal.points.Y[Kraal.samples.in!=Kraal.moving.which],ncol=2)
    Kraal.grid.open<<-sort(c(Kraal.grid.open,Kraal.samples.WhereInGrid[Kraal.samples.in==Kraal.moving.which]))
    Kraal.samples.WhereInGrid<<-Kraal.samples.WhereInGrid[Kraal.samples.in!=Kraal.moving.which]
    ...
    if (n.in==n)
      ...
      {
      Kraal.axes.Y<<-matrix(Kraal.axes.Y[Kraal.variables.in!=Kraal.moving.which],ncol=2)
      Kraal.grid.open<<-sort(unique(c(Kraal.grid.open,Kraal.variables.WhereInGrid[Kraal.variables.in==Kraal.moving.which])))
      Kraal.variables.WhereInGrid<<-Kraal.variables.WhereInGrid[Kraal.variables.in!=Kraal.moving.which]
      variables.in<<-sort(c(variables.in,Kraal.moving.which))
      p.in<<-length(variables.in)
      Kraal.variables.in<<-Kraal.variables.in[Kraal.variables.in!=Kraal.moving.which]
      Additional.Interpolate.ANewSample.var<<-tclVar("0")
      Additional.Interpolate.ANewSample.values<<-matrix(colMeans(Data[samples.in,variables.in]),ncol=1)
      if (p.in==p)
        {
...
Kraal.FollowThrough.cmd<-function()
  ...
  if (any(Data[samples.in,variables.in]<=0))
    {
    if (substr(tclvalue(tkget(SettingsBox.transformation.combo)),1,3)=="Log")
      {
      tkmessageBox(title="Transformation",parent=GUI.TopLevel,message="Log-transformations are available only when all variable values are positive.",icon="warning",type="ok")
      tkfocus(GUI.TopLevel)
      tkconfigure(SettingsBox.transformation.combo,values=c("Centre","Centre, scale","Unitise, centre"),text="Centre",height=3)
      }
      else tkconfigure(SettingsBox.transformation.combo,values=c("Centre","Centre, scale","Unitise, centre"),height=3)
    }
    else tkconfigure(SettingsBox.transformation.combo,values=c("Centre","Centre, scale","Unitise, centre","Log, centre","Log, centre, scale","Log, unitise, centre"),height=6)
